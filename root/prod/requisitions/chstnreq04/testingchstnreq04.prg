...
head o.order_id
 ...
 if(op.communication_type_cd = 6006_COSIGNED_REQ_CD)
 	call echo(build("op.communication_type_cd = 6006_COSIGNED_REQ_CD"))
   ...
   and pl2.POSITION_CD in (88_PHYSICIANCARDIOLOGY_CD			;oa.order_provider_id
 	...
 	call echo(build("op.communication_type_cd = ",uar_get_code_display(op.communication_type_cd)))
    call echo(build("ono.notification_type_flag = ",trim(cnvtstring(ono.notification_type_flag))))
 ...
 call echo(build("if (ono.notification_status_flag = 2 and ono.NOTIFICATION_TYPE_FLAG = 2 )"))
 call echo(build("ono.notification_status_flag =",trim(cnvtstring(ono.notification_status_flag)),
 				 "and ono.NOTIFICATION_TYPE_FLAG = ",trim(cnvtstring( ono.NOTIFICATION_TYPE_FLAG)) ))
 if (ono.notification_status_flag = 2 and ono.NOTIFICATION_TYPE_FLAG = 2 )
 	 call echo(build("o.CATALOG_TYPE_CD =",uar_get_code_display(o.CATALOG_TYPE_CD)))
     	if(o.CATALOG_TYPE_CD = 6000_AMBULATORYPROCEDURES_CD)
            ...
            call echo("still inside 6000_AMBULATORYPROCEDURES_CD")                  
           elseif
           				(pl2.POSITION_CD in (
                                88_PHYSANESTHESIOLOGY_CD,
                                88_PHYSCOLONANDRECTALSURGERY_CD,
                   ...
                   orders->qual[ord_cnt].elec_signby=build2(orders->qual[ord_cnt].order_dr,"  NPI #:",orders->qual[ord_cnt].ord_npi)
 			call echo(build("pl2.POSITION_CD=",uar_Get_code_display(pl2.position_cd)))
            endif
           else 
            call echo(build("OUTSIDE pl2.POSITION_CD=",uar_Get_code_display(pl2.position_cd)))
 	    endif
 		
 		call echo(build("OUTSIDE pl2.POSITION_CD=",uar_Get_code_display(pl2.position_cd)))
 		call echo(build("OUTSIDE o.CATALOG_TYPE_CD =",uar_get_code_display(o.CATALOG_TYPE_CD)))	
 endif
