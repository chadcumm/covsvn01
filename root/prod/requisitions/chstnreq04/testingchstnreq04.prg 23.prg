  ...
  for (y = 1 to orders->qual[x].d_cnt)
    if (orders->qual[x].d_qual[y].special_ind = 1)
      ...
      for (z = 1 to pt->line_cnt)
        ...
        set orders->qual[x].disp_ln_qual[z].disp_line = trim(pt->lns[z].line)
        call echo(build2("orders->qual[",x,"].disp_ln_qual[",z,"].disp_line=",orders->qual[x].disp_ln_qual[z].disp_line))
      endfor
    endif
  endfor
endfor
...
;021
 ...
 call echo("Printing disp_ln_qual")
        
      for (zz = 1 to orders->qual[vv].disp_ln_cnt)
      	"{CPI/14}", call print(calcpos(250,ycol))
        call echo(build2("orders->qual[",vv,"].disp_ln_qual[",zz,"].disp_line=",orders->qual[vv].disp_ln_qual[zz].disp_line))
