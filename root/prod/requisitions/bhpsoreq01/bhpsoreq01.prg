...
/*****************************************************************************
        ...
        Source file name:       bhpsoreq01.PRG
        Object name:            bhpsoreq01
        Request #:              N/A
...
;    *028 06/10/18 ccumminr				updated for onetime pso printout    *
;~DE~************************************************************************
 
drop program bhpsoreq01:dba go
create program bhpsoreq01:dba
...
( 1 name = vc
  ...
  1 sex = vc
  1 disch_dt_tm = vc
  1 order_dr_name = vc ;028
  1 order_dr_npi = vc ;028
...
*****************************************************************************/
declare modnum				= c4 with protect,constant("v028") ;026;027
;declare filename			= vc with protect,constant(concat(trim(cnvtlower(curprog)),"_",trim(cnvtstring(request->order_dr_id))))	;026
declare filename			= vc with protect,constant(concat("bh_pso_",trim(cnvtstring(request->order_dr_id))))	;026
...
if (allergy->cnt > 0)
   set pt->line_cnt = 0
   set max_length = 65          ; Number of characters to be prined per line
...
head o.order_id
 ...
 ;028 start
 orders->qual[ord_cnt].elec_signby=build2(orders->qual[ord_cnt].order_dr,"  NPI #:",orders->qual[ord_cnt].ord_npi)
 orders->qual[ord_cnt].elec_signby2=build2(orders->qual[ord_cnt].order_dr,"  NPI #:",orders->qual[ord_cnt].ord_npi)
 ;028 end
...
join of1
  ...
  and   of1.description  in("Bed Type","Inpatient Admit Date and Time","Reason for Hospitalization","Admit To")
...
plan p
	where p.person_id = request->order_dr_id
join pa
    where outerjoin(p.PERSON_ID)=(pa.PERSON_ID)
    and pa.ACTIVE_IND = outerjoin(1)
    and pa.PRSNL_ALIAS_TYPE_CD=outerjoin(NPI_CD)    ;021 
detail
	orders->order_dr_name = p.name_full_formatted
	orders->order_dr_npi = trim(pa.alias)
with nocounter

for (x = 1 to order_cnt)
	set orders->qual[x].elec_signby=build2(orders->order_dr_name,"  NPI #:",orders->order_dr_npi)
	set orders->qual[x].elec_signby2=build2(orders->order_dr_name,"  NPI #:",orders->order_dr_npi)
endfor
/******************************************************************************
...
execute cpm_create_file_name filename,"pdf"	;028
...
head page
 ...
 ;021_head
  ...
  ;028 "{CPI/14}{POS/380/210}", "Attending MD: "
  ;028 if (orders->attending_cnt = 1 AND attending_line_cnt = 1)                                     /*13 starts here*/
  ;028   "{CPI/14}{POS/440/210}", call print(orders->attending[1].attending_phy),row+1
  ;028 elseif (orders->attending_cnt = 1 AND attending_line_cnt > 1 AND attending_wrap = 1)
  ;028   "{CPI/14}{POS/440/210}", call print(orders->attending[1].attending_phy),row+1
  ;028   "{CPI/14}{POS/440/220}", call print(orders->attending[2].attending_phy),row+1
  ;028 elseif (orders->attending_cnt > 1 AND attending_wrap = 0)
  ;028   "{CPI/14}{POS/440/210}", call print(orders->attending[1].attending_phy),row+1
  ;028   "{CPI/14}{POS/440/220}", "More Attending MDs Exist",row+1
  ;028 endif                                                                                         /*13 ends here*/
...
detail
  ...
  if (orders->qual[vv]->display_ind = 1)
    ...
    ycol=ycol+10
  ;028  "{CPI/14}{b}", call print(calcpos(440,ycol)), "Order Action: ",
  ;028      call print(uar_get_code_display(orders->qual[vv].action_type_cd)), row+1
 
;021
       ;NPI FOR ORDER PROVIDER
    Var1= build2(orders->order_dr_name,"  NPI #:",orders->order_dr_npi )
...
;028	if((orders->qual[vv].sup_signby != null) and (orders->adt_ind = 1))		;022
 																			;022
;028	    "{CPI/14}", call print(calcpos(85,ycol)), "Supervising Provider: ",	;022
;028	        call print(orders->qual[vv].sup_signby);, row+1					;022
;028	endif																	;022
...
/* 028    if(orders->qual[vv].elec_signby != null)
        ycol = ycol + 10
...
028 */ 
  ...
  ;028 ycol = ycol + 10
   ;028 "{CPI/14}", call print(calcpos(85,ycol)), "Entered By: ",
    ;028    call print(orders->qual[vv].enter_by);, row+1
 
 
    ;CHANGED THE POSITION OF COMMUNICATION TYPE     ;021
 
   ;028 ycol = ycol + 10
;028    "{CPI/14}", call print(calcpos(85,ycol)), "Communication Type: ",
    ;028    call print(orders->qual[vv].communication_type)
    ...
    "{CPI/14}", call print(calcpos(440,ycol+18)), "Order Dt/Tm: ",
        call print(orders->qual[vv].signed_dt), row+1
...
ycol = ycol + 100
"{CPI/13}{B}", call print(calcpos(85,ycol)), "Signature: _____________________________________________________ ", "{ENDB}" 
"{CPI/13}{B}", call print(calcpos(450,ycol)), "Date : __________________ ", "{ENDB}" 

;028 with nocounter, maxrow=800, maxcol=750, dio=postscript
with nocounter, maxrow=800, maxcol=750, dio=pdf ;028

;028 set spool = value(trim(cpm_cfn_info->file_name_path)) value(trim(request->printer_name)) with deleted
free set dclcom 
set dclcom = build2("mv ",cpm_cfn_info->FILE_NAME_FULL_PATH," /cerner/d_p0665/ccluserdir")
set dclstat = 0 
...
#exit_script
set last_mod = "029 updated for BH PSO"
end
go
