...
002 	06/10/2019	CCUMMIN4				Updated with EMTALA DX Logic
...
DECLARE DIAG_DISCH_VAR		 = F8 WITH CONSTANT(UAR_GET_CODE_BY("DISPLAY_KEY",17,"DISCHARGE")),PROTECT ;002
DECLARE CC_VAR				 = F8 WITH CONSTANT(UAR_GET_CODE_BY("DISPLAY_KEY",17,"REASONFORVISIT")),PROTECT	;cheif complaint 002
...
DETAIL
	...
	case (dtcode)
		of DIAG_DISCH_VAR :
												if (d_cnt = 1) a->qual[d1.seq].diagnosis  = dx
												else
													a->qual[d1.seq].diagnosis = concat(a->qual[d1.seq].diagnosis, ", ", dx)
												endif

		of cc_var		  : a->qual[d1.seq].cc_dx	   = dx
	endcase
