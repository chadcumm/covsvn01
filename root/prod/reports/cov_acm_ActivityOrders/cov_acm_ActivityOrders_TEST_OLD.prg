...
******************************************************************************
	...
	Executing from:		CCL
 	...
 	Special Notes:		Report Types:
 	
						  1 - Patients currently in a bed (Active Patients)
...
******************************************************************************
 	...
 	----------	--------------------	--------------------------------------
001	02/19/2021	Todd A. Blanchard		Added prompt for report type.
										Added DTA for Activity Status ADL.
******************************************************************************/
 
drop program cov_acm_ActivityOrders_TEST:DBA go
create program cov_acm_ActivityOrders_TEST:DBA
...
with OUTDEV, facility, nurse_unit, start_datetime, end_datetime, report_type
...
declare dta_ambqual_var					= f8 with constant(1908507.00) ; Ambulation Quality
declare dta_activity_status_adl_var		= f8 with constant(700588.00) ; Activity Status ADL ;001

declare op_facility_var			= c2 with noconstant("")
declare op_nurseunit_var		= c2 with noconstant("")

declare encntr_sql				= vc with noconstant("1 = 1") ;001
...
; define sql for $report_type ;001
if ($report_type = 1)
    set encntr_sql = build2("e.encntr_status_cd not in (discharged_var)",
					    	"and e.loc_bed_cd > 0.0")
    
elseif ($report_type = 2)
    set encntr_sql = build2("e.encntr_status_cd in (discharged_var)",
    						"and e.disch_dt_tm > e.reg_dt_tm")
...
where
	o.activity_type_cd = patient_activity_var
...
head o.order_id
	...
	call alterlist(activity_data->list[cnt].orders, ocnt)
	
	activity_data->list[cnt].order_cnt							= ocnt 
	activity_data->list[cnt].orders[ocnt].order_id				= o.order_id
	activity_data->list[cnt].orders[ocnt].order_dt_tm			= o.current_start_dt_tm
	activity_data->list[cnt].orders[ocnt].order_mnemonic		= o.order_mnemonic	
	activity_data->list[cnt].orders[ocnt].order_status			= uar_get_code_display(o.order_status_cd)
	activity_data->list[cnt].orders[ocnt].ordering_physician	= trim(per_oa.name_full_formatted, 3)
...
where
	...
	and ce.task_assay_cd in (dta_ambqual_var, dta_activity_status_adl_var) ;001
...
detail
	...
	activity_data->list[idx].dtas[dcnt].result_val			= replace(replace(ce.result_val, char(13), ""), char(10), "")		
...
;	, patient_id			= activity_data->list[d1.seq].person_id
	...
	, disch_dt_tm			= format(activity_data->list[d1.seq].disch_dt_tm, "mm/dd/yyyy hh:mm;;q")
