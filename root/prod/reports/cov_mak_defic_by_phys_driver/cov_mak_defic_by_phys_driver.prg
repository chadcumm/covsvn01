...
******************************************************************************
	...
	Source file name:	cov_mak_defic_by_phys_ccl.prg
	Object name:		cov_mak_defic_by_phys_ccl
	Request #:			5582 (6358)
 
	Program purpose:	Lists deficiencies assigned to physicians.
...
******************************************************************************
 	...
 	----------	--------------------	--------------------------------------
001	10/31/2019	Todd A. Blanchard		Changed data->defic_qual[*].status to c40.
002	11/01/2019	Todd A. Blanchard		Added discharge date range prompts.
003 05/19/2020	Chad Cummings			Copied driver, removed date range
******************************************************************************/

drop program cov_mak_defic_by_phys_driver go
create program cov_mak_defic_by_phys_driver
...
with OUTDEV, ORGANIZATIONS, PHYSICIANS;, start_datetime, end_datetime
...
from
    ...
    ,prsnl_alias pa
plan d
;003 where data->qual[d.seq].disch_dt_tm between cnvtdatetime($start_datetime) and cnvtdatetime($end_datetime) ;002
join pa
...
;003 and data->qual[d.seq].disch_dt_tm between cnvtdatetime($start_datetime) and cnvtdatetime($end_datetime)) ;002
...
;003 and data->qual[d1.seq].disch_dt_tm between cnvtdatetime($start_datetime) and cnvtdatetime($end_datetime) ;002
...
detail
	...
	data->qual[d1.seq].defic_qual[d2.seq].last_communication_type_cd = o.latest_communication_type_cd
	data->qual[d1.seq].defic_qual[d2.seq].last_communication_type = uar_get_code_display(o.latest_communication_type_cd)
	;call echo(p.name_full_formatted)
with nocounter
 
call echo(build2("Finding powerplan descriptions "))
select into "nl:"
from
	 (dummyt   d1  with seq = value(size(data->qual, 5)))
	,(dummyt   d2  with seq = 1)
...
;003 and data->qual[d1.seq].disch_dt_tm between cnvtdatetime($start_datetime) and cnvtdatetime($end_datetime) ;002
join d2 where data->qual[d1.seq].defic_qual[d2.seq].order_id > 0.0
		and    maxrec(d3, size(data->qual[d1.seq].defic_qual[d2.seq].order_qual, 5))
join d3 where data->qual[d1.seq].defic_qual[d2.seq].order_qual[d3.seq].orders_pathway_catalog_id > 0.0
join pc where pc.pathway_catalog_id = data->qual[d1.seq].defic_qual[d2.seq].order_qual[d3.seq].orders_pathway_catalog_id
detail
	data->qual[d1.seq].defic_qual[d2.seq].pathway_catalog_id = pc.pathway_catalog_id
	data->qual[d1.seq].defic_qual[d2.seq].pathway_description = pc.description
	call echo(pc.description)
with nocounter
...
detail
	...
	t_rec->qual[t_rec->cnt].latest_comm_type		= data->qual[d.seq]->defic_qual[ddefic.seq].last_communication_type
	t_rec->qual[t_rec->cnt].latest_comm_type_cd		= data->qual[d.seq]->defic_qual[ddefic.seq].last_communication_type_cd
	...
	t_rec->qual[t_rec->cnt].powerplan_desc			= data->qual[d.seq]->defic_qual[ddefic.seq].pathway_description
	if (size(data->qual[d.seq]->defic_qual[ddefic.seq].order_qual,5) > 0)
		if (size(data->qual[d.seq]->defic_qual[ddefic.seq].order_qual,5) = 1)
			...
			t_rec->qual[t_rec->cnt].order_notification_id = 99999999999.00
		endif
	endif
with nocounter
/*
