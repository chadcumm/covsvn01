...
******************************************************************************
	...
	Source file name:  	cov_phq_lwbs_form.prg
	Object name:		cov_phq_lwbs_form
	Request#:			2212
...
plan tc where tc.checkin_dt_tm between cnvtdatetime(start_qtr) and cnvtdatetime(end_qtr)
...
Detail
	...
	if(n.nomenclature_id = 280693164.00)
		lwbs->plist[cnt].triage_flag = 1
	else
		lwbs->plist[cnt].triage_flag = 0
	endif
...
prompt
	"Output to File/Printer/MINE" = "MINE"   ;* Enter or select the printer or file name to send this report to.
	, "Select Facility" = 2552503613.00
	, "Select Quarter" = "q1"
	, "Select Year" = 0
 
with OUTDEV, facility, quarter, year_prmt
...
select distinct into "NL:"
  ...
  location	 = uar_get_code_description(e.loc_facility_cd)
, e.person_id
, e.encntr_id
, tc.checkin_dt_tm
, tc.checkout_dt_tm
...
Detail
	...
	lwbs->plist[cnt].facility        = location
	lwbs->plist[cnt].ed_group        = group
	lwbs->plist[cnt].encntrid        = e.encntr_id
	lwbs->plist[cnt].chkout_dispo_cd = e.disch_disposition_cd
	lwbs->plist[cnt].disposition     = dispo_cond
	...
	if (e.disch_disposition_cd = 19945000.00)
		lwbs->plist[cnt].dispo_cd = 1
	else
		lwbs->plist[cnt].dispo_cd = 0
	endif
