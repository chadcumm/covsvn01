...
******************************************************************************
	...
	Source file name:	COV_LAB_CRITICAL_RESULT_NOTE.prg
	Object name:		COV_LAB_CRITICAL_RESULT_NOTE
	Program purpose:	Critical results notification report with 30 minute look ahead
	Executing from:		DA2
 	Special Notes:
...
-----------------------------------------------------------------------------------------------------
10/20/2022  Steve Czubek   CR#10290 new version with 30 minutes look ahead based on cov_phq_critical_result_12hr
 
*******************************************************************************************************/
 
drop program COV_LAB_CRITICAL_RESULT_NOTE:dba go
create program COV_LAB_CRITICAL_RESULT_NOTE:dba
...
Record chart(
	...
	1 plist[*]
		...
		2 result[*]
			...
			3 lab_result_cmnt_dt = dq8
			3 prsnl_notify = vc
			3 perform_dt = vc
			3 lab_comment_to_note = vc
...
Detail
	a->qual[d.seq].labqual[d2.seq].rescomm = replace(trim(lt.long_text),concat(char(13),char(10)),' ')
 	a->qual[d.seq].labqual[d2.seq].rescommdt = rc.comment_dt_tm  ;;;used gets reset below
 	a->qual[d.seq].labqual[d2.seq].rescommdttm = format(rc.comment_dt_tm, "mm/dd/yyyy hh:mm;;q");;; not used
 	a->qual[d.seq].labqual[d2.seq].labtat =    ;;;not used
...
Detail
	a->qual[d.seq].labqual[d2.seq].nurscomm = ce.result_val  ;;; not used
 	a->qual[d.seq].labqual[d2.seq].rescommdt = ce.performed_dt_tm ;;used
 	a->qual[d.seq].labqual[d2.seq].rescommdttm = format(ce.performed_dt_tm, "mm/dd/yyyy hh:mm;;q")  ;; not used
 	a->qual[d.seq].labqual[d2.seq].nursetat =     ;; not used
...
,a->qual[d.seq].labqual[d2.seq].collect_loc
...
Head result_id
	...
	chart->plist[cnt].result[rcnt].accnbr = a->qual[d.seq].labqual[d2.seq].accnbr
	chart->plist[cnt].result[rcnt].order_id = a->qual[d.seq].labqual[d2.seq].orderid
	...
	chart->plist[cnt].result[rcnt].lab_result_cmnt_dt =  a->qual[d.seq].labqual[d2.seq].rescommdt
	                                                                           ;; can come from providernotification or result_comment
...
join ce where ce.encntr_id = chart->plist[d1.seq].encntrid
	and ce.person_id = chart->plist[d1.seq].personid
 	and ce.event_end_dt_tm > cnvtlookbehind("5,MIN", cnvtdatetime(chart->plist[d1.seq].result[d2.seq].lab_result_cmnt_dt))
	and ce.event_end_dt_tm < cnvtlookahead("30,MIN", cnvtdatetime(chart->plist[d1.seq].result[d2.seq].lab_result_cmnt_dt))
...
join cdr
    ...
    and cdr.valid_until_dt_tm > outerjoin(cnvtdatetime(curdate, curtime))
...
Detail
  	case (ce.event_cd)
		...
		of prsnl_notify_var:
            ...
            chart->plist[idx].result[d2.seq].lab_comment_to_note = cnvtstring(datetimediff(ce.event_end_dt_tm,
                                                        cnvtdatetime(chart->plist[d1.seq].result[d2.seq].lab_result_cmnt_dt), 4))
		Of call_reason_var:
...
SELECT INTO $OUTDEV
	...
	, ACCESSION = trim(SUBSTRING(1, 20, CHART->plist[D1.SEQ].result[D2.SEQ].accnbr))
	...
	, CMMNT_TO_NOTIFY_MINS = substring(1, 10, CHART->plist[D1.SEQ].result[D2.SEQ].lab_comment_to_note)
