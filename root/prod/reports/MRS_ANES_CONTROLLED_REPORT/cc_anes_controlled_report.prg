...
******************************************************************************
	...
	Source file name:	 cov_anes_controlled_report.prg
	Object name:		   cov_anes_controlled_report
	Request #:
...
001 	03/01/2019   
002
003		06/25/19	CCUMMIN4				Switched admin_dosage away from SUM()
******************************************************************************/

drop program cov_anes_controlled_report go
create program cov_anes_controlled_report
...
declare admin_dose = f8 with public, noconstant(0.0)
declare admin_amount = f8 with public, noconstant(0.0)
...
admin_dose = admin_dose + smi.admin_dosage
admin_amount = admin_amount + smi.admin_amount
...
if(nValidUOM = 0)
	...
	;temp->rec[rec_counter].ord[ord_cnt].ord_amount = admin_dosage ;003
	
	;003 temp->rec[rec_counter].ord[ord_cnt].ord_amount
	;003 	= concat(trim(cnvtstring(sum(smi.admin_dosage),7,2))," ",trim(uar_get_code_display(sma.dosage_unit_cd),3)," = ",\
	;003	trim(cnvtstring(sum(smi.admin_amount),7,2))," ",trim(uar_get_code_display(sma.amount_unit_cd),3))
	
	temp->rec[rec_counter].ord[ord_cnt].ord_amount
		= concat(trim(cnvtstring(admin_dose,7,2))," ",trim(uar_get_code_display(sma.dosage_unit_cd),3)," = ",\
		trim(cnvtstring(admin_amount,7,2))," ",trim(uar_get_code_display(sma.amount_unit_cd),3))	
	;else
	;	temp->rec[rec_counter].ord[ord_cnt].ord_amount
	;	= concat(trim(cnvtstring(sum(smi.admin_amount)))," ",uar_get_code_display(sma.amount_unit_cd))
 
	;endif
 	call echo(build2("admin_dosage=",admin_dose))
endif
;002 - End
