...
******************************************************************************
	...
	Solution:			Quality
	source file name:	      cov_phq_ekg_orders_all_test.prg
	object name:		cov_phq_ekg_orders_all_test
	Request#:			
	Program purpose:	      EKG Orders(all)
	Executing from:		DA2
 	Special Notes:          Copy of EKG orders report with no exclusion
...
----------	-------------------------------------------------------------------------------------------------------
06/29/22    Geetha     CR#        Initial Release       
09/30/22    Geetha     CR#13717   Add ED ekg order   
-------------------------------------------------------------------------------------------------------------------*/
 
drop program cov_phq_ekg_orders_all_test:dba go
create program cov_phq_ekg_orders_all_test:dba
...
Record pat(
	...
	1 plist[*]
		...
		2 olist[*]
			3 ekg_id = i4
			3 ekg_orderid = f8
			3 ekg_order_location = vc
			3 ekg_orderdt = dq8
			3 ekg_order_mnemonic = vc
			3 ekg_ordering_pr = vc
			3 ekg_order_priority = vc
			3 ekg_indication_reason = vc
			;3 resu_med_intervention = vc
	)
...
join o where o.encntr_id = e.encntr_id
	...
	and o.catalog_cd in(ekg_elect_12_var, ed_ekg_elect_12_var)
	and o.active_ind = 1
...
Head o.order_id	
	...
	pat->plist[cnt].olist[ocnt].ekg_id = ocnt
	pat->plist[cnt].olist[ocnt].ekg_orderid = o.order_id
	pat->plist[cnt].olist[ocnt].ekg_order_mnemonic = o.order_mnemonic
	pat->plist[cnt].olist[ocnt].ekg_orderdt = o.orig_order_dt_tm
Foot e.encntr_id
	pat->plist[cnt].enc_ekg_cnt = ocnt
...
join oa where oa.order_id = pat->plist[d1.seq].olist[d2.seq].ekg_orderid
...
Head oa.order_id
	idx = 0
	icnt = 0
      idx = locateval(icnt ,1 ,pat->rec_cnt ,oa.order_id ,pat->plist[d1.seq].olist[icnt].ekg_orderid)
      ;while(idx > 0)
      	pat->plist[d1.seq].olist[idx].ekg_ordering_pr = pr.name_full_formatted
...
join od where od.order_id = pat->plist[d1.seq].olist[d2.seq].ekg_orderid
...
Head od.order_id
	idx = 0
	icnt = 0
      idx = locateval(icnt ,1 ,pat->rec_cnt ,od.order_id ,pat->plist[d1.seq].olist[icnt].ekg_orderid)
Head od.oe_field_id	      
      	case(od.oe_field_meaning) 
      		of ordr_priority_var:
      			pat->plist[d1.seq].olist[idx].ekg_order_priority = od.oe_field_display_value
      		of ordr_reason_var:	
      			pat->plist[d1.seq].olist[idx].ekg_indication_reason = od.oe_field_display_value
      	endcase		
with nocounter 
...
elh.encntr_id, ord_id = pat->plist[d1.seq].olist[d2.seq].ekg_orderid
...
join elh where elh.encntr_id = pat->plist[d1.seq].encntrid
	and pat->plist[d1.seq].olist[d2.seq].ekg_orderdt != 0
	and (cnvtdatetime(pat->plist[d1.seq].olist[d2.seq].ekg_orderdt) between elh.beg_effective_dt_tm and elh.end_effective_dt_tm)
...
Head ord_id
 	icnt = 0
	idx = 0
	idx = locateval(icnt,1 ,pat->rec_cnt, ord_id, pat->plist[d1.seq].olist[icnt].ekg_orderid)
	if(idx > 0)
		pat->plist[d1.seq].olist[idx].ekg_order_location = order_pat_loc
	endif
...
fin = pat->plist[d1.seq].fin, enc = pat->plist[d1.seq].encntrid, dnr_ord = dnr->list[d3.seq].dnr_orderid
,resu_name = dnr->list[d3.seq].dnr_clin_display
, ekg_dt = format(pat->plist[d1.seq].olist[d2.seq].ekg_orderdt,"mm-dd-yyyy hh:mm;;d") 
, dnr_start_dt = format(dnr->list[d3.seq].dnr_ordered_status_dt,"mm-dd-yyyy hh:mm;;d") 
, dnr_end_dt = format(dnr->list[d3.seq].dnr_discontinue_status_dt,"mm-dd-yyyy hh:mm;;d") 

from	 (dummyt d1  with seq = size(pat->plist, 5))
	,(dummyt d2  with seq = 1)
	,(dummyt d3 with seq = dnr->dnr_rec_cnt)

plan d1 where maxrec(d2, size(pat->plist[d1.seq].olist, 5))

join d2

join d3 where pat->plist[d1.seq].encntrid = dnr->list[d3.seq].dnr_encntrid
	and (format(pat->plist[d1.seq].olist[d2.seq].ekg_orderdt,"mm-dd-yyyy hh:mm;;d") between
		format(dnr->list[d3.seq].dnr_ordered_status_dt,"mm-dd-yyyy hh:mm;;d") 
		and format(dnr->list[d3.seq].dnr_discontinue_status_dt,"mm-dd-yyyy hh:mm;;d") )
	
	OR (format(pat->plist[d1.seq].olist[d2.seq].ekg_orderdt,"mm-dd-yyyy hh:mm;;d") >=
		format(dnr->list[d3.seq].dnr_ordered_status_dt,"mm-dd-yyyy hh:mm;;d") 
		and format(dnr->list[d3.seq].dnr_discontinue_status_dt,"mm-dd-yyyy hh:mm;;d") = ' ' )
...
Head enc
	...
	idx = 0
	icnt = 0
      idx = locateval(icnt ,1 ,pat->rec_cnt ,enc ,pat->plist[d1.seq].encntrid)
Head dnr_ord
	dnr_list = build2(trim(dnr_list),'[' ,trim(resu_name),'- (',dnr_start_dt,' TO ',dnr_end_dt,' ) ]',',')
Foot enc
	pat->plist[d1.seq].resu_med_intervention = replace(trim(dnr_list),",","",2)
with nocounter 
...
;======================================================================================================
 ...
 select into $outdev
	...
	, ekg_order_dt = format(pat->plist[d1.seq].olist[d2.seq].ekg_orderdt, 'mm/dd/yy hh:mm:ss;;q')
	, ekg_order_location = trim(substring(1, 500, pat->plist[d1.seq].olist[d2.seq].ekg_order_location))
	, ekg_order = trim(substring(1, 500, pat->plist[d1.seq].olist[d2.seq].ekg_order_mnemonic))
	, ordering_provider = trim(substring(1, 60, pat->plist[d1.seq].olist[d2.seq].ekg_ordering_pr))
	, ekg_priority = trim(substring(1, 30, pat->plist[d1.seq].olist[d2.seq].ekg_order_priority))
	, ekg_indication_reason = trim(substring(1, 30, pat->plist[d1.seq].olist[d2.seq].ekg_indication_reason))
...
plan d1 where maxrec(d2, size(pat->plist[d1.seq].olist, 5))
	;and trim(substring(1, 60, pat->plist[d1.seq].olist[d2.seq].ekg_ordering_pr))!= 'SYSTEM, SYSTEM*'

join d2

order by facility	, fin, ekg_order_dt
