...
;	---	----------	--------	----------------------------------------------- *
;	001	08/15/2018	AJF			Initial Release                   		 		*
;	002	08/20/2018	AJF			Added personnel position code and Star ID		*
;	003	10/20/2019	TAB			Added Communication Type and PowerPlan Name		*
;********************************************************************************
   ...
   1 qual [* ]
     ...
     2 defic_qual [* ]
       ...
       3 order_qual [* ]
         ...
         4 orders_latest_communication_type = vc ;003
         ...
         4 orders_pathway_description = vc ;003
...
join p
	where p.person_id = cbr.request_prsnl_id
...
from (dummyt d with seq = value(size(data->qual,5))),
     (dummyt ddefic with seq = value(data->max_defic_qual_count)),
     orders o,
     order_notification cbr,
     prsnl p
plan (d where (d.seq > 0)
	and data->qual[d.seq].physician_username = "HIMREFUSAL")
join (ddefic where (ddefic.seq <= size(data->qual[d.seq].defic_qual,5)))
join o
	where o.order_id = data->qual[d.seq]->defic_qual[ddefic.seq].order_id
join cbr
	where cbr.order_id = o.order_id   
join p
	where p.person_id = cbr.from_prsnl_id
order by
	 cbr.order_id
	,cbr.status_change_dt_tm
	
head cbr.order_id
	data->qual[d.seq]->defic_qual[ddefic.seq].reject_reason = cbr.notification_comment
	data->qual[d.seq]->defic_qual[ddefic.seq].reject_prsnl = p.name_full_formatted
with nocounter
*/
...
detail
  ...
  col 0
    ...
    '"',scanned_prsnl,'"',",",
    refuse_prsnl = trim((data->qual[d.seq]->defic_qual[ddefic.seq].reject_prsnl),3)
    '"',refuse_prsnl,'"',",",
    refuse_reason = trim(data->qual[d.seq]->defic_qual[ddefic.seq].reject_reason,3)
    '"',refuse_reason,'"',",",
    comm_type = trim(data->qual[d.seq]->defic_qual[ddefic.seq]->order_qual[1].orders_latest_communication_type,3)
    '"',comm_type,'"',",",
    pp_name = trim(data->qual[d.seq]->defic_qual[ddefic.seq]->order_qual[1].orders_pathway_description,3)
    '"',pp_name,'"' 
 
with nocounter, landscape, format = variable, maxcol = 15000;, maxrow = 1, formfeed = none;, append
  
;call echorecord(data,"ccluserdir:him_def_data.dat")
