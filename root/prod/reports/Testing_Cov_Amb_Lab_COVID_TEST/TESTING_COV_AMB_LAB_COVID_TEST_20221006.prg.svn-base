...
CALL ECHO(CONCAT("FIN Data for the report at ",FORMAT(CNVTDATETIME(CURDATE,CURTIME3),"MM/DD/YYYY hh:mm:ss;;d")))
 
/*Build the FIN data*/
SELECT EA.alias
FROM ENCNTR_ALIAS EA
WHERE (expand(num, 1, ocnt, EA.encntr_id, orderencounter->olist[num].encounter_id)
	AND EA.active_ind = 1
...
head EA.encntr_id
	...
	idx = 0
	next = 0
	idx = locateval(numx, 1, ocnt, EA.encntr_id, orderencounter->olist[numx].encounter_id)
	next = idx+1
 
detail
	if (idx > 0)
		orderencounter->olist[idx].FIN = EA.alias
		while(next <= ocnt	AND orderencounter->olist[next].encounter_id = orderencounter->olist[idx].encounter_id)
			orderencounter->olist[next].FIN = EA.alias
			next = next+1
		endwhile
	endif
foot EA.encntr_id
...
CALL ECHO(CONCAT("CMRN Data for the report at ",FORMAT(CNVTDATETIME(CURDATE,CURTIME3),"MM/DD/YYYY hh:mm:ss;;d")))
 
/*build CMRN data*/
SELECT PA.alias
FROM PERSON_ALIAS PA
WHERE expand(num, 1, ocnt, PA.person_id, orderencounter->olist[num].patient_id)
...
head PA.person_id
	...
	sample = 0
	next = 0
	idx = locateval(numx, 1, ocnt, PA.person_id, orderencounter->olist[numx].patient_id)
	next = idx+1
 
detail
	if (idx > 0)
		orderencounter->olist[idx].cMRN = PA.alias
		while(next <= ocnt	AND orderencounter->olist[next].patient_id = orderencounter->olist[idx].patient_id)
			orderencounter->olist[next].cMRN = PA.alias
			next = next+1
		endwhile
	endif
foot PA.person_id
