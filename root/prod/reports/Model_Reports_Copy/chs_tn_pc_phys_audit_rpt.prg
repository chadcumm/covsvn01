...
if (IsPromptEmpty(5) OR IsPromptAny(5))
    set phy_nme_parser = "1=1"
 	...
 	set phy_nme_parser = GetPromptList (5 ,"pr.person_id" ,LIST_IN )
endif
...
if (IsPromptEmpty(6) OR IsPromptAny(6))
    set mid_lvl_parser = "1=1"
 	...
 	set mid_lvl_parser = GetPromptList (6 ,"pr.person_id" ,LIST_IN )
endif
...
join ce
    ...
    and ce.event_cd in (72_wellchildofficenote_cd, 72_procedurenote_cd,
                        ...
                        72_annualphysicalofficenote_cd, 2563650561.00)
...
detail
    ;if(ce.result_status_cd != 8_inprogress_cd)
        ;if(cep.action_type_cd = 21_verify_cd and cep.action_status_cd = 103_completed_cd)
            phys_data->qual[cnt]->mid_lvl_pvd_name = trim(pr.name_full_formatted,3)
...
/**************************************************************
;Mid Level Provider & Physician name receiving the Note details
**************************************************************/
select into "nl:"
...
order by pr.person_id,e.encntr_id,ce.event_id
...
;EXECUTE chs_tn_pc_phys_audit_rpt:dba "MINE", "10-JAN-2019 19:48:00", "10-JAN-2020 19:48:00", "*", "*",2.00 go