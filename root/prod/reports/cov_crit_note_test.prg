...
******************************************************************************
	...
	Source file name:	      cov_phq_critical_result_12hr.prg
	Object name:		cov_phq_critical_result_2hr
	Request#:			1070
...
drop program cov_crit_note_test:dba go
create program cov_crit_note_test:dba
...
plan d where maxrec(d2,a->qual[d.seq].labcnt)
	and a->qual[d.seq].nu_code != 0.0
	;and operator(a->qual[d.seq].nu_code, opr_nu_var, $nurse_unit)
...
Head result_id
	...
	chart->plist[cnt].result[rcnt].lab_perform_dt = a->qual[d.seq].labqual[d2.seq].verdatetm
	...
	chart->plist[cnt].result[rcnt].lab_result_cmnt_dt =  a->qual[d.seq].labqual[d2.seq].rescommdt
		;trim(substring(1,25, a->qual[d.seq].labqual[d2.seq].rescommdttm))
...
SELECT INTO $OUTDEV
	...
	, patient_location = trim(SUBSTRING(1, 30, CHART->plist[D1.SEQ].pat_loc))
	...
	, LAB_PERFORM_DT = trim(FORMAT(CHART->plist[D1.SEQ].result[D2.SEQ].lab_perform_dt, 'MM/DD/YYYY HH:MM;;D'))
