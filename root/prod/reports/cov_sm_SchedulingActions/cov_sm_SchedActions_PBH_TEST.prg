...
******************************************************************************
	...
	Source file name:	cov_sm_SchedulingActions_PBH.prg
	Object name:		cov_sm_SchedulingActions_PBH
	Request #:			7708, 12349
	...
	Executing from:		CCL
 	...
 	Special Notes:		For behavioral health only.
...
drop program cov_sm_SchedActions_PBH_TEST:DBA go
create program cov_sm_SchedActions_PBH_TEST:DBA
...
select
	if (substring(1, 1, reflect(parameter(parameter2($physician_group), 0))) = "I")
		...
		where
			1 = 1
			and sa.beg_dt_tm > cnvtdatetime("01-MAY-2018 000000")
...
from
	...
	, (inner join SCH_APPT sar on sar.sch_event_id = sa.sch_event_id
		and sar.beg_dt_tm > cnvtdatetime("01-MAY-2018 000000")			
	...
	, (inner join SCH_EVENT_ACTION seact on seact.sch_event_id = sev.sch_event_id
		...
		and seact.resource_cd >= 0.0
		and (
			(operator(seact.sch_action_cd, op_action_var, $action)
				and seact.sch_action_cd in (confirm_var, cancel_var, noshow_var))
 
			or
 
			(operator(seact.sch_reason_cd, op_action_var, $action)
				and seact.sch_reason_cd in (cancel_unable_sched_var))
		)
...
head seact.sch_action_id
	...
	sched_appt->list[cnt].order_phy_group		= trim(org_ps.org_name, 3) ;001
