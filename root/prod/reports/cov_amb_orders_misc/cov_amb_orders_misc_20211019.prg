...
----    ----------- ----------------------- ---------------------------------------
001     10/08/2021  Dawn Greer, DBA         Original Release - CR 11378
002     10/19/2021  Dawn Greer, DBA         CR 11444 - Exclude Discontinued and Canceled
003     10/20/2021  Dawn Greer, DBA         CR 11444 - Error in report.  Fixed field name
************************************************************************************/
...
SELECT DISTINCT
  ...
  ,Originating_encntr_id = ord.originating_encntr_id
  ,Order_Status = UAR_GET_CODE_DISPLAY(ord.order_status_cd)
  ,Lab_Test_Code = TRIM(od_code.oe_field_display_value,3)
  ,Lab_Test_Desc = TRIM(od_desc.oe_field_display_value,3)
...
,(LEFT JOIN PERSON_ALIAS cmrn ON (pat.person_id = cmrn.person_id	;003
 	AND cmrn.active_ind = 1
 	AND cmrn.end_effective_dt_tm > CNVTDATETIME(CURDATE,CURTIME3)
 	AND cmrn.person_alias_type_cd = 2 /*CMRN*/
	))
,(LEFT JOIN ORDER_DETAIL od_code ON (ord.order_id = od_code.order_id
	AND od_code.oe_field_id IN (3555136775.00 /*LabCorp Test Code*/, 274145197.00 /*Laboratory Test CODE*/)
	))
,(LEFT JOIN ORDER_DETAIL od_desc ON (ord.order_id = od_desc.order_id
	AND od_desc.oe_field_id =  3555177137.00 /*LabCorp Test Name*/
	))
,(LEFT JOIN CLINICAL_EVENT ce ON (ord.encntr_id = ce.encntr_id
	AND ord.person_id = ce.person_id
	AND (ord.order_id = ce.order_id
		OR TRIM(od_code.oe_field_display_value,3) = TRIM(SUBSTRING(16,6,ce.reference_nbr),3))
	AND ce.view_level = 1
	AND ce.valid_until_dt_tm >= CNVTDATETIME(CURDATE,CURTIME3)
	AND ce.contributor_system_cd IN (2560913087.00 /*LabCorp AMB*/, 2561182275.00 /*LabCorp Unmatched*/,
		2560912703 /*QUEST_AMB*/,2561182359 /*QUEST_AMB_UNMATCH*/, 2804250365 /*LABDAQ*/, 2553019355 /*LABDAQC*/,
		2553019331 /*LABDAQS*/,2559046255 /*FCOR*/)
	))
...
AND ord.order_status_cd NOT IN (2545 /*Discontinued*/,2542 /*Canceled*/)	;002
...
DETAIL
	...
	ENDIF
 	...
 	a->qual[cnt].person_id = person_id
	a->qual[cnt].patient = patient
	a->qual[cnt].dob = FORMAT(dob, "MM/DD/YYYY")
	a->qual[cnt].cmrn = cmrn
	a->qual[cnt].fin = fin
	a->qual[cnt].order_id = order_Id,
	a->qual[cnt].encntr_id = encntr_id
	a->qual[cnt].originating_encntr_id = originating_encntr_id
	a->qual[cnt].order_status = order_status
	a->qual[cnt].Lab_Test_Code = Lab_Test_Code
	a->qual[cnt].Lab_Test_Desc = Lab_Test_Desc
...
IF(a->rec_cnt >= 0)
	...
	SELECT INTO $outdev
		...
		Lab_Test_Code = a->qual[d.seq].Lab_Test_Code,
		Lab_Test_Desc = a->qual[d.seq].Lab_Test_Desc,
