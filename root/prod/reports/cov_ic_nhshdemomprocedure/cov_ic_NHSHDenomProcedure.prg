 ...
 *****************************************************************************
    ...
    Source file name:  cov_ic_NHSHDenomProcedure.prg
    Object name:       cov_ic_NHSHDenomProcedure
 	CR #:			   517
...
drop program cov_ic_NHSNDenomProcedure:dba go
create program cov_ic_NHSNDenomProcedure:dba
...
;set START_DATE = CNVTDATETIME("01-JAN-2019 00:00:00")  ;$STARTDATE_PMPT
;set END_DATE   = CNVTDATETIME("01-JUL-2019 00:00:00")  ;$ENDDATE_PMPT
...
;	CALL ECHO(build('EVENTID :',cb.event_id))
;	call echo(build('blobsize :', blobsize))
...
;call echo(build('casenbrpos = ',casenbrpos))
 	if (casenbrpos > 0)
 		endcasepos = findstring("/",blobres,casenbrpos)
 
 		if (endcasepos  > 0)
 			casenbr = substring(casenbrpos + 13,(endcasepos-1) - (casenbrpos + 13),blobres)
;call echo(build('casenbr = ',casenbr))
 		endif
 	endif
;call echo(blobres)
;call echo(build('blobres len:', textlen(blobres)))
...
;call echo(build('asablob = ',asablob))
	blobres = fillstring(40000,' ')
    blobin = fillstring(40000,' ')
	blobsize = 0
	blobout = fillstring(40000,' ')
...
;		and pd.periop_doc_id = cnvtreal(substring(1,8,ce.reference_nbr))
		...
		and pd.doc_type_cd in (
			CLMC_DOC_TYPE_VAR, CMC_DOC_TYPE_VAR, FLMC_DOC_TYPE_VAR, FSR_DOC_TYPE_VAR, LCMC_DOC_TYPE_VAR,
			MHAS_DOC_TYPE_VAR, MHHS_DOC_TYPE_VAR, MMC_DOC_TYPE_VAR, PWMC_DOC_TYPE_VAR, RMC_DOC_TYPE_VAR
			)
