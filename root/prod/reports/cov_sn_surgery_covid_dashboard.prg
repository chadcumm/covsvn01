...
drop program cov_sn_surgery_covid_dashboard:dba go
create program cov_sn_surgery_covid_dashboard:dba
...
Head l.location_cd
	...
	dash->fac_cnt = cnt
	dash->fac[cnt].facility = facility_name
	dash->fac[cnt].facility_cd = l.location_cd
	dash->fac[cnt].appt_bookid = evaluate(l.location_cd,
...
plan o where o.orig_order_dt_tm between cnvtdatetime(start_dt_tm) and cnvtdatetime(end_dt_tm)
	and o.catalog_cd in(covid_ref_lab_var, covid_mpl_ref_lab_var, covid_in_house_var, covid_respiratory_var)
...
For(fcnt = 1 to dash->fac_cnt)
	execute cov_sn_sched_dept_count 'NL:', value(dash->fac[fcnt].appt_bookid), $department, $start_datetime, $end_datetime
...
select into $outdev
	...
	, Department_scheduled_count = dash->fac[d1.seq].list[d2.seq].department_cnt
