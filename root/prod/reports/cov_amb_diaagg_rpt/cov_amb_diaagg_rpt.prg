...
05/24/22	David Baumgardner		CR12586 Update the communication type to pull the original communication type
										and add the entered by field.
07/26/22	David Baumgardner		Updating the prompts to set the datetime to start at the beginning of the day and end of the
									day.  Stemmed from IN1478711.
11/08/22  Steve Czubek        CR 10730 correct provider and facility sort.  Add audit/detail prompt.
															Default provider and status prompts to Any.
******************************************************************************/
 
DROP PROGRAM cov_amb_diaagg_rpt GO
CREATE PROGRAM cov_amb_diaagg_rpt
...
declare new_order_var = f8 with constant(uar_get_code_by("DISPLAYKEY",6003, "ORDER"))
declare ACTIVATE = f8 with constant(uar_get_code_by("DISPLAYKEY",6003, "ACTIVATE"))
declare CANCEL = f8 with constant(uar_get_code_by("DISPLAYKEY",6003, "CANCEL"))
declare CANCELDISCONTINUE = f8 with constant(uar_get_code_by("DISPLAYKEY",6003, "CANCELDISCONTINUE"))
declare CANCELANDREORDER = f8 with constant(uar_get_code_by("DISPLAYKEY",6003, "CANCELANDREORDER"))
declare COMPLETE = f8 with constant(uar_get_code_by("DISPLAYKEY",6003, "COMPLETE"))
declare VOID = f8 with constant(uar_get_code_by("DISPLAYKEY",6003, "VOID"))
declare DISCONTINUE = f8 with constant(uar_get_code_by("DISPLAYKEY",6003, "DISCONTINUE"))
...
record orderencounter (
  1 olist[*]
    ...
    2 status_date = dq8
    2 order_status = c15
    2 order_status_audit_list[*]
			3  audit_order_status = vc
			3 audit_status_by = vc
			3 audit_date = vc
    2 order_desc = c40
    2 future_loc = c40
    2 sched_loc = c40
    ...
    2 sch_tat_days = i4
    2 auth_tat_days = i4
    ;05/24/22 add in the entered by field to output with the report.
    2 entered_by = c40
)
...
SELECT DISTINCT INTO $OUTDEV
	...
	, ENTERED_BY = EP.name_full_formatted
FROM
	ENCOUNTER   E
	, ORDERS   O
	, ORDER_DETAIL   OD
...
JOIN OD WHERE OUTERJOIN(O.ORDER_ID) = OD.order_ID
	AND OD.oe_field_MEANING = outerjoin ('REQSTARTDTTM')
...
JOIN OA WHERE OA.order_id = OUTERJOIN(O.ORDER_ID)
	and OA.action_type_cd = new_order_var
JOIN EP WHERE EP.person_id = OA.action_personnel_id
...
head o.order_id
	...
	orderencounter->olist[ocnt].entered_by = ENTERED_BY
...
select into "nl:"
from
	(dummyt d with seq = size(orderencounter->olist, 5))
	,order_action oa
	,prsnl ps
plan d
join oa
	where oa.order_id = orderencounter->olist[d.seq].order_id
join ps
...
head d.seq
	a_cnt = 0
detail
	if (oa.action_type_cd in  (new_order_var,
														ACTIVATE,
														CANCEL,
														CANCELDISCONTINUE,
														CANCELANDREORDER,
														...
														DISCONTINUE))
		a_cnt = a_cnt + 1
		stat = alterlist(orderencounter->olist[d.seq]->order_status_audit_list, a_cnt)
		orderencounter->olist[d.seq]->order_status_audit_list[a_cnt].audit_date = format(oa.action_dt_tm, "MM-DD-YYYY HH:MM")
		orderencounter->olist[d.seq]->order_status_audit_list[a_cnt].audit_status_by = ps.name_full_formatted
		case (oa.action_type_cd)
		of new_order_var:
			orderencounter->olist[d.seq]->order_status_audit_list[a_cnt].audit_order_status = "Future"
		of ACTIVATE:
			orderencounter->olist[d.seq]->order_status_audit_list[a_cnt].audit_order_status = "Ordered"
		of CANCEL:
			orderencounter->olist[d.seq]->order_status_audit_list[a_cnt].audit_order_status = "Cancel"
		of CANCELDISCONTINUE:
			orderencounter->olist[d.seq]->order_status_audit_list[a_cnt].audit_order_status = "Canceled D/C"
		of CANCELANDREORDER:
			orderencounter->olist[d.seq]->order_status_audit_list[a_cnt].audit_order_status = "Canceled Reordered"
		of COMPLETE:
			orderencounter->olist[d.seq]->order_status_audit_list[a_cnt].audit_order_status = "Completed"
		of VOID:
			orderencounter->olist[d.seq]->order_status_audit_list[a_cnt].audit_order_status = "Voided"
		of DISCONTINUE:
			orderencounter->olist[d.seq]->order_status_audit_list[a_cnt].audit_order_status = "Discontinued"
		endcase
	endif
...
if ($detail_audit != "Audit")
	...
	,ENTERED_BY = orderencounter->olist[D1.SEQ].entered_by
...
WITH NOCOUNTER, SEPARATOR=" ", FORMAT
	...
	,ORDER_DATE = FORMAT(orderencounter->olist[D1.SEQ].order_date,"mm/dd/yyyy;;d")
	,ORDER_DESC = orderencounter->olist[D1.SEQ].order_desc
	,ORDER_STATUS = substring(1, 20, orderencounter->olist[D1.SEQ]->order_status_audit_list[d2.seq].audit_order_status)
	,STATUS_DATE = substring(1, 20, orderencounter->olist[D1.SEQ]->order_status_audit_list[d2.seq].audit_date)
	,STATUS_CHANGED_BY = substring(1, 30, orderencounter->olist[D1.SEQ]->order_status_audit_list[d2.seq].audit_status_by)
	from
  	(dummyt d1 with seq = size(orderencounter->olist, 5))
    ,(dummyt   d2  with seq = 1)
plan d1
	where maxrec(d2, size(orderencounter->olist[d1.seq].order_status_audit_list, 5))
join d2
