drop program cov_Newborn_FollowUp:DBA go
create program cov_Newborn_FollowUp:DBA
...
declare coombs_var           = f8 with constant(uar_get_code_by("DISPLAY", 72, "DAT Poly")),protect
...
FROM
   ...
   ,(
		( select distinct ce.encntr_id, ce.event_cd, ce.result_val, ce.updt_dt_tm, ce.result_status_cd
			...
			and ce.event_cd in(
			  delivery_dt_var, delivery_phys_var, other_provider_var, delivery_method_var, attend_phys_var, Resuscitation_var, infant_gender_var,
...
SELECT INTO "NL:"
	...
	ea.alias
 	...
 	, mom_dob             = max(pe1.birth_dt_tm)
 	, mom_regdt           = max(e.reg_dt_tm)
	, mom_blood_type      = max(evaluate(i2.event_cd, mom_blood_type_var  ,      ce.result_val, 0, ""))
	, hbsag               = max(evaluate(i2.event_cd, hbsag_var           ,      ce.result_val, 0, ""))
	, gbs                 = max(evaluate(i2.event_cd, gbs_var             ,      ce.result_val, 0, ""))
	, gbs_abx             = max(evaluate(i2.event_cd, gbs_abx_var         ,      ce.result_val, 0, ""))
	, doses_abx           = max(evaluate(i2.event_cd, doses_abx_var       ,      ce.result_val, 0, ""))
	, intra_abx           = max(evaluate(i2.event_cd, intra_abx_var       ,      ce.result_val, 0, ""))
	, toxi_scrn           = max(evaluate(i2.event_cd, toxi_scrn_var       ,      ce.result_val, 0, ""))
	, alcohol_use         = max(evaluate(i2.event_cd, alcohol_use_var     ,      ce.result_val, 0, ""))
	, cocaine_use         = max(evaluate(i2.event_cd, cocaine_use_var     ,      ce.result_val, 0, ""))
	, mom_drug_scrn       = max(evaluate(i2.event_cd, drug_scrn_var       ,      ce.result_val, 0, ""))
	, preg_risk           = max(evaluate(i2.event_cd, preg_risk_var       ,      ce.result_val, 0, ""))
	, med_preg            = max(evaluate(i2.event_cd, med_preg_var        ,      ce.result_val, 0, ""))
	, preg_risk_other     = max(evaluate(i2.event_cd, preg_risk_other_var ,      ce.result_val, 0, ""))
	, preg_risk_dtl       = max(evaluate(i2.event_cd, preg_risk_dtl_var   ,      ce.result_val, 0, ""))
	, ega                 = max(evaluate(i2.event_cd, ega_var             ,      ce.result_val, 0, ""))
	, rubella             = max(evaluate(i2.event_cd, rubella_var         ,      ce.result_val, 0, ""))
	, rpr                 = max(evaluate(i2.event_cd, rpr_var             ,      ce.result_val, 0, ""))
	, hepatitis_c         = max(evaluate(i2.event_cd, hepatitis_c_var     ,      ce.result_val, 0, ""))
	, hsv                 = max(evaluate(i2.event_cd, hsv_var             ,      ce.result_val, 0, ""))
	, hiv                 = max(evaluate(i2.event_cd, hiv_var             ,      ce.result_val, 0, ""))
	, gonorhea            = max(evaluate(i2.event_cd, gonorhea_var        ,      ce.result_val, 0, ""))
	, chlamydia           = max(evaluate(i2.event_cd, chlamydia_var       ,      ce.result_val, 0, ""))
	, tobacco             = max(evaluate(i2.event_cd, tobacco_var         ,      ce.result_val, 0, ""))
	, drug                = max(evaluate(i2.event_cd, drug_var            ,      ce.result_val, 0, ""))
	, marijuana           = max(evaluate(i2.event_cd, marijuana_var       ,      ce.result_val, 0, ""))
	, dcs_nitified        = max(evaluate(i2.event_cd, dcs_nitified_var    ,      ce.result_val, 0, ""))
	, rom_dt              = max(evaluate(i2.event_cd, rom_dt_var          ,      ce.result_val, 0, ""))
	, rom_deli_tot        = max(evaluate(i2.event_cd, rom_deli_tot_var    ,      ce.result_val, 0, ""))
	, lngh_rom            = max(evaluate(i2.event_cd, lngh_rom_var        ,      ce.result_val, 0, ""))
	, deli_anes           = max(evaluate(i2.event_cd, deli_anes_var       ,      ce.result_val, 0, ""))
	, med_labor           = max(evaluate(i2.event_cd, med_labor_var       ,      ce.result_val, 0, ""))
	, dt_norcotic         = max(evaluate(i2.event_cd, dt_norcotic_var     ,      ce.result_val, 0, ""))
	, name_abx            = max(evaluate(i2.event_cd, name_abx_var        ,      ce.result_val, 0, ""))
	, ld_complica         = max(evaluate(i2.event_cd, ld_complica_var     ,      ce.result_val, 0, ""))
