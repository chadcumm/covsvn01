...
declare 333_consultdoc_cd = f8 with constant(uar_get_code_by("MEANING",333,"CONSULTDOC"))
declare reportText = vc with noconstant(" ")

call log_message("pfmt_cov_notify_eso validating 101305 request...", log_level_debug)
if(validate(requestin->request->encntr_prsnl_reltn_qual))
	call log_message("pfmt_cov_notify_eso inside 101305 validate(requestin->request->encntr_prsnl_reltn_qual)", log_level_debug)
  	if(size(requestin->request->encntr_prsnl_reltn,5) > 0)
    	call log_message("pfmt_cov_notify_eso inside request->encntr_prsnl_reltn,5) > 0", log_level_debug)
    	...
    	for(count = 1 to size(requestin->request->encntr_prsnl_reltn,5))
      		...
      		if (requestin->request->encntr_prsnl_reltn[count].encntr_prsnl_r_cd = 333_consultdoc_cd)
      			...
      			call log_message("pfmt_cov_notify_eso INCLUDING encounter", log_level_debug)
      		endif
    	endfor
  	endif
endif
...
if(size(encounters->list,5) = 0)
	call log_message("pfmt_cov_notify_eso validating 600312 request...", log_level_debug)
	if(validate(requestin->request->encntr_id))
		call log_message("pfmt_cov_notify_eso 600312 inside validate(requestin->request->encntr_id)", log_level_debug)
	    ...
	    set reportText = concat("EncounterID: ", cnvtstring(requestin->request->encntr_id))
      	...
      	call log_message(reportText, log_level_debug)
      	if (requestin->request->encntr_prsnl_reltn_cd = 333_consultdoc_cd)
...
if(size(encounters->list,5) = 0)
	call log_message("pfmt_cov_notify_eso validating 600319 request...", log_level_debug)
	if(validate(requestin->request->elist[1].encntr_id))
		call log_message("pfmt_cov_notify_eso inside 600319 validate(requestin->request->elist[1].encntr_id)", log_level_debug)
  		if(size(requestin->request->elist,5) > 0)
    		call log_message("pfmt_cov_notify_eso inside requestin->request->elist,5) > 0", log_level_debug)
    		...
    		for(count = 1 to size(requestin->request->elist,5))
      			...
      			call log_message(reportText, log_level_debug)
      			if (requestin->request->elist[count].encntr_prsnl_reltn_cd = 333_consultdoc_cd)
...
call log_message(build2("pfmt_cov_notify_eso calling ",cnvtstring(count)," of ",
    ...
    call log_message(build2(
    							^send_outbound(^,
    							cnvtstring(encounters->list[count].patient_id),^,^,
    							cnvtstring(encounters->list[count].encounter_id),^,^,
    							cnvtstring(encounters->list[count].patient_id),^"A08")^)
    							,log_level_debug)	
