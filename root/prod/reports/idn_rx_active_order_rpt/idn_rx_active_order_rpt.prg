DROP PROGRAM idn_rx_active_order_rpt :dba GO
CREATE PROGRAM idn_rx_active_order_rpt :dba
 PROMPT
  "Output to File/Printer/MINE" = "MINE" ,
  "Select facility" = value ("*                                       " ) ,
  "Select unit" = value ("*                                       " ) ,
  "Medication search (primary/generic)" = ""
  WITH outdev ,facility ,unit ,med_str
 DECLARE fac_cnt = i4
 ...
 SELECT INTO "nl:"
  ...
  PLAN (o
   ...
   AND (o.order_status_cd IN (cordered ,
   cpending ) )
  ...
  HEAD REPORT
   cnt = 0
  HEAD o.order_id
   cnt = (cnt + 1 ) ,stat = alterlist (ord->ord ,cnt ) ,ord->ord[cnt ].order_id = o.order_id
  WITH nullreport
 ;end select
 FREE RECORD fin
 RECORD fin (
 ...
 SELECT INTO "nl:"
  ...
  HEAD REPORT
   cnt = 0
  HEAD o.encntr_id
   cnt = (cnt + 1 ) ,stat = alterlist (fin->fin ,cnt ) ,fin->fin[cnt ].encntr_id = o.encntr_id ,fin->
   fin[cnt ].fin_nbr = trim (ea.alias )
  WITH outerjoin = d2
 ;end select
 SELECT INTO  $OUTDEV
