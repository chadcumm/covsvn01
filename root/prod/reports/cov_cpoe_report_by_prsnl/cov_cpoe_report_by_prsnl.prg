...
----------	--------------------	------------------------------------------
001			Chad Cummings			Udpated provider parser to use variable (https://wiki.cerner.com/x/DhbPhw)
002         Chad Cummings			Added action user and position to output
******************************************************************************/
 
drop program cov_cpoe_report_by_prsnl:dba go
create program cov_cpoe_report_by_prsnl:dba
...
with OUTDEV, START_DATETIME, END_DATETIME, REPORT_TYPE, CHILD_ORDERS, NEW_PROVIDER
...
declare template_flag_var 		= vc with noconstant(" ")
declare template_flag_oa_var 	= vc with noconstant(" ")
declare new_provider_var	 	= vc with noconstant(" ")
declare new_provider_id			= f8 with noconstant(0.0) ;001

set new_provider_id = $NEW_PROVIDER ;001

if ($CHILD_ORDERS = 1)
	set template_flag_var 		= "o.template_order_flag in(0,1,5)"
	set template_flag_oa_var 	= "oa.template_order_flag in(0,1,5)"
else
	set template_flag_var 		= "1=1"
	set template_flag_oa_var 	= "1=1"
endif
 
if ($NEW_PROVIDER = 0)
	set new_provider_var = "p2.person_id > 0.0"
else
	;001 set new_provider_var = "p2.person_id = $NEW_PROVIDER"
	set new_provider_var = "p2.person_id =new_provider_id" ;001
endif 
...
,action_prsnl = substring(1,100,i.action_name) ;002
,action_position = uar_get_code_display(i.action_position) ;002
,communication_type = trim(uar_get_code_display(i.communication_type_cd))
,i.denominator
,i.numerator
,order_dt = format(i.orig_order_dt_tm, 'mm/dd/yyyy hh:mm;;q')
,order_mmyyyy = format(i.orig_order_dt_tm, 'YYYYMM;;q')
;,order_year = format(i.orig_order_dt_tm, 'YYYY;;q')
...
,child_order = if (i.template_order_flag in(0,1,5)) "N" else "Y" endif
,template_flag = i.template_order_flag
from(
(select distinct
	...
	, numerator = evaluate2(
		...
		elseif(oa.communication_type_cd = 20094437) 1 	;Initiate Planned Orders No Cosign
		elseif(oa.communication_type_cd = 2553560089) 1 ;Standing Order Cosign
		else 0 endif)
	,e.loc_facility_cd
	,e.loc_nurse_unit_cd
	,action_name = p1.name_full_formatted ;002
	,action_position = p1.position_cd ;002
...
;   ,19468404.00	;Cosign Required
 ...
 ,2553560097.00	;Per Protocol No Cosign
  ,2576706321.00	;Per Nutrition Policy No Cosign
	...
	and parser(template_flag_oa_var)
	...
	and parser(new_provider_var)	;p2.person_id =$NEW_PROVIDER; specific Dr.
	with sqltype('vc','vc','vc','dq8','dq8','f8','f8','f8','f8','f8','f8','i2'
	,'f8','dq8','f8','dq8','vc','i2','i2','f8','f8','vc','f8','i2')
	)i
)
