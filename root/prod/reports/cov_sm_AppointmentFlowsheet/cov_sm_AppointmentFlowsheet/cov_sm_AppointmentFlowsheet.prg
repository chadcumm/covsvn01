...
******************************************************************************
	...
	Source file name:	cov_sm_AppointmentFlowsheet.prg
	Object name:		cov_sm_AppointmentFlowsheet
	Request #:			14, 6087, 11683
	...
	Executing from:		CCL
 	...
 	Special Notes:		Includes future appointments.
...
001	05/22/2018	Todd A. Blanchard		Revised query for bookshelf resources.
 										Revised logic for empty procedures list.
...
006	08/07/2018	Todd A. Blanchard		Revised logic to be patient-centric.
 										Revised query for department selection.
 										Revised prompt for departments.
007	08/09/2018	Todd A. Blanchard		Revised prompt for departments.
008	08/22/2018	Todd A. Blanchard		Revised criteria for future appointments.
009	08/27/2018	Todd A. Blanchard		Revised CCL for performance.
010	08/28/2018	Todd A. Blanchard		Revised queries for primary surgeon.
011	09/18/2018	Todd A. Blanchard		Revised patient dob for time zone.
012	12/13/2018	Todd A. Blanchard		Added perioperative departments to prompts.
 										Revised logic for department prompt.
013	12/18/2018	Todd A. Blanchard		Revised queries for FINs.
014	12/19/2018	Todd A. Blanchard		Revised queries for Comments.
015	01/11/2019	Todd A. Blanchard		Added ssn alias_pool_cd to record structure
 										to support masking in layout builder.
016	10/25/2019	Todd A. Blanchard		Revised CCL for cpt codes and comments.
017	02/27/2020	Todd A. Blanchard		Revised logic for phone numbers due to 
										2018.01 upgrade.
018	04/28/2021	Todd A. Blanchard		Added checks for version_dt_tm.
019	07/19/2021	Todd A. Blanchard		Revised query for insurance data. 
020	12/02/2021	Todd A. Blanchard		Added Covenant Health Diagnostics West to prompt. 
 
******************************************************************************/
 
drop program cov_sm_AppointmentFlowsheet:DBA go
create program cov_sm_AppointmentFlowsheet:DBA
...
declare comment1_var				= f8 with constant(75299257.00) ;019
declare comment2_var				= f8 with constant(663791.00) ;019
...
from
	...
	, (left join SCH_EVENT_DETAIL sed1 on sed1.sch_event_id = sev.sch_event_id
		...
		and sed1.version_dt_tm > sysdate ;018
		and sed1.active_ind = 1)
 
	, (left join SCH_EVENT_DETAIL sed2 on sed2.sch_event_id = sev.sch_event_id
		and sed2.oe_field_id in (comment1_var, comment2_var) ;019
		and sed2.version_dt_tm > sysdate ;018
	...
	, (left join SCH_EVENT_DETAIL sed3 on sed3.sch_event_id = sev.sch_event_id
		...
		and sed3.version_dt_tm > sysdate ;018
	...
	, (left join SCH_EVENT_DETAIL sed4 on sed4.sch_event_id = sev.sch_event_id
		...
		and sed4.version_dt_tm > sysdate ;018
	...
	, (left join SCH_EVENT_DETAIL sed5 on sed5.sch_event_id = sev.sch_event_id
		...
		and sed5.version_dt_tm > sysdate ;018
