...
******************************************************************************
	...
	Source file name:  	cov_phq_lwbs_metrics_detailed.prg
	Object name:		cov_phq_lwbs_metrics_detailed
	Request#:			2212 - detailed for validation
...
drop program cov_phq_lwbs_metrics_detailed go
create program cov_phq_lwbs_metrics_detailed
...
Detail
	...
	lwbs->plist[cnt].fin             = ea.alias
	lwbs->plist[cnt].pat_name        = p.name_full_formatted
...
SELECT DISTINCT INTO value($outdev)
	...
	, FIN = SUBSTRING(1, 30, LWBS->plist[D1.SEQ].fin)
	, PATIENT_NAME = SUBSTRING(1, 100, LWBS->plist[D1.SEQ].pat_name)
	...
	;, DISPOSITION = SUBSTRING(1, 100, LWBS->plist[D1.SEQ].disposition)
	, DISCHARGE_TO = SUBSTRING(1, 100, LWBS->plist[D1.SEQ].discharge_to)
...
ORDER BY facility, ed_group, fin, adult_flag, pedi_flag, lwbs_flag, Triage_flag
