...
drop program cov_him_FaceToFace_Note:dba go
create program cov_him_FaceToFace_Note:dba
...
/**************************************************************/ 
; select consult and progress note data
select into "nl:"
from 
	ENCOUNTER e
	...
	, (left join CLINICAL_EVENT ce on ce.encntr_id = e.encntr_id
		...
		and ce.result_status_cd not in (inerror_var, inprogress_var)
	...
	, (left join CLINICAL_EVENT ce2 on ce2.encntr_id = e.encntr_id
		and ce2.event_class_cd in (mdoc_var)
		and ce2.event_cd in (pmr_progress_note_var)
		and ce2.result_status_cd not in (inerror_var, inprogress_var)
		and ce2.valid_until_dt_tm > cnvtdatetime(curdate, curtime3)
		)
		
	, (left join PRSNL per2 on per2.person_id = ce2.performed_prsnl_id
...
;	if (ce.event_id > 0.0)
		...
		; indicators
		if (data->qual[cnt].consult[ccnt].result_status_cd = anticipated_var)
			data->qual[cnt].consult[ccnt].is_anticipated = 1
			data->qual[cnt].is_anticipated = 1
		endif
;	endif
...
;	if (ce2.event_id > 0.0)
		...
		call alterlist(data->qual[cnt].progress, pcnt)
		
		data->qual[cnt].progress_cnt							= pcnt
		data->qual[cnt].progress[pcnt].event_id					= ce2.event_id
		data->qual[cnt].progress[pcnt].event_cd					= ce2.event_cd
		data->qual[cnt].progress[pcnt].event_class_cd			= ce2.event_class_cd
		data->qual[cnt].progress[pcnt].event_title_text			= ce2.event_title_text
		data->qual[cnt].progress[pcnt].performed_prsnl_id		= ce2.performed_prsnl_id
		data->qual[cnt].progress[pcnt].performed_by				= per2.name_full_formatted
		data->qual[cnt].progress[pcnt].result_status_cd			= ce2.result_status_cd
		data->qual[cnt].progress[pcnt].result_dt_tm				= ce2.event_end_dt_tm
		data->qual[cnt].progress[pcnt].result_tz				= ce2.event_end_tz
		
		; indicators
		if (data->qual[cnt].progress[pcnt].result_status_cd = anticipated_var)
			data->qual[cnt].progress[pcnt].is_anticipated = 1
			data->qual[cnt].is_anticipated = 1
		endif
;	endif
	...
	for (i = 1 to total_days)
		...
		if (data->qual[d1.seq].consult_cnt > 0)
			for (j = 1 to data->qual[d1.seq].consult_cnt)		
				if (not data->qual[d1.seq].consult[j].is_anticipated)
		...
		if (data->qual[d1.seq].progress_cnt > 0)
			for (k = 1 to data->qual[d1.seq].progress_cnt)		
				if (not data->qual[d1.seq].progress[k].is_anticipated)
...
with nocounter, outerjoin = d2, outerjoin = d3, time = 180
...
;	, encntr_id				= data->qual[d1.seq].encntr_id
	...
	; consult note
	, consult_event				= if (not data->qual[d1.seq].consult[d2.seq].is_anticipated)
									trim(uar_get_code_display(data->qual[d1.seq].consult[d2.seq].event_cd), 3)
								  endif	
								  
	, consult_event_title		= if (not data->qual[d1.seq].consult[d2.seq].is_anticipated)
									trim(data->qual[d1.seq].consult[d2.seq].event_title_text, 3)
								  endif	
	
	, consult_result_status		= if (not data->qual[d1.seq].consult[d2.seq].is_anticipated)
									trim(uar_get_code_display(data->qual[d1.seq].consult[d2.seq].result_status_cd), 3)
								  endif	
	
	, consult_result_dt_tm		= if (not data->qual[d1.seq].consult[d2.seq].is_anticipated)
									datetimezoneformat(data->qual[d1.seq].consult[d2.seq].result_dt_tm, 
													   data->qual[d1.seq].consult[d2.seq].result_tz, "mm/dd/yyyy hh:mm zzz;;q", 0)
								  endif	
													 
	, consult_performed_by		= if (not data->qual[d1.seq].consult[d2.seq].is_anticipated)
;									trim(per.name_full_formatted, 3)
										trim(data->qual[d1.seq].consult[d2.seq].performed_by, 3)
								  endif	
	
	; progress note
	, progress_event				= if (not data->qual[d1.seq].progress[d3.seq].is_anticipated)
										trim(uar_get_code_display(data->qual[d1.seq].progress[d3.seq].event_cd), 3)
									  endif
									  
	, progress_event_title			= if (not data->qual[d1.seq].progress[d3.seq].is_anticipated)
										trim(data->qual[d1.seq].progress[d3.seq].event_title_text, 3)
									  endif
									  
	, progress_result_status		= if (not data->qual[d1.seq].progress[d3.seq].is_anticipated)
										trim(uar_get_code_display(data->qual[d1.seq].progress[d3.seq].result_status_cd), 3)
									  endif
									  
	, progress_result_dt_tm			= if (not data->qual[d1.seq].progress[d3.seq].is_anticipated)
										datetimezoneformat(data->qual[d1.seq].progress[d3.seq].result_dt_tm, 
														   data->qual[d1.seq].progress[d3.seq].result_tz, "mm/dd/yyyy hh:mm zzz;;q", 0)
									  endif
	
	, progress_performed_by			= if (not data->qual[d1.seq].progress[d3.seq].is_anticipated)
;										trim(per2.name_full_formatted, 3)
										trim(data->qual[d1.seq].progress[d3.seq].performed_by, 3)
									  endif
...
;join per
;where
;	per.person_id = data->qual[d1.seq].consult[d2.seq].performed_prsnl_id
;	and per.active_ind = 1
...
;join per2
;where
;	per2.person_id = data->qual[d1.seq].progress[d3.seq].performed_prsnl_id
;	and per2.active_ind = 1
...
order by
	...
	, if (not data->qual[d1.seq].consult[d2.seq].is_anticipated)
		data->qual[d1.seq].consult[d2.seq].result_dt_tm
	  endif
	  
	, consult_event
	
	, if (not data->qual[d1.seq].progress[d3.seq].is_anticipated)
		data->qual[d1.seq].progress[d3.seq].result_dt_tm
	  endif
	
	, progress_event
	
;with nocounter, outerjoin = d2, dontcare = per, outerjoin = d3, dontcare = per2, separator = " ", format, time = 180
with nocounter, outerjoin = d2, outerjoin = d3, separator = " ", format, time = 180
