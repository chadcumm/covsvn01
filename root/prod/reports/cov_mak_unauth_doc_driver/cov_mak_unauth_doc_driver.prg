...
002   06/02/2020  Chad Cummings			added In Progress status documents, echos
003   06/03/2020  Chad Cummings			removed him document requirement
******************************************************************************/
...
CREATE PROGRAM cov_mak_unauth_doc_driver :dba
 ...
 DECLARE inprogress_cd = f8 WITH public ,noconstant (0.0 ) ;002
 DECLARE doc_cd = f8 WITH public ,noconstant (0.0 ) ;003
 DECLARE mdoc_cd = f8 WITH public ,noconstant (0.0 ) ;003
 ...
 SET stat = uar_get_meaning_by_codeset (8 ,"IN PROGRESS" ,1 ,inprogress_cd ) ;002
 SET stat = uar_get_meaning_by_codeset (21 ,"SIGN" ,1 ,sign_cd )
 SET stat = uar_get_meaning_by_codeset (53 ,"DOC" ,1 ,doc_cd ) ;003
 SET stat = uar_get_meaning_by_codeset (53 ,"MDOC" ,1 ,mdoc_cd ) ;003
 ...
 CASE (request->date_flag )
  OF 1 :
   call echo(build2("request->date_flag = 1")) ;002
   SELECT INTO "nl:"
   ...
   ;end select
  OF 2 :
   call echo(build2("request->date_flag = 2")) ;002
   SELECT INTO "nl:"
    e.encntr_id ,
    ce.clinical_event_id ,
    ce.event_cd ,
    ...
    FROM (dummyt d WITH seq = value (org_qual_cnt ) ),
     (encounter e ),
    ;003 (clinical_event ce ),
    (clinical_event ce ) ;003
    ;003 (him_event_extension hee )
    PLAN (d )
     JOIN (e
     ...
     ;002 AND (ce.result_status_cd = unauth_cd )
     AND (ce.result_status_cd in(inprogress_cd,unauth_cd)) ;002
     AND (ce.event_class_cd in(doc_cd,mdoc_cd)) ;003
     AND ((ce.parent_event_id + 0 ) = ce.event_id )
     AND NOT (EXISTS (
     (SELECT
      cep.ce_event_prsnl_id
    ...
    DETAIL
     IF ((request->debug_ind = 1 ) ) doc_qual_cnt = (doc_qual_cnt + 1 ) ,stat = alterlist (temp->
      ...
      doc_qual_cnt ].clinical_event_id = ce.clinical_event_id ,temp->qual[encntr_qual_cnt ].doc_qual[
      doc_qual_cnt ].event_disp = event_disp ,temp->qual[encntr_qual_cnt ].doc_qual[doc_qual_cnt ].
      valid_from_dt_tm = ce.valid_from_dt_tm, temp->qual[encntr_qual_cnt ].doc_qual[
      doc_qual_cnt ].event_id = ce.event_id
     ENDIF
    WITH nocounter
   ;end select
 ENDCASE
 ...
 SELECT INTO "nl:"
  ...
  WITH nocounter
 ;end select
 call echo(build2("hold_file=",hold_file)) ;002
