...
Date Written :  7/24/18
Program Title:  Blood Bank CrossMatch Release Report
Source File	 :  cov_bbt_ops_batch_release.prg
Object Name	 :	bbt_ops_batch_release
Directory	 :	cust_script
DVD version  :	2017.11.1.81
HNA version  :	2017
CCL version  :	8.2.3
Purpose      :  This program has been modified from the
				Cerner standard original program. The original
				program has been translated and used in this
				program. The original Cerner code can be found in
				the reports folder of subversion if the updated
				program needs to be backed out.
 
Tables Read  :  encounter, person, orders, clinical_event
Tables
Updated      :	NA
Include Files:  NA
Executing
Application  :	Operations
Special Notes: 	Launched from the Operations.
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
Mod #        By           Date           Purpose
CR2078       MLayman      07/24/18       Adding the Patient ABO RH to the report.
...
DETAIL
	...
	ops_request->productlist[d.seq].productevent[d2.seq].pat_aborh =
	BUILD2(TRIM(UAR_GET_CODE_DISPLAY(pa.abo_cd)),' ',
 ...
 SELECT INTO cpm_cfn_info->file_name_logical
  ...
  HEAD pe.product_event_id
   ...
   IF ((pos > 0 ) )
    ...
    ].productevent[pos ].xm_exp_dt_tm "@DATETIMECONDENSED;;d" ,
    COL 127 ops_request->productlist[d_ar.seq].productevent[pos].pat_aborh,
    row + 1 ,formatted_acc = cnvtacc (ord.accession ) ,col 58 ,formatted_acc ,col 97 ,reason ,row + 1
   ENDIF
  DETAIL
   row + 0
  FOOT PAGE
