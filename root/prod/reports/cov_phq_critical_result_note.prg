...
drop program cov_phq_critical_result_note:dba go
create program cov_phq_critical_result_note:dba
...
join ce where ce.encntr_id = chart->plist[d1.seq].encntrid
	and ce.person_id = chart->plist[d1.seq].personid
 	and (ce.event_end_dt_tm between cnvtdatetime(chart->plist[d1.seq].result[d2.seq].lab_result_cmnt_dt)
		and cnvtlookahead('2, h',cnvtdatetime(chart->plist[d1.seq].result[d2.seq].lab_result_cmnt_dt)))
...
SELECT INTO $OUTDEV
	...
	, patient_location = trim(SUBSTRING(1, 100, CHART->plist[D1.SEQ].result_doc_location))
