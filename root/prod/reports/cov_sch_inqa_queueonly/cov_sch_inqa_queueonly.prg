...
Program Title	:	Request List by Queue
Source File		:	cov_sch_inqa_queueonly.prg
Object Name		:	cov_sch_inqa_queueonly
Directory		:	cust_script
DVD Version		:	2017.07.1.81
HNA Version		:	2015.01
CCL Version		:	8.8.3
Purpose			: 	This program is a translated copy of the original
					Cerner script - chs_sch_inqa_stjude_queueonly. CR 1259
...
Special Notes	:
Usage			:	cov_sch_inqa_queueonly go
Mod		Date		Engineer				Comment
----    ----------- ----------------------- ---------------------------
001		04/10/2018	Mike Layman				CR 1259
002		08/15/2018	Todd A. Blanchard		CR 3058 - Added ordering physician.
003		08/16/2018	Todd A. Blanchard		CR 3046 - Added order date/time.
004		09/19/2018	Todd A. Blanchard		CR 2948 - Added scheduled resource.
005		10/30/2018	Todd A. Blanchard		CR 3559 - Added ordering physician group.
006		11/02/2018	Todd A. Blanchard		Adjusted sql for physician practice.
007		11/14/2018	Todd A. Blanchard		Adjusted sql for scheduled auth number.
008		11/20/2018	Todd A. Blanchard		Adjusted sql for health plan data.
009		06/05/2019	Todd A. Blanchard		CR 5582 - Added encounter health plan auth_nbr.
010		09/10/2019	Todd A. Blanchard		CR 5516 - Added comments.
011     01/24/2020	Chad M. Cummings		Testing update to include hearing imparement.
012     02/04/2020	Chad M. Cummings		Added languge.
013     02/06/2020  Chad M. Cummings		Added column for alerts and languge.
014		03/02/2020	Todd A. Blanchard		Changed data sources of earliest_date and 
 											order_date columns.
015		08/06/2020	Todd A. Blanchard		Fixed issue with sch_inst and special_inst.
016		03/08/2022	Todd A. Blanchard		CR 12349 - Changed practice site display to org name.
017		08/03/2022	Todd A. Blanchard		CR 13340 - Added scheduling location.
018		09/08/2022	Todd A. Blanchard		CR 13605 - Added scheduling location.
...
DROP PROGRAM cov_sch_inqa_queueonly :dba GO
CREATE PROGRAM cov_sch_inqa_queueonly :dba
 DECLARE oe_authnumber = f8 WITH protect ,constant (uar_get_code_by("DISPLAYKEY" ,16449 ,"SCHAUTHNUMBER"))
 DECLARE oe_scheduling_location = f8 WITH protect ,constant(uar_get_code_by ("DISPLAYKEY" ,16449 ,"SCHEDULINGLOCATION")) ;017
 DECLARE oe_priority = f8 WITH protect ,constant (uar_get_code_by("DISPLAYKEY" ,16449 ,"PRIORITY")) ;018
 DECLARE oe_ord_physician = f8 WITH protect ,constant (uar_get_code_by("DISPLAYKEY" ,16449 ,"SCHEDULING ORDERING PHYSICIAN")) ;002
 DECLARE oe_sch_instructions = f8 WITH protect ,constant (uar_get_code_by("DISPLAYKEY" ,16449 ,"SCHEDULINGINSTRUCTIO12654")) ;010
 DECLARE oe_special_instructions = f8 WITH protect ,constant (uar_get_code_by("DISPLAYKEY" ,16449 ,"SPECIALINSTRUCTIONS")) ;010
 DECLARE oe_requested_start_dt_tm = f8 WITH protect ,constant (uar_get_code_by("DISPLAYKEY" ,16449 ,"REQUESTEDSTARTDATETIME")) ;014
 ...
 RECORD reply (
   ...
   1 query_qual [* ]
     ...
     2 sch_event_id = f8
     2 priority = c255 ;018
     2 order_dt_tm = dq8 ;003
     2 orders = vc
     2 order_cmt = vc
     2 pp_activity_id = f8
     2 prior_auth = vc
     2 ordering_physician = vc ;002
     2 ord_phys_group = vc ;005
     2 pp_activity = vc
     ...
     2 auth_nbr = vc ;009
     2 sch_inst = c255 ;010
     2 special_inst = c255 ;010
     2 scheduling_location = c255 ;017
     2 chart_alerts = vc ;012
   1 status_data
     2 status = c1
     2 subeventstatus [1 ]
       3 operationname = c25
 ...
 IF ((cs23010_disppidrl_cd > 0.0 ) )
  ...
  ;end select
 ENDIF
 SET reply->attr_qual_cnt = 40 ; limit columns ;010 ;012 ;017 ;018
 ...
 IF ((pref_value_disppidrl > 0.0 ) )
  SET reply->attr_qual_cnt = (reply->attr_qual_cnt + 6 )
 ENDIF
 SET t_index = 0
 ...
 ;017
 SET t_index = (t_index + 1 )
 SET reply->attr_qual[t_index ].attr_name = "scheduling_location"
 SET reply->attr_qual[t_index ].attr_label = uar_i18ngetmessage (i18nhandle ,"Scheduling Location" ,
  "Scheduling Location" )
 SET reply->attr_qual[t_index ].attr_type = "vc"
 ;
 SET t_index = (t_index + 1 )
 ...
 SET reply->attr_qual[t_index ].attr_type = "vc"
 SET t_index = (t_index + 1 )
 SET reply->attr_qual[t_index ].attr_name = "prior_auth"
 SET reply->attr_qual[t_index ].attr_label = uar_i18ngetmessage (i18nhandle ,"Prior Auth" ,
  "Prior Auth" )
 SET reply->attr_qual[t_index ].attr_type = "vc"
 ;009
 SET t_index = (t_index + 1 )
 SET reply->attr_qual[t_index ].attr_name = "auth_nbr"
 SET reply->attr_qual[t_index ].attr_label = uar_i18ngetmessage (i18nhandle ,"Health Plan Auth" ,
  "Health Plan Auth" )
 SET reply->attr_qual[t_index ].attr_type = "vc"
 ;
 SET t_index = (t_index + 1 )
  SET reply->attr_qual[t_index ].attr_name = "healthplan"
  SET reply->attr_qual[t_index ].attr_label = uar_i18ngetmessage (i18nhandle ,"Health Plan" ,
  "Health Plan" )
 SET reply->attr_qual[t_index ].attr_type = "vc"
 SET t_index = (t_index + 1 )
 ...
 ;003
 SET t_index = (t_index + 1 )
 SET reply->attr_qual[t_index ].attr_name = "order_dt_tm"
 SET reply->attr_qual[t_index ].attr_label = uar_i18ngetmessage (i18nhandle ,"Order Date" ,
  "Order Date" )
 SET reply->attr_qual[t_index ].attr_type = "dq8"
 ;
 SET t_index = (t_index + 1 )
 ...
 SET reply->attr_qual[t_index ].attr_type = "vc"
 SET t_index = (t_index + 1 )
 SET reply->attr_qual[t_index ].attr_name = "ordering_physician" ;002
 SET reply->attr_qual[t_index ].attr_label = uar_i18ngetmessage (i18nhandle ,"Ordering Physician" ,
  "Ordering Physician" )
 SET reply->attr_qual[t_index ].attr_type = "vc"
 ;005
 SET t_index = (t_index + 1 )
 SET reply->attr_qual[t_index ].attr_name = "ord_phys_group"
 SET reply->attr_qual[t_index ].attr_label = uar_i18ngetmessage (i18nhandle ,"Group/Practice" ,
  "Group/Practice" )
 SET reply->attr_qual[t_index ].attr_type = "vc"
 ;
 SET t_index = (t_index + 1 )
 ...
 ;004
 SET t_index = (t_index + 1 )
 SET reply->attr_qual[t_index ].attr_name = "resource"
 SET reply->attr_qual[t_index ].attr_label = uar_i18ngetmessage (i18nhandle ,"Scheduled Resource" ,
  "Scheduled Resource" )
 SET reply->attr_qual[t_index ].attr_type = "vc"
 ;
 ;010
 SET t_index = (t_index + 1 )
 SET reply->attr_qual[t_index ].attr_name = "sch_inst"
 SET reply->attr_qual[t_index ].attr_label = uar_i18ngetmessage (i18nhandle ,"Notes" , "Notes" )
 SET reply->attr_qual[t_index ].attr_type = "vc"
 SET t_index = (t_index + 1 )
 SET reply->attr_qual[t_index ].attr_name = "special_inst"
 SET reply->attr_qual[t_index ].attr_label = uar_i18ngetmessage (i18nhandle ,"Comments" ,
  "Comments" )
 SET reply->attr_qual[t_index ].attr_type = "vc"
 ;
 ;012
 SET t_index = (t_index + 1 )
 SET reply->attr_qual[t_index ].attr_name = "chart_alerts"
 SET reply->attr_qual[t_index ].attr_label = uar_i18ngetmessage (i18nhandle ,"Alerts" , "Alerts" )
 SET reply->attr_qual[t_index ].attr_type = "vc"
 ;
 IF ((pref_value_disppidrl > 0.0 ) )
  SET t_index = (t_index + 1 )
  SET reply->attr_qual[t_index ].attr_name = "pp_activity"
  SET reply->attr_qual[t_index ].attr_label = uar_i18ngetmessage (i18nhandle ,"PowerPlan Activity" ,
   "PowerPlan Activity" )
  SET reply->attr_qual[t_index ].attr_type = "vc"
  SET t_index = (t_index + 1 )
  ...
  SET reply->attr_qual[t_index ].attr_label = uar_i18ngetmessage (i18nhandle ,
   "PowerPlan Phase Activity" ,"PowerPlan Phase Activity" )
  SET reply->attr_qual[t_index ].attr_type = "vc"
  SET t_index = (t_index + 1 )
  ...
  SET reply->attr_qual[t_index ].attr_label = uar_i18ngetmessage (i18nhandle ,
   "PowerPlan Phase Reference" ,"PowerPlan Phase Reference" )
  SET reply->attr_qual[t_index ].attr_type = "vc"
  SET t_index = (t_index + 1 )
  SET reply->attr_qual[t_index ].attr_name = "pp_scheduled_phase"
  SET reply->attr_qual[t_index ].attr_label = uar_i18ngetmessage (i18nhandle ,
   "PowerPlan Scheduled Phase" ,"PowerPlan Scheduled Phase" )
  SET reply->attr_qual[t_index ].attr_type = "vc" 
 ENDIF
...
;      "HH:MM;;DATE" )
;    ELSE 
    	reply->query_qual[reply->query_qual_cnt ].time = ""
;    ENDIF
    ,
    IF ((ad_null = 0 ) )
    ...
    ENDIF
   ENDIF
  ; reply->query_qual[reply->query_qual_cnt].encntrid = e.encntr_id
  FOOT REPORT
...
FOR (cnt = 1 to reply->query_qual_cnt)
	...
	IF (reply->query_qual[cnt].hide#encounterid > 0)
		...
		FROM
			...
			, health_plan hp
			, encntr_plan_auth_r epar ;009
			, authorization au ;009
		...
		;009
		JOIN epar
		WHERE epar.encntr_plan_reltn_id = outerjoin(epr.encntr_plan_reltn_id)
		AND epar.active_ind = outerjoin(1)
		JOIN au
		WHERE au.authorization_id = outerjoin(epar.authorization_id)
		AND au.active_ind = outerjoin(1)
		;
 
		DETAIL
			reply->query_qual[cnt].healthplan = trim(hp.plan_name, 3)
			reply->query_qual[cnt].auth_nbr = trim(au.auth_nbr, 3)
	...
	ELSE
		...
		;009
		JOIN ppar
		WHERE ppar.person_plan_reltn_id = outerjoin(ppr.person_plan_reltn_id)
		AND ppar.active_ind = outerjoin(1)
		JOIN au
		WHERE au.authorization_id = outerjoin(ppar.authorization_id)
		AND au.active_ind = outerjoin(1)
		;
...
ENDFOR
 ...
 SELECT INTO "nl:"
  ...
  PLAN (d
   ...
   ;004
   JOIN (ar
   WHERE (ar.sch_event_id = reply->query_qual[d.seq ].hide#scheventid )
   AND (ar.schedule_id = reply->query_qual[d.seq ].hide#scheduleid )
   AND (ar.version_dt_tm = cnvtdatetime ("31-DEC-2100 00:00:00.00" ) )
   AND (ar.role_meaning != "PATIENT" ) )
   ;
  ORDER BY d.seq ,
   t_sort
  DETAIL
   ...
   reply->query_qual[d.seq ].resource = uar_get_code_display(ar.resource_cd) ;004
  WITH nocounter
 ;end select
 ...
 SELECT INTO "nl:"
  FROM (dummyt d WITH seq = value (reply->query_qual_cnt ) ),
   ...
   (PRSNL per ), ;005
   (PRSNL_RELTN pr ), ;005
   (PRACTICE_SITE ps ), ;005
   (ORGANIZATION org_ps ) ;016
  PLAN (d )
   JOIN (a
   WHERE (a.sch_event_id = reply->query_qual[d.seq ].hide#scheventid )
   AND (a.oe_field_id = oe_ord_physician ) ;002
   AND (a.beg_effective_dt_tm <= cnvtdatetime (curdate ,curtime3 ) )
   AND (a.end_effective_dt_tm > cnvtdatetime (curdate ,curtime3 ) )
   AND (a.active_ind = 1 ) )
   ;006
   JOIN (per
   WHERE (per.person_id = outerjoin(a.oe_field_value))
   AND (per.active_ind = outerjoin(1) ))
   JOIN (pr
   WHERE (pr.person_id = outerjoin(per.person_id))
   AND (pr.parent_entity_name = outerjoin("PRACTICE_SITE"))
   AND (pr.active_ind = outerjoin(1)))
   JOIN (ps
   WHERE (ps.practice_site_id = outerjoin(pr.parent_entity_id)))
   ;016
   JOIN (org_ps
   WHERE (org_ps.organization_id = outerjoin(ps.organization_id)))
   ;
  ORDER BY d.seq ,
   a.sch_action_id ,
   a.updt_dt_tm DESC
  HEAD a.sch_action_id
   null
  DETAIL
   reply->query_qual[d.seq ].ordering_physician = trim (a.oe_field_display_value ,3 ) ;002
   reply->query_qual[d.seq ].ord_phys_group = trim (org_ps.org_name ,3 ) ;006 ;016
  FOOT  a.sch_action_id
   null
  WITH nocounter
 ;end select
 ...
 SELECT INTO "nl:"
  ...
  PLAN (d
   ...
   AND (od.oe_field_id = outerjoin(oe_requested_start_dt_tm)))
   
  ORDER BY d.seq ,
   e.protocol_seq_nbr ,
   a.order_seq_nbr ,
   od.action_sequence ;014 
  ...
  DETAIL
   ...
   ,t_record->event_qual[d.seq ].order_qual[t_record->event_qual[d.seq ].order_qual_cnt ].order_id = a.order_id ,
	;003 ;014
    IF ((o.orig_order_dt_tm > cnvtdatetime ("01-JAN-1800 00:00:00.00" ) ) )
    	t_record->event_qual[d.seq ].order_qual[t_record->event_qual[d.seq ].order_qual_cnt ].order_dt_tm =
    		cnvtdatetime (o.orig_order_dt_tm)
    ELSE
    	t_record->event_qual[d.seq ].order_qual[t_record->event_qual[d.seq ].order_qual_cnt ].order_dt_tm = 0
    ENDIF
    
    ;014
    reply->query_qual[d.seq ].earliest_dt_tm = cnvtdate(od.oe_field_dt_tm_value)
 ...
 SELECT INTO "nl:"
  FROM (dummyt d WITH seq = value (reply->query_qual_cnt ) ),
   ...
   (order_detail od3 ), ;010
   (order_detail od4 ), ;017
   (order_detail od5 ) ;018
  PLAN (d )
   JOIN (a
   WHERE (a.sch_event_id = reply->query_qual[d.seq ].hide#scheventid )
   AND (a.beg_effective_dt_tm <= cnvtdatetime (curdate ,curtime3 ) )
   ...
   ;007
   JOIN (od
   WHERE (od.order_id = outerjoin(a.order_id))
   AND (od.oe_field_id = outerjoin(oe_authnumber)))
   
   ;010
   JOIN (od2
   WHERE (od2.order_id = outerjoin(a.order_id))
   AND (od2.oe_field_id = outerjoin(oe_sch_instructions)))
   
   ;010
   JOIN (od3
   WHERE (od3.order_id = outerjoin(a.order_id))
   AND (od3.oe_field_id = outerjoin(oe_special_instructions)))
   
   ;017
   JOIN (od4
   WHERE (od4.order_id = outerjoin(a.order_id))
   AND (od4.oe_field_id = outerjoin(oe_scheduling_location)))
   
   ;018
   JOIN (od5
   WHERE (od5.order_id = outerjoin(a.order_id))
   AND (od5.oe_field_id = outerjoin(oe_priority)))
   
  ORDER BY d.seq ,
   a.order_id ,
   od.action_sequence DESC ;007
   
  HEAD a.order_id
   null
  DETAIL
   reply->query_qual[d.seq ].prior_auth = trim (od.oe_field_display_value ,3 ) ;007
   reply->query_qual[d.seq ].sch_inst = trim (od2.oe_field_display_value ,3 ) ;010
   reply->query_qual[d.seq ].special_inst = trim (od3.oe_field_display_value ,3 ) ;010
   reply->query_qual[d.seq ].scheduling_location = trim (od4.oe_field_display_value ,3 ) ;017
   reply->query_qual[d.seq ].priority = trim (od5.oe_field_display_value ,3 ) ;018
  FOOT  a.order_id
   null
  WITH nocounter
  ;
  ...
  SELECT INTO "nl:"
   ...
   HEAD o.order_id
    IF ((reply->query_qual[d.seq ].orders <= " " ) )
    	reply->query_qual[d.seq ].order_dt_tm = t_record->event_qual[d.seq ].order_qual[d2.seq ].order_dt_tm ;003
   ...
   DETAIL
    ...
    AND (od_exists = 1 ) )
     ;007
     IF (od.oe_field_meaning_id = 124 )
     	reply->query_qual[d.seq ].prior_auth = od.oe_field_display_value
     ENDIF
     ;
     ,
     ...
     ,
     IF ((pref_value_disppidrl > 0.0 ) )
      IF ((od.oe_field_meaning_id = 3519 ) ) reply->query_qual[d.seq ].pp_activity_id = od
       .oe_field_value ,reply->query_qual[d.seq ].pp_activity = od.oe_field_display_value
      ENDIF
      ,
      IF ((od.oe_field_meaning_id = 3520 ) ) reply->query_qual[d.seq ].pp_phase_activity = od
       .oe_field_display_value
      ENDIF
      ,
      IF ((od.oe_field_meaning_id = 3521 ) ) reply->query_qual[d.seq ].pp_reference = od
       .oe_field_display_value
      ENDIF
      ,
      IF ((od.oe_field_meaning_id = 3522 ) ) reply->query_qual[d.seq ].pp_phase_reference = od
       .oe_field_display_value
      ENDIF
      ,
      IF ((od.oe_field_meaning_id = 3523 ) ) reply->query_qual[d.seq ].pp_scheduled_phase_id = od
       .oe_field_value ,reply->query_qual[d.seq ].pp_scheduled_phase = od.oe_field_display_value
      ENDIF
      ,
      IF ((pref_value_disppidrl = 1.0 ) )
 ...
 ENDIF
	...
	where (reply->query_qual[d.seq ].hide#personid > 0 )
	join pp
	where pp.person_id = reply->query_qual[d.seq ].hide#personid
		and pp.process_alert_cd = value(uar_get_code_by("DISPLAY",19350,"Hearing Impaired"))
		and pp.active_ind = 1
	
	detail
		if (reply->query_qual[d.seq ].chart_alerts = " ")
			reply->query_qual[d.seq ].chart_alerts
			     = build2(trim(uar_get_code_display(pp.process_alert_cd))," ", reply->query_qual[d.seq ].chart_alerts)
		else
			reply->query_qual[d.seq ].chart_alerts
				= build2(trim(uar_get_code_display(pp.process_alert_cd)),",", reply->query_qual[d.seq ].chart_alerts)
		endif
	...
	join pp
	where pp.person_id = reply->query_qual[d.seq ].hide#personid
		and pp.language_cd not in(value(uar_get_code_by("MEANING",36,"ENG")))
	detail
		if (reply->query_qual[d.seq ].chart_alerts = " ")
			reply->query_qual[d.seq ].chart_alerts
			     = build2(trim(uar_get_code_display(pp.language_cd))," ", reply->query_qual[d.seq ].chart_alerts)
		else
			reply->query_qual[d.seq ].chart_alerts
				= build2(trim(uar_get_code_display(pp.language_cd)),",", reply->query_qual[d.seq ].chart_alerts)
		endif
 ...
 IF ((pref_value_disppidrl = 1.0 ) )
  SELECT INTO "nl:"
   ...
   val_resource = substring (1 ,30 ,reply->query_qual[d.seq ].resource ) , ;004
   val_notes = substring (1 ,255 ,reply->query_qual[d.seq ].sch_inst) , ;010 ;015
   val_comments = substring (1 ,255 ,reply->query_qual[d.seq ].special_inst) , ;010 ;015
   val_scheduling_location = substring (1 ,255 ,reply->query_qual[d.seq ].scheduling_location) , ;017
   val_prior_auth = substring (1 ,30 ,reply->query_qual[d.seq ].prior_auth ) ,
   val_auth_nbr = substring (1 ,30 ,reply->query_qual[d.seq ].auth_nbr ) , ;009
   val_healthplan =  substring(1,35,reply->query_qual[d.seq].healthplan), ;008
   val_ordering_physician = substring (1 ,100 ,reply->query_qual[d.seq ].ordering_physician ) , ;002
   val_ord_phys_group = substring (1 ,100 ,reply->query_qual[d.seq ].ord_phys_group ) , ;005
   ...
   val_priority = reply->query_qual[d.seq ].priority , ;018
   val_order_dt_tm = reply->query_qual[d.seq ].order_dt_tm , ;003
   ...
   val_pp_activity = substring (1 ,value (max_length_pp_activity ) ,reply->query_qual[d.seq ].
    pp_activity ) ,
   val_pp_phase_activity = substring (1 ,value (max_length_pp_phase_activity ) ,reply->query_qual[d
    .seq ].pp_phase_activity ) ,
   val_pp_reference = substring (1 ,value (max_length_pp_reference ) ,reply->query_qual[d.seq ].
    pp_reference ) ,
   val_pp_phase_reference = substring (1 ,value (max_length_pp_phase_reference ) ,reply->query_qual[d
    .seq ].pp_phase_reference ) ,
   val_pp_scheduled_phase_id = reply->query_qual[d.seq ].pp_scheduled_phase_id ,
   val_pp_scheduled_phase = substring (1 ,value (max_length_pp_scheduled_phase ) ,reply->query_qual[d
    .seq ].pp_scheduled_phase )
   FROM (dummyt d WITH seq = value (reply->query_qual_cnt ) )
   PLAN (d )
   ...
   DETAIL
    ...
    reply->query_qual[sch2_idx ].resource = val_resource , ;004
    reply->query_qual[sch2_idx ].sch_inst = val_notes , ;010 ;015
    reply->query_qual[sch2_idx ].special_inst = val_comments , ;010 ;015
    reply->query_qual[sch2_idx ].scheduling_location = val_scheduling_location , ;017
    reply->query_qual[sch2_idx ].prior_auth = val_prior_auth ,
    reply->query_qual[sch2_idx ].auth_nbr = val_auth_nbr , ;009
    reply->query_qual[sch2_idx ].healthplan = val_healthplan,
    reply->query_qual[sch2_idx ].ordering_physician = val_ordering_physician , ;002
    reply->query_qual[sch2_idx ].ord_phys_group = val_ord_phys_group , ;005
    ...
    reply->query_qual[sch2_idx ].order_dt_tm = val_order_dt_tm , ;003
    reply->query_qual[sch2_idx ].orders = val_orders ,
    reply->query_qual[sch2_idx ].order_cmt = val_order_cmt ,
    reply->query_qual[sch2.idx ].priority = val_priority , ;018
...
#exit_script
 ...
 FREE SET t_record
 SET lastmod =
 "001 12/13/2017 DS051289 CCPS-12760 Adding Prior Auth and Ordering Physician column to Request List."
END GO
