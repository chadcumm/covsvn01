...
******************************************************************************
	...
	Solution:			Pharmacy
	Source file name:	      cov_pha_medadmin_validation.prg
	Object name:		cov_pha_medadmin_validation
 
	Request#:			As per Tommy Paterson
	Program purpose:	      Pharmacy Medication administration validation
	Executing from:		DA2
...
DROP PROGRAM cov_pha_medadmin_validation :dba go
CREATE PROGRAM cov_pha_medadmin_validation :dba
...
with OUTDEV, start_datetime, end_datetime, facility_list, class_code,
	code_value
...
declare filename_var = vc WITH noconstant(CONCAT('cer_temp:',TRIM(cnvtlower(uar_get_displaykey($facility_list))),'_pha_scorecard_medadmin.txt')), PROTECT
declare ccl_filepath_var = vc WITH noconstant(CONCAT('$cer_temp/',TRIM(cnvtlower(uar_get_displaykey($facility_list))),'_pha_scorecard_medadmin.txt')), PROTECT
...
declare class_code_var = vc with noconstant('')
 
;Set class code
case ($class_code)
	of 1 : set class_code_var = build2("MED_ADMIN->mlist[D1.SEQ].drug_class_code1 = $code_value")
	of 2 : set class_code_var = build2("MED_ADMIN->mlist[D1.SEQ].drug_class_code2 = $code_value")
	of 3 : set class_code_var = build2("MED_ADMIN->mlist[D1.SEQ].drug_class_code3 = $code_value")
 	else   set class_code_var = build2("MED_ADMIN->mlist[D1.SEQ].fin != ''")
endcase
...
Detail
	...
	med_admin->mlist[mcnt].parser_class_code = class_code_var
...
Head cmr.event_id
	...
	while(idx > 0)
		if(med_admin->mlist[idx].strength_dose = '0.000' or med_admin->mlist[idx].strength_dose = '')
			med_admin->mlist[idx].strength_dose = cnvtstring(cmr.admin_dosage)
...
if($class_code = 1)
	...
	, ORDERING_PHYSICIAN_NAME = TRIM(SUBSTRING(1, 50, MED_ADMIN->mlist[D1.SEQ].ordering_physician_name))
	, ORDERING_PHYSICIAN_NUMBER = TRIM(SUBSTRING(1, 15, MED_ADMIN->mlist[D1.SEQ].ordering_phys_number))
	...
	FROM (DUMMYT   D1  WITH SEQ = VALUE(SIZE(MED_ADMIN->mlist, 5)))
 
	PLAN D1 where MED_ADMIN->mlist[D1.SEQ].drug_class_code1 = $code_value
...
elseif($class_code = 2)
	...
	PLAN D1 where MED_ADMIN->mlist[D1.SEQ].drug_class_code2 = $code_value
...
elseif($class_code = 3)
	...
	, ORDERING_PHYSICIAN_NAME = TRIM(SUBSTRING(1, 50, MED_ADMIN->mlist[D1.SEQ].ordering_physician_name))
	, ORDERING_PHYSICIAN_NUMBER = TRIM(SUBSTRING(1, 15, MED_ADMIN->mlist[D1.SEQ].ordering_phys_number))
	, ATTENDING_PHYSICIAN_NUMBER =trim(SUBSTRING(1, 30, MED_ADMIN->mlist[D1.SEQ].atend_prsnl_number))
	, PERFORMED_PERSON_USERNAME = SUBSTRING(1, 30, MED_ADMIN->mlist[D1.SEQ].perform_username)
 
	FROM (DUMMYT   D1  WITH SEQ = VALUE(SIZE(MED_ADMIN->mlist, 5)))
 
	PLAN D1 where MED_ADMIN->mlist[D1.SEQ].drug_class_code3 = $code_value
