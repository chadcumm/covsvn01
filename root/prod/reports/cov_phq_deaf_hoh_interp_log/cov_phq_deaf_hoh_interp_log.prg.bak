...
select into $outdev
sa.appt_location_cd, sa.encntr_id, sa.person_id,sa.beg_dt_tm, sa.sch_event_id, sa.beg_dt_tm ';;q'
, deaf_response = sed.oe_field_display_value, interp_type = sed1.oe_field_display_value, sed1.oe_field_id
 
from sch_appt sa
	, sch_event_detail sed
	, (left join sch_event_detail sed1 on sed1.sch_event_id = sed.sch_event_id
...
select into $outdev
ce2.person_id,ce2.encntr_id ,ce2_eid = ce2.event_id ,ce2.parent_event_id, ce2.result_val, ce2.verified_dt_tm ';;q'
,ce_eid = ce.event_id, ce.result_val, ce.parent_event_id, ce.verified_dt_tm ';;q'
 
from clinical_event ce2
	,(left join clinical_event ce on ce.encntr_id = ce2.encntr_id 
		and ce.parent_event_id = ce2.parent_event_id
...
/*join ce where ce.encntr_id = ce2.encntr_id 
	and ce.parent_event_id = ce2.parent_event_id
	and ce.event_id = (select max(ce1.event_id) from clinical_event ce1
		where ce1.encntr_id = ce.encntr_id
		and ce1.parent_event_id = ce.parent_event_id
...
order by ce2.encntr_id, ce2.parent_event_id, ce2.verified_dt_tm desc
...
Head ce2.encntr_id
	icnt = 0
	idx = 0
      idx = locateval(icnt ,1 ,pat->rec_cnt ,ce2.encntr_id ,pat->plist[icnt].encntrid)
...
;--------------------------------------------------------------------------------------------------------------
;Interpreter Service - patient/companion
...
plan ce where expand(num, 1,pat->rec_cnt, ce.encntr_id, pat->plist[num].encntrid)
	and ce.event_cd = value(uar_get_code_by("DISPLAY", 72, "Interpreter Services for:"));patient/companion
	and ce.valid_until_dt_tm = cnvtdatetime ("31-DEC-2100 00:00:00")
	and ce.result_status_cd IN (23.00, 34.00, 25.00, 35.00)
	and ce.event_id = (select max(ce1.event_id) from clinical_event ce1
			where ce1.encntr_id = ce.encntr_id
...
Head ce.encntr_id
	...
	idx = 0
      ...
      if(idx > 0)
      	pat->plist[idx].nu_interp_service_for = ce.result_val ;patient/companion
      endif	
with nocounter, expand = 1
...
plan ce where expand(num, 1,pat->rec_cnt, ce.encntr_id, pat->plist[num].encntrid)
	and ce.event_id = (select max(ce1.event_id) from clinical_event ce1
		where ce1.encntr_id = ce.encntr_id
		and (ce1.event_cd = value(uar_get_code_by("DISPLAY", 72, "Interpreter Services")) 
			or ce1.event_cd = value(uar_get_code_by("DISPLAY", 72, "Interpreter Agency Name")))
		and ce1.valid_until_dt_tm = cnvtdatetime ("31-DEC-2100 00:00:00" )
		and ce1.result_status_cd IN (23.00, 34.00, 25.00, 35.00)
		group by ce1.encntr_id, ce1.event_cd)
...
from  pm_transaction pt
	, (left join person_patient pp on pp.person_id = pt.n_person_id
...
select into $outdev
	facility = trim(uar_get_code_display(pat->plist[d1.seq].facility))
	...
	, sch_interpreter_response = trim(substring(1, 300, pat->plist[d1.seq].sch_interp_req))
	, sch_interpreter_request_dt = pat->plist[d1.seq].sch_interp_req_dt ';;q'
	, sch_interpreter_type = trim(substring(1, 300, pat->plist[d1.seq].sch_interp_type))
	, reg_dt_tm = pat->plist[d1.seq].reg_dt ';;q'
	, reg_interpreter_response = trim(substring(1, 300, pat->plist[d1.seq].reg_interp_res))
	, reg_interpreter_req_dt = pat->plist[d1.seq].reg_interp_req_dt ';;q'
	, reg_interpreter_type = trim(substring(1, 300, pat->plist[d1.seq].reg_interp_type))
	, nu_communication_barrier = trim(substring(1, 1000, pat->plist[d1.seq].nu_interp_res))
	, nu_requested_communication_type = trim(substring(1, 300, pat->plist[d1.seq].nu_interp_type))
	, nu_interpreter_req_dt = pat->plist[d1.seq].nu_interp_req_dt ';;q'
	, nature_of_service_provided = trim(substring(1, 1000, pat->plist[d1.seq].nu_typ_service_provided))
	, service_provided_dt = pat->plist[d1.seq].nu_interp_service_dt ';;q'
	, reason_no_service = trim(substring(1, 1000, pat->plist[d1.seq].reason_no_service))
	, employee_decision_no_service = trim(substring(1, 100, pat->plist[d1.seq].pr_no_service_decision))
	, form_on_chart = trim(substring(1, 100, pat->plist[d1.seq].comm_assess_form))
