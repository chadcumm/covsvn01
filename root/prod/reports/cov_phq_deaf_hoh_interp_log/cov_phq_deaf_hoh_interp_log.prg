...
----------	-----------------------------------------------------------------
09-21-22    Geetha         Initial release as excel format
******************************************************************************/
...
plan sa where expand(num, 1, pat->rec_cnt, sa.encntr_id, pat->plist[num].encntrid)
	;where sa.encntr_id = 131604420.00
	and sa.active_ind = 1
	and sa.sch_state_cd = value(uar_get_code_by("DISPLAY", 14233, "Confirmed"))
	and sa.sch_role_cd = value(uar_get_code_by("DISPLAY", 14250, "Patient"))
...
;with nocounter, separator=" ", format, format(date,"mm-dd-yyyy hh:mm:ss;;d"), time = 180, uar_code(d,1), expand = 1;, maxrow = 10000
;go to exitscript
...
;--------------------------------------------------------------------------------------------------------------
;Nursing ; fin = '5225800629', eid = 131462592.00 9/15/06am
...
;Lori to get with Debi - charted time or service begin time (not in build yet)
 ...
 ;9/21-  42963395.00 Interpreter Agency Name	Stratus Video Remote Interpreting (sign/foreign language) ;actual patient result
