DROP PROGRAM cov_rx_new_order_rpt :dba GO
CREATE PROGRAM cov_rx_new_order_rpt :dba
 PROMPT
 ...
 SELECT INTO "nl:"
  ...
  HEAD REPORT
   cnt = 0
  HEAD o.order_id
   cnt = (cnt + 1 ) ,stat = alterlist (ord->ord ,cnt ) ,ord->ord[cnt ].order_id = o.order_id ,ord->
   ord[cnt ].encntr_loc_hist_id = elh.encntr_loc_hist_id
  WITH nullreport
 ;end select
 FREE RECORD fin
 RECORD fin (
