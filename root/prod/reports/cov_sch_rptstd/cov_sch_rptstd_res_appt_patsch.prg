...
******************************************************************************
	...
	Source file name:	cov_sch_rptstd_res_appt_patsch.prg
	Object name:		cov_sch_rptstd_res_appt_patsch
	Request #:			12544
 
	Program purpose:	Provides data for Standard Resource Appointment List with all Person Appointments.
...
DROP PROGRAM cov_sch_rptstd_res_appt_patsch :dba GO
CREATE PROGRAM cov_sch_rptstd_res_appt_patsch :dba
 ...
 SELECT INTO  $1
  ...
  HEAD PAGE
   ...
   row + 1 ,
   "{POS/126/70}{B}Resource Daily Appointment Summary (with all Appointments)" ,
   row + 1 ,
   "{F/4}{CPI/11}{LPI/6}" ,
    ...
    FOR (j = 1 TO t_rec->qual[i ].patient_qual_cnt )
     ...
     IF ((t_rec->qual[i ].patient_qual[j ].appt_qual_cnt > 0 ) )
      ...
      FOR (k = 1 TO t_rec->qual[i ].patient_qual[j ].appt_qual_cnt )
       ...
       "@SHORTDATE" ,
       CALL print (calcpos (212 ,y_pos ) ) ,t_rec->qual[i ].patient_qual[j ].appt_qual[k ].beg_dt_tm
       "@TIMENOSECONDS" ,
       ...
       appt_synonym_free ,
       CALL print (calcpos (416 ,y_pos ) ) ,t_rec->qual[i ].patient_qual[j ].appt_qual[k ].
       sch_state_disp ,
       CALL print (calcpos (486 ,y_pos ) ) ,t_rec->qual[i ].patient_qual[j ].appt_qual[k ].
       primary_resource_mnem ,row + 1
      ENDFOR
     ENDIF
    ENDFOR
    ,
