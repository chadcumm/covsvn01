...
******************************************************************************
	...
	Source file name:	cov_sch_rptstd_res_appt_mri_pm.prg
	Object name:		cov_sch_rptstd_res_appt_mri_pm
	Request #:			12618
 
	Program purpose:	Provides data for Standard MRI Resource Appointment List with Pacemaker.
...
DROP PROGRAM cov_sch_rptstd_res_appt_mri_pm:dba GO
CREATE PROGRAM cov_sch_rptstd_res_appt_mri_pm:dba
 ...
 DECLARE mri_pacemaker = f8 WITH public ,constant (4036671.00 )
 DECLARE mri_yes = f8 WITH public ,constant (uar_get_code_by("DISPLAYKEY", 20322, "YES"))
 FREE SET t_rec
 RECORD t_rec (
   1 temp_string = vc
   1 temp_size = i4
  ...
  PLAN (d )
   ...
   AND (exists (
   	...
   	where 
		...
		AND ed2.oe_field_id = mri_pacemaker
		AND ed2.oe_field_value = mri_yes
   ))
   )
   JOIN (oef
   WHERE (oef.oe_field_id = ed.oe_field_id ) 
   AND (oef.oe_field_id in (mri_pacemaker) ))
 ...
 SELECT INTO  $1
  ...
  HEAD PAGE
   ...
   row + 1 ,
   "{POS/140/70}{B}MRI Resource Daily Appointment Summary (with Pacemaker)" ,
   row + 1 ,
   "{F/4}{CPI/11}{LPI/6}" ,
