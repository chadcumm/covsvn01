...
*********************************************************************
 ...
 ----  ----------- ---------------- ---------------------------
 0001  06/29/2020   Dawn Greer, DBA  For the Get Orders query made some tables
                                     be a left join so that orders that do not have
                                     results will show up.
 0002  06/30/2020   Dawn Greer, DBA  If no records found then just display a blank report
 0003  01/25/2021   Dawn Greer, DBA  CR 6941 - Add Provider Prompt and cleanup code,
                                     replace variables with code values and documentation
*************************************************************************************/
...
with OUTDEV, Facility, ordlab, perflab, actsubtype, test, prov, enctype, begdate,
	enddate
...
;SubType Prompt
IF ($actsubtype = 1)
	SET actsubtypesql = BUILD('oc.activity_subtype_cd IN (',2499.00 /*Chemistry*/,',', 2500.00 /*Coagulation*/,',',
	2501.00 /*Hematology*/,',', 2507.00 /*Immunology*/,',',2510.00 /*Urinalysis*/,',', 2553776215.00 /*Micro/GenLab*/,')')
ELSE
...
FROM order_catalog oc
	...
	,(LEFT JOIN orc_resource_list orl ON (o.catalog_cd = orl.catalog_cd
		AND orl.service_resource_cd IN (SELECT sr.service_resource_cd
			...
			AND sr.service_resource_type_cd IN (823.00 /*Bench*/, 827.00 /*Instrument*/))
		))
	...
	,(LEFT JOIN service_resource sr ON (pr.service_resource_cd = sr.service_resource_cd
		AND sr.activity_type_cd IN (692 /*Gen Lab*/, 674 /*Blood Bank*/, 696 /*Micro*/)
		))
...
HEAD o.order_id
	...
	IF (mod(cnt,10) = 1 OR cnt = 1)
 	...
 	a->oqual[cnt].ordering_prov = ord_prov.name_full_formatted
...
FROM order_catalog oc,
	 ...
	 prsnl ord_prov,
	 encounter e
PLAN oc WHERE parser(testsql)
JOIN o WHERE o.orig_order_dt_tm BETWEEN CNVTDATETIME($begdate)
	AND CNVTDATETIME($enddate)
	AND oc.catalog_cd = o.catalog_cd
	AND O.activity_type_cd IN (692 /*Gen Lab*/, 674 /*Blood Bank*/, 696 /*Micro*/)
	AND O.dept_status_cd IN (9312 /*Completed*/, 9329 /*Preliminary*/, 9321 /*Final*/)
	AND o.cs_flag IN (1.00 /*Care Set Parent*/, 4.00 /*Supergroup Parent*/, 16.00 /*Care Plan Parent*/)
JOIN oa WHERE o.order_id = oa.order_id
	AND oa.action_type_cd = 2534.00 /*order*/
	AND PARSER(provprompt)
JOIN ord_prov WHERE oa.order_provider_Id = ord_prov.person_id
...
SELECT INTO 'nl:'
FROM location l
WHERE l.location_type_cd = 791.00 /*Laboratory*/
AND l.active_ind = 1
