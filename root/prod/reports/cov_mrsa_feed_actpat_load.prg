...
******************************************************************************
	...
	Solution:			Quality
	Source file name:	      cov_phq_chg_mrsa_feed.prg
	Object name:		cov_phq_chg_mrsa_feed
	Request#:			9964
...
drop program cov_mrsa_feed_actpat_load:dba go
create program cov_mrsa_feed_actpat_load:dba
...
set edt_var = '05-AUG-2021 23:59:59'
...
declare filename_var = vc WITH constant('cer_temp:cov_mrsa_feed_aug06_16.txt'), PROTECT
declare ccl_filepath_var = vc WITH constant('$cer_temp/cov_mrsa_feed_aug06_16.txt'), PROTECT
...
plan e where e.loc_facility_cd = $acute_facility_list
	...
	and e.encntr_status_cd = 854.00 ;Aactive
...
e.encntr_id, e.encntr_type_cd, e.reg_dt_tm, e.disch_dt_tm, e.arrive_dt_tm, elh.encntr_type_cd
 
from 	encounter e
	, encntr_loc_hist elh
 
plan e where e.loc_facility_cd = $acute_facility_list ;where e.encntr_id = 124751871
	and e.reg_dt_tm >= cnvtdatetime('01-JAN-2021 00:00:00')
	and e.encntr_type_cd in(309308.00, 309312.00,19962820.00);Inpatient,Observation,Outpatient in a Bed
	and e.active_ind = 1
...
Detail
	...
	pat->plist[cnt].unit = e.loc_nurse_unit_cd
...
;-----------------------------------------------------------------------------------------------
;Patient population - Discharged patients
...
Head e.encntr_id
	idx = 0
	icnt = 0
      idx = locateval(icnt ,1 ,size(pat->plist,5) ,e.encntr_id ,pat->plist[icnt].encntrid)
	if(idx = 0) ;not an existing encounter
		cnt += 1
		pat->rec_cnt = cnt
		call alterlist(pat->plist, cnt)
		pat->plist[cnt].enc_ecnt = ecnt
...
;------------------------------------------------------------------------------------------
;Patients admitted on or after 06/01/21 - get data up to to 8/5/21 regardless of patient status - 8/18
 
;Patients admitted on or after 06/01/21 - get data from 8/6 to 8/16/21 regardless of patient status - 8/19
 
 
select into $outdev
 
e.encntr_id, e.encntr_type_cd, e.reg_dt_tm, e.disch_dt_tm
 
from 	encounter e
	, encntr_loc_hist elh
 
plan e where e.loc_facility_cd = $acute_facility_list
	;and e.reg_dt_tm between cnvtdatetime($start_datetime) and cnvtdatetime($end_datetime) - 8/18
	and e.reg_dt_tm between cnvtdatetime('01-JUN-2021 00:00:00') and cnvtdatetime('16-AUG-2021 23:59:59') ;8/19
	and e.encntr_type_cd in(309308.00, 309312.00,19962820.00);Inpatient,Observation,Outpatient in a Bed
	and NOT(e.disch_dt_tm <= cnvtdatetime('06-AUG-2021 00:00:00'));8/19
	and e.active_ind = 1
...
join a where a.person_id = pat->plist[d.seq].personid
	and a.created_dt_tm between cnvtdatetime($start_datetime) and cnvtdatetime($end_datetime)
	and a.active_ind = 1
...
join ta where ta.encntr_id = pat->plist[d.seq].encntrid
	and ta.task_create_dt_tm between cnvtdatetime($start_datetime) and cnvtdatetime($end_datetime)
	and ta.active_ind = 1
...
select into $outdev
enc = pat->plist[d.seq].encntrid, ce.event_id,ce.event_title_text, ce.result_val
...
Head enc
	;call echo(build('enc = ', enc))
	idx = 0
	icnt = 0
      idx = locateval(icnt ,1 ,size(pat->plist,5),enc ,pat->plist[icnt].encntrid)
      ecnt = pat->plist[d.seq].enc_ecnt
      stat = alterlist(pat->plist[idx]->events, ecnt+100)
Head ce.event_id
	ecnt += 1
	if(mod(ecnt, 10) = 1 and ecnt > 100)
		stat = alterlist(pat->plist[idx]->events, ecnt+9)
	endif
 
	;call alterlist(pat->plist[idx].events, ecnt)
	;call echo(build('enc = ', enc,'--idx = ', idx,'---event_id = ', ce.event_id, '--- ecnt = ', ecnt))
	pat->plist[idx].enc_ecnt = ecnt
	pat->plist[idx].events[ecnt].trackid = ce.event_id
	pat->plist[idx].events[ecnt].eventid = ce.event_id
...
Foot enc
	stat = alterlist(pat->plist[idx]->events, ecnt)
...
;------------------------------------------------------------------------------------------------------------
;Assign/overrite default Nurse Units
 
select into $outdev
 
enc = nu->list[d3.seq].encntrid, eve_id = nu->list[d3.seq].eventid, dt = nu->list[d3.seq].date_val ';;q'
