...
******************************************************************************
	...
	Request#:			3805
	Program purpose:	      Snapshot of Powerchart
	Executing from:		DA2
 	Special Notes:
...
drop program cov_phq_patient_dashboard_copy:dba go
create program cov_phq_patient_dashboard_copy:dba
...
declare resucite_var     = f8 with constant(uar_get_code_by('DISPLAY', 200, 'Resuscitation Status/Medical Interventio')), protect
...
detail
	mas->report_ran_by = usr_name
with nocounter
 
;------------------------------------------------------------------------------------------
;Main Qualification - all inpatients
select into 'nl:'
...
Detail
	...
	mas->plist[cnt].pat_name = patient_name
...
, prsnl = initcap(pr.name_full_formatted)
...
Head epr.encntr_id
	...
	admit_pr = fillstring(1000," "), atten_pr = fillstring(1000," "),cnslt_pr = fillstring(1000," ")
Detail
	case (epr.encntr_prsnl_r_cd)
		of admit_phys_var:
			admit_pr = build2(trim(admit_pr),'[' ,trim(prsnl),']',',')
		of atten_phys_var:
			atten_pr = build2(trim(atten_pr),'[' ,trim(prsnl),']',',')
		of cnslt_phys_var:
			cnslt_pr = build2(trim(cnslt_pr),'[' ,trim(prsnl),']',',')
 	endcase
 
Foot epr.encntr_id
	mas->plist[idx].admit_prsnl = replace(trim(admit_pr),",","",2)
	mas->plist[idx].attend_prsnl = replace(trim(atten_pr),",","",2)
	mas->plist[idx].consult_prsnl = replace(trim(cnslt_pr),",","",2)
 
 	;build2(trim(fall_list_var),trim(event1),':-[' ,trim(ce.result_val),']',',')
with nocounter
...
join o where o.encntr_id = mas->plist[d.seq].encntrid
	...
	and o.order_id = (select max(o1.order_id) from orders o1
			 where o1.encntr_id = o.encntr_id and o1.order_mnemonic = o.order_mnemonic)
	and o.active_status_cd = 188
	and o.order_status_cd = 2550.00 ;Ordered
	and o.current_start_dt_tm <= sysdate
	and (o.projected_stop_dt_tm > sysdate or o.projected_stop_dt_tm is null)
...
join ce where ce.person_id = mas->plist[d.seq].personid
	...
	and ce.event_cd in(f_fall_high_var, f_age_85_var, f_bone_dis_var, f_coagu_disord_var, f_surg_anes_var
					, f_fall_score_var, f_sph_score_var)
	;f_hist_fall_var, f_second_dig_var, f_ambu_aid_var, f_ivhep_fall_var, f_gait_weak_var, f_mental_stat_var-removed as per Lori
	and ce.event_id = (select max(ce1.event_id) from clinical_event ce1 where ce1.encntr_id = ce.encntr_id
		and ce1.event_cd = ce.event_cd
		group by ce1.encntr_id, ce1.event_cd)
...
order by dg.encntr_id, dg.diag_priority asc ;dg_dt1 ;dg.beg_effective_dt_tm
...
;As per Debby's July,2019 spec.
; Show only data elements used in Layout builder.
SELECT DISTINCT INTO $OUTDEV
	...
	, AGE = SUBSTRING(1, 10, MAS->plist[D1.SEQ].age)
	, GENDER = SUBSTRING(1, 30, MAS->plist[D1.SEQ].gender)
	, DOB = SUBSTRING(1, 30, MAS->plist[D1.SEQ].dob)
	, PAT_TYPE = SUBSTRING(1, 50, MAS->plist[D1.SEQ].pat_type)
	, ADMIT_DT = SUBSTRING(1, 30, MAS->plist[D1.SEQ].admit_dt)
	, FIN = SUBSTRING(1, 10, MAS->plist[D1.SEQ].fin)
	, PATIENT_PIN = SUBSTRING(1, 10, MAS->plist[D1.SEQ].pat_pin)
	...
	, ADMIT_PRSNL = SUBSTRING(1, 200, MAS->plist[D1.SEQ].admit_prsnl)
	, CONSULT_PRSNL = SUBSTRING(1, 200, MAS->plist[D1.SEQ].consult_prsnl)
	, FALL_INTERVENTION = trim(SUBSTRING(1, 700, MAS->plist[D1.SEQ].fall))
	, DIAGNOSES = trim(SUBSTRING(1, 700, MAS->plist[D1.SEQ].diagnoses))
...
ORDER BY NURSE_UNIT, ROOM, PAT_NAME, FIN
 
 
WITH NOCOUNTER, SEPARATOR=" ", FORMAT
 
/*
;Original - Show all data elements that are built in this file
