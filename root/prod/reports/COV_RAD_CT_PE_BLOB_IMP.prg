...
003     04/21/2020  Dawn Greer, DBA         Changed the MRN to be the CMRN.  Remove 
                                            Returns from the impression string                                            
...
DECLARE cmrn = f8 WITH CONSTANT(UAR_GET_CODE_BY('DISPLAYKEY',4,'COMMUNITYMEDICALRECORDNUMBER')), PROTECT  ;003 - Changed to CMRN
...
CALL ECHO(BUILD('cmrn :', cmrn))		;003 - Changed to CMRN
...
AND pa.person_alias_type_cd = cmrn   ;003 - Changed to CMRN
...
INTO "NL:"
	...
	Impression = REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(CONCAT(a->qual[d.seq].impression,FILLSTRING(1500,' ')),
	FILLSTRING(120,' '),' '),FILLSTRING(50,' '),' '),FILLSTRING(30,' '),' '),FILLSTRING(20,' '),' '),FILLSTRING(15,' '),' '),
	FILLSTRING(5,' '),' ')
