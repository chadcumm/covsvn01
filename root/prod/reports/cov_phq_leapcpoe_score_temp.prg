...
******************************************************************************
	...
	Solution:			Quality
	Request#:			12795
	Source file name:	      cov_phq_leapcpoe_score_temp.prg
	Object name:		cov_phq_leapcpoe_score_temp
	Program purpose:	      CPOE Leapfrog Scorecard
	Executing from:		DA2
 	Special Notes:		copy from cov_phq_leapcpoe_scorcard_det.prg
 					- exclude temp units by having nom and denom '0'
...
5/12/22      Geetha    CR# 12795    Initial release
 
******************************************************************************/
 
drop program cov_phq_leapcpoe_score_temp:DBA go
create program cov_phq_leapcpoe_score_temp:DBA
...
Head elh.encntr_id
	...
	cnt = 0
      ...
      while(idx > 0)
      	olist->list[idx].nunit = elh.loc_nurse_unit_cd;trim(uar_get_code_display(elh.loc_nurse_unit_cd))
		idx = locateval(cnt ,(idx+1) ,olist->rec_cnt ,elh.encntr_id ,olist->list[cnt].encntrid)
    	endwhile
with nocounter
 
;---------------------------------------------------------------------------------------------------------
;Overrite Nume and Denom for Temp units
...
join cv where cv.code_value = olist->list[d1.seq].nunit
	and cv.code_set = 220
...
Head cv.code_value
	idx = 0
	cnt = 0
      idx = locateval(cnt ,1 ,olist->rec_cnt ,cv.code_value ,olist->list[cnt].nunit)
      while(idx > 0)
      	olist->list[idx].nume = 0
      	olist->list[idx].denome = 0
		idx = locateval(cnt ,(idx+1) ,olist->rec_cnt ,cv.code_value ,olist->list[cnt].nunit)
    	endwhile
...
select into $outdev
	...
	, nurse_unit = trim(uar_get_code_display(olist->list[d1.seq].nunit))
