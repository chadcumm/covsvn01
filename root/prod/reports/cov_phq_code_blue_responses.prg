...
;-------------------------------------------------------------------------------------------------------
;Patient location at time of Cardio arrest
 /*
select into $outdev
...
join elh where elh.encntr_id = rapid->plist[d1.seq].encntrid
	and (cnvtdatetime(rapid->plist[d1.seq].events[d2.seq].cardio_arrest_dt) between elh.beg_effective_dt_tm and elh.end_effective_dt_tm)
	and elh.active_ind = 1
 
order by elh.encntr_id, eventid
 
Head eventid
	if(rapid->plist[d1.seq].events[d2.seq].pat_code_blue_location = ' ')
		rapid->plist[d1.seq].events[d2.seq].pat_code_blue_location = pat_loc
	endif
with nocounter
 */
...
order by facility, patient_name, fin, blue->plist[d1.seq].events[d2.seq].code_blue_called_dt
