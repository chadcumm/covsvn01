...
drop program ccummin4_cov1_sn_sched go
create program ccummin4_cov1_sn_sched
...
head  SURG_AREA
call echo(build2("|------HEAD SURG_AREA------|"))
	iF (AREA_CNT>0)
		Y_POS=5
		break
	ENDIF
...
HEAD CASE_NUMBER
call echo(build2("|------HEAD CASE_NUMBER------|"))
call echo(build2("CASE_NUMBER:",CASE_NUMBER))
call echo(build2("SCHED_OP:",SCHED_OP))
;Calculate estimated lines for case record
	...
	PROC_COUNT=0
	;002 stat=ALTERLIST(SG->PROC,0)
	 stat=ALTERLIST(CS->PROC,0) ;002
...
call echo(build2("|------Detail------|"))
	...
	ENDIF
	IF (PROC_TEMP="1")
		call echo(build2("!!!INSIDE PROC_TEMP=1!!!!!"))
		...
		ENDIF
		call echo(build2("before SURG_PROC_TEXT=",SURG_PROC_TEXT))
		IF (TEXTLEN(trim(proc_text,3))>1)
			...
			ENDIF
		ENDIF
		call echo(build2("after SURG_PROC_TEXT=",SURG_PROC_TEXT))
 
		call echo(build2("before surg_p_text=",surg_p_text))
		if (surg_p_text="")
			surg_p_text=procedure
		else
			surg_p_text=concat(surg_p_text,", ",procedure)
		endif
		call echo(build2("after surg_p_text=",surg_p_text))
	endif
    call echorecord(CS)
 
    call echo(build2("|------END detail------|"))
Foot CASE_NUMBER
call echo(build2("|------FOOT CASE_NUMBER------|"))
	call echo(build2("SURG_PROC_TEXT=",SURG_PROC_TEXT))
	call echo(build2("surg_p_text=",surg_p_text))
	...
	;PRINT PROCEDURE
	call echo(build("PRINTING PROCEDURE"))
	call echo(build2("checking if surg_proc_text is empty"))
	if (trim(surg_proc_text,3)="")
		call echo(build(^INSIDE surg_proc_text,3)=""^))
		surg_proc_text=surg_p_text
	else														;3230
		surg_proc_text=build2(surg_p_text,"; ",surg_proc_text)	;3230
	endif
	call echo(build2("FINAL SURG_PROC_TEXT=",SURG_PROC_TEXT))	;3230
...
;	CALL print (calcpos (96 ,(y_pos  ) ) ),MODIFIER
 ...
 ;PRINT SCHEDULE COMMENT
	...
	PROC_COUNT=0
foot Surg_Area
call echo(build2("|------Foot Surg_Area------|"))
	room_cnt=0
	date_cnt=0
foot page
	y_pos=730
