...
******************************************************************************
	...
	Solution:			Quality
	Source file name:	      cov_phq_bcma_leapfrog_scorecard.prg - original copy
	Object name:		cov_phq_bcma_lfrog_scorecard
	Request#:			4549
	Program purpose:	      BCMA & Leapfrog scorecard
	Executing from:		DA2
 	Special Notes:		temp copy to include exclusion list as of 06/03/20
...
drop program cov_phq_bcmafrog_scorecard_tmp:DBA go
create program cov_phq_bcmafrog_scorecard_tmp:DBA
;temp testing copy to include exclusion list as of 06/03/20
...
charge_num = trim(meds->list[d.seq].charge_number, 3), cat = meds->list[d.seq].catalogcd
, item = meds->list[d.seq].itemid, drug = meds->list[d.seq].order_mnemonic
...
;join cv1 where cv1.d bc->list[d.seq].nurse_unit
