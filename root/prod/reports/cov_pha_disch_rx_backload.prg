...
******************************************************************************
	...
	Solution:			Pharmacy
	Source file name:	      cov_pha_disch_rx_backload.prg
	Object name:		cov_pha_disch_rx_backload
	Request#:			Based on cov_pha_discharge_rx_meds.prg
	Program purpose:	      Prescribed medications at patient discharge
	Executing from:
 	Special Notes:          Backload (Astream - Jerry Inman)
...
drop program cov_pha_disch_rx_backload:DBA go
create program cov_pha_disch_rx_backload:DBA
...
declare filename_var = vc WITH constant('cer_temp:pha_rx_dec31_18.txt'), PROTECT
declare ccl_filepath_var = vc WITH constant('$cer_temp/pha_rx_dec31_18.txt'), PROTECT
 
;declare filename_var = vc WITH constant('cer_temp:pha_discharged_rx.txt'), PROTECT
;declare ccl_filepath_var = vc WITH constant('$cer_temp/pha_discharged_rx.txt'), PROTECT
   ...
   Select into value(filename_var)
	...
	Head d.seq
		...
		output_orders = build(output_orders
			...
			,wrap3(cnvtstring(rx->olist[d.seq].facility_cd))
			,wrap3(rx->olist[d.seq].federal_tax_id)
			,wrap3(rx->olist[d.seq].nurse_unit)
			,wrap3(rx->olist[d.seq].fin)
			,wrap3(rx->olist[d.seq].mrn)
			...
			,wrap3(rx->olist[d.seq].duration_unit)
