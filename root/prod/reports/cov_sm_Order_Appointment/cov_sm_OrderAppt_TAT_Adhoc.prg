...
******************************************************************************
	...
	Executing from:		CCL
 	...
 	Special Notes:		Prompts change the where statement to filter by:
						...
						*** This version is for special adhoc requests. ***
...
022	11/10/2020	Todd A. Blanchard		Added city and zipcode data.
023	03/07/2022	Todd A. Blanchard		Changed practice site display to org name.
 
******************************************************************************/
 
drop program cov_sm_OrderAppt_TAT_Adhoc:DBA go
create program cov_sm_OrderAppt_TAT_Adhoc:DBA
...
if (validate(request->batch_selection) = 1)
	if ($report_type = 0)
		...
		set end_datetime = cnvtlookahead("5,D", end_datetime) ;019			
	endif
else
	set start_datetime = cnvtdatetime($start_datetime)
...
select
	if ($report_type in (0, 1, 3))
		...
		from
				...
				and org.organization_id not in (
					...
					where 
						...
						and ps.organization_id not in (
							; acutes
							3144501.00, 675844.00, 3144505.00, 3144499.00, 3144502.00, 3144503.00, 3144504.00, 0.0
						)
				)
			...
			, (left join ORGANIZATION org_ps on org_ps.organization_id = ps.organization_id) ;023
...
;				and sea.order_status_meaning not in ("CANCELED", "DISCONTINUED") ;010
			...
			, (left join ORGANIZATION org_psoa on org_psoa.organization_id = ps_oa.organization_id) ;023
 
			, (inner join PERSON p on p.person_id = sa.person_id
				and p.person_id = 15824606.00 ; TODO: TEST
				)
...
detail
	...
	tat_data->list[cnt].city					= a.city ;022
	tat_data->list[cnt].zipcode					= a.zipcode ;022
	...
	tat_data->list[cnt].order_state				= uar_get_code_display(o.sch_state_cd) ; TODO: TEST
	
	;002
	tat_data->list[cnt].ordering_physician		= if (sed.oe_field_value > 0.0) ;015
	...
	tat_data->list[cnt].ord_phys_group			= if (ps.practice_site_id > 0.0) ;015
													trim(org_ps.org_name, 3) ;023
												  else
													trim(org_psoa.org_name, 3) ;023
												  endif
	...
	tat_data->list[cnt].prior_auth				= trim(od3.oe_field_display_value, 3) ;003
	...
	tat_data->list[cnt].auth_nbr				= trim(au.auth_nbr, 3) ;003
...
distinct into value(output_var)
	...
	, city							= tat_data->list[d1.seq].city ;022
	, zipcode						= tat_data->list[d1.seq].zipcode ;022
