...
******************************************************************************
 	...
 	----------	--------------------	--------------------------------------
001	07/09/2020	Todd A. Blanchard		Included initial sleep studies, etc.
 
******************************************************************************/
 
drop program cov_sm_OrderAppt_Analysis_TEST:DBA go
create program cov_sm_OrderAppt_Analysis_TEST:DBA
...
;			, (left join SCH_ENTRY sen on sen.sch_event_id = sev.sch_event_id)
			...
			, (left join SCH_EVENT_ATTACH sea on sea.sch_event_id = sev.sch_event_id ;001
				and sea.attach_type_cd = attachtype_order_var
				and sea.active_ind = 1)
 
			, (left join ORDERS o on o.order_id = sea.order_id ;001
			...
			, (left join ORDER_ACTION oa on oa.order_id = o.order_id ;001
				and oa.action_type_cd = order_var
				and oa.action_sequence > 0)
 
			, (left join PRSNL per_oa on per_oa.person_id = oa.order_provider_id) ;001
	...
	, ordering_physician = evaluate2(
 	...
 	, ord_phys_group = evaluate2(
		if (size(trim(ps.practice_site_display, 3)) > 0)
			trim(ps.practice_site_display, 3)
		else
			trim(ps_oa.practice_site_display, 3)
		endif
		)
	
	, appt_type = uar_get_code_display(sev.appt_type_cd) ;001
