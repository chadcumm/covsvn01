...
drop program cov_bh_treat_plan_op_mtp:dba go
create program cov_bh_treat_plan_op_mtp:dba
...
WITH outdev ,fac ,excel
 ...
 IF (( $EXCEL = 1 ) )
  CALL output (null )
 ELSE
 	EXECUTE cov_bh_treat_plan_op_lb  $OUTDEV
 ...
 SUBROUTINE  clinical_event (null )
  SELECT INTO "nl:"
   ...
   PLAN (ce
    ...
    AND (ce.event_cd = op_mtp_update_var)
...
call echorecord(treat_plan_rpt) 
