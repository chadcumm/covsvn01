...
function ccpsPMSearchSetup(_ctrlPMSearch, _btnPMSearch, _btnPMRemove, _PMSearchMode, _PMSelectMode, _PMCtrlDataset)
{
    ...
    ctrlPMResults.onClick = validateResultList;
    ctrlPMResults.onValidate = validateResultList;
    ctrlPMResults.onQueryValue = buildPromptString;
    btnPMSearch.onClick = PMSearchClick;
    btnPMRemove.onClick = PMRemoveClick;
    ...
    else  // pmSearch.pmEncounterMode
    {
        ...
        // Format is: <column header>, <internal name>, <visible>, <width>
        if (ctrlPMResultsDataset == null)
            ctrlPMResultsDataset = [["Encounter ID", "ENCOUNTER_ID",        false, 20],
                                    ["FIN",          "FIN NBR",             true,  20],
                                    ["Patient Name", "NAME_FULL_FORMATTED", true, 100]];
        ...
        pmSearch.EncounterMultiSelect = false;
        pmSearch.AllEncountersButton = false;
...
/**
* Create the parameter string that is sent to the script.
*/
function buildPromptString(sender)
{
    var recordCnt = ctrlPMResults.recordCount;
    var curRec = -1;
    var resultList = "";
 
 
    for (curRec = ctrlPMResults.getNextSelection(-1); curRec > -1; curRec = ctrlPMResults.getNextSelection(curRec))
    {
        if (resultList != "")
            resultList += ",";
 
        resultList += ctrlPMResults.getField(curRec, 0);
    }
