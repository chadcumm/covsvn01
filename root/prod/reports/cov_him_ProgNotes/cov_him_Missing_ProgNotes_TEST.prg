...
drop program cov_him_Missing_ProgNotes_TEST:dba go
create program cov_him_Missing_ProgNotes_TEST:dba
...
;
declare op_facility_var				= vc with noconstant(fillstring(100, " ")), private
...
head ce.encntr_id
	...
	if ((e.disch_dt_tm > cnvtdatetime(curdate, curtime)) or nullind(e.disch_dt_tm))
		prognotedata->qual[cnt].los_days		= datetimediff(cnvtdatetime(curdate, curtime), e.reg_dt_tm)
		prognotedata->qual[cnt].los_hours		= datetimediff(cnvtdatetime(curdate, curtime), e.reg_dt_tm, 3)
	else
