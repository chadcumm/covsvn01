...
drop program cov_sm_Provider_Audit_TEST:DBA go
create program cov_sm_Provider_Audit_TEST:DBA
...
declare exam_ordered_var			= f8 with constant(uar_get_code_by("DISPLAYKEY", 14192, "ORDERED"))
...
record ordappt_data (
	...
	1 list[*]
		...
		2 accession					= c20 ;002
		2 exam_status				= c40 ;002
		2 report_status				= c40 ;002
...
if ($report_type = 0)
	...
	from
		...
		, (left join ORDER_RADIOLOGY orad on orad.order_id = o.order_id)
...
;		)
		...
		and o.active_ind = 1
		and nullval(orad.exam_status_cd, 0.0) in (exam_ordered_var, 0.0) ;002
	...
	detail
		...
		ordappt_data->list[cnt].order_type				= uar_get_code_display(o.activity_type_cd) ;002
		...
		ordappt_data->list[cnt].accession				= cnvtacc(orad.accession)
		ordappt_data->list[cnt].exam_status				= uar_get_code_display(orad.exam_status_cd) ;002
		ordappt_data->list[cnt].report_status			= uar_get_code_display(orad.report_status_cd) ;002
...
if ($report_type = 0)
	select distinct into value($OUTDEV)
		...
		, order_type			= trim(ordappt_data->list[d1.seq].order_type, 3) ;002
		...
		, accession				= trim(ordappt_data->list[d1.seq].accession, 3) ;002
		, exam_status			= trim(ordappt_data->list[d1.seq].exam_status, 3) ;002
		, report_status			= trim(ordappt_data->list[d1.seq].report_status, 3) ;002
	...
	order by
		...
		, ordappt_data->list[d1.seq].order_received
		, ordappt_data->list[d1.seq].appt_date
		, ordappt_data->list[d1.seq].order_id
