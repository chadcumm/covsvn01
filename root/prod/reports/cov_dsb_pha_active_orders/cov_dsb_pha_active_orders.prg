...
**************************************************************************************************
	...
	Solution:
	Source file name:  	cov_dsb_pha_active_orders.prg
	Object name:		cov_dsb_pha_active_orders
	CR#:
 
	Program purpose:	Tranlated from Cerner's "dsb_pha_active_orders.prg"
	Executing from:		CCL
  	Special Notes:
...
*  	-----------  ----------  --------------------  -----------------------------------------------
*	001			 Feb 2022	 Dan Herren			   CR 12260
*
**************************************************************************************************/
 
DROP PROGRAM cov_dsb_pha_active_orders :dba GO
CREATE PROGRAM cov_dsb_pha_active_orders :dba
 
prompt
	"Output to File/Printer/MINE" = "MINE"                                                                ;* Enter or select the p
	, "Select your Facility:" = 0
	, "Display Active Orders Only?:" = 1
	, "Begin Date:" = "CURDATE"                                                                           ;* ONLY REQUIRED IF NOT
	, "End Date:" = "CURDATE"                                                                             ;* ONLY REQUIRED IF NOT
	;<<hidden>>"Enter product Description, Brand Name, CDM, or Med Id: (Then press TAB to Search)" = ""
...
DETAIL
 	faccnt = (faccnt + 1) ,
	IF ((faccnt > size(fac_req->facilities ,5)))
		stat = alterlist(fac_req->facilities ,(faccnt + 50))
	ENDIF ,
...
DETAIL
  	prdcnt = (prdcnt + 1) ,
  	IF ((prdcnt > size(results->products ,5)))
		stat = alterlist(results->products ,(prdcnt + 20))
 	ENDIF,
...
IF (( $ACTIVEORD = 1))
  	...
  	SELECT INTO "nl: "
   	...
   	PLAN (ed
    	WHERE (ed.end_effective_dt_tm BETWEEN cnvtdatetime((curdate - 1) ,curtime3) AND cnvtdatetime("31-DEC-2100"))
    		AND expand (facidx ,1 ,size(fac_req->facilities ,5) ,ed.loc_facility_cd ,fac_req->facilities[facidx ].facility_cd)
	...
	JOIN (od
    	WHERE (od.encntr_id = ed.encntr_id)
    		...
    		AND (od.profile_display_dt_tm BETWEEN cnvtdatetime(curdate ,curtime3) AND cnvtdatetime("31-DEC-2100")))
    JOIN (op
    ...
    JOIN (o
    	WHERE (op.order_id = o.order_id)
    		AND (o.order_status_cd IN (cdordered ,cdsuspended)))
    JOIN (ea1
    ...
    JOIN (pw
   	...
   	DETAIL
    	ordcnt = (ordcnt + 1) ,
    	IF ((ordcnt > size(results->products[mypos].orders ,5)))
			stat = alterlist(results->products[mypos].orders ,(ordcnt + 50))
    	ENDIF,
    	results->products[mypos].orders[ordcnt].facility_cd = e.loc_facility_cd ,
    	results->products[mypos].orders[ordcnt].facility_disp = uar_get_code_display(e.loc_facility_cd) ,
    	results->products[mypos].orders[ordcnt].nurse_unit = trim(uar_get_code_display(e.loc_nurse_unit_cd)) ,
    	...
    	results->products[mypos].orders[ordcnt].stop_dt_tm_disp = concat(format (o.projected_stop_dt_tm ,"mm/dd/yyyy HH:MM;;Q")
			,evaluate (o.stop_type_cd ,cdsoftstop ," (s)" , cdhardstop ," (h)" ,cdphysstop ," (p)" ,"")) ,
    	results->products[mypos].orders[ordcnt].order_phys_name = trim(pr1.name_full_formatted) ,
    	results->products[mypos].orders[ordcnt].order_phys_pos_disp = trim(uar_get_code_display(pr1.position_cd)) ,
    	results->products[mypos].orders[ordcnt].entered_by_name = trim(pr2.name_full_formatted) ,
    	results->products[mypos].orders[ordcnt].entered_by_pos_disp = trim(uar_get_code_display(pr2.position_cd)) ,
...
ELSE
	...
	FROM (ORDERS o),
   	...
   	PLAN (o
		WHERE (o.orig_order_dt_tm BETWEEN cnvtdatetime(concat($BEGINDT ," 00:00:00"))
			AND cnvtdatetime(concat($ENDDT ," 23:59:59")))
    		AND (o.product_id = 0)
	...
	JOIN (e
    	WHERE (e.encntr_id = o.encntr_id)
    		AND expand (facidx ,1 ,size(fac_req->facilities ,5)
				,e.loc_facility_cd ,fac_req->facilities[facidx ].facility_cd))
    JOIN (od
    ...
    JOIN (pw
   	...
   	DETAIL
    	...
    	results->products[mypos].orders[ordcnt].facility_disp = uar_get_code_display(elh.loc_facility_cd) ,
    	results->products[mypos].orders[ordcnt].nurse_unit = trim(uar_get_code_display(elh.loc_nurse_unit_cd)) ,
    	results->products[mypos].orders[ordcnt].room_bed = evaluate (elh.loc_room_cd ,0.0 ,"" ,
			build(uar_get_code_display(elh.loc_room_cd) ,"-" ,uar_get_code_display(elh.loc_bed_cd))) ,
    	results->products[mypos].orders[ordcnt].fin = trim(ea1.alias) ,
    	results->products[mypos].orders[ordcnt].discharge_dt_tm = e.disch_dt_tm ,
    	results->products[mypos].orders[ordcnt].patient_name = trim(p.name_full_formatted) ,
    	results->products[mypos].orders[ordcnt].order_id = od.order_id ,
...
;;begin 001
;==============================================================================
; GET ORDER INDICATION DATA
;==============================================================================
CALL echo(build("*** GET ORDER INDICATION DATA ***"))
 
SELECT INTO "nl:"
FROM ORDER_DETAIL od
	,(DUMMYT d1 WITH seq = size(results->products, 5))
...
JOIN od
	WHERE od.order_id = results->products[d1.seq].orders[d2.seq].order_id
;		AND od.oe_field_id = 2555037795 ;Pharmacy Anticoagulant Indications
		AND od.oe_field_id in (24861495, 2555037795, 29598149, 677336, 2552706741)
;;;		Pharmacy ABX Indications, Pharmacy Anticoagulant Indications, Pharmacy Bolus Indication
;;;		Pharmacy Compound Indication, Pharmacy Indication Freetext
 
 
ORDER BY d1.seq, d2.seq, od.order_id
 
DETAIL
	results->products[d1.seq].orders[d2.seq].order_indication = od.oe_field_display_value
...
SELECT INTO  $OUTDEV
	med_id 					= substring(1 ,20 ,results->products[d1.seq].pyxis_id) ,
  	...
  	start_dt_tm 			= format (results->products[d1.seq].orders[d2.seq].start_dt_tm ,"mm/dd/yyyy HH:MM;;Q") ,
  	...
  	ordering_phys_position	= substring(1 ,50 ,results->products[d1.seq].orders[d2.seq].order_phys_pos_disp) ,
  	entered_by 				= substring(1 ,100 ,results->products[d1.seq].orders[d2.seq].entered_by_name) ,
  	entered_by_position 	= substring(1 ,50 ,results->products[d1.seq].orders[d2.seq].entered_by_pos_disp) ,
  	ordered_dt_tm 			= format (results->products[d1.seq].orders[d2.seq].ordered_dt_tm ,"mm/dd/yyyy HH:MM;;Q") ,
  	order_status 			= substring(1 ,40 ,results->products[d1.seq].orders[d2.seq].order_status) ,
  	discharge_dt_tm 		= format (results->products[d1.seq].orders[d2.seq].discharge_dt_tm ,"mm/dd/yyyy HH:MM;;Q") ,
  	order_comment 			= trim(replace (replace (substring(1 ,200 ,results->products[d1.seq].orders[d2.seq].order_comments)
								,char (13) ,"   ") ,char (10) ,"   ")) ,
  	order_indication 		= substring(1 ,40 ,results->products[d1.seq].orders[d2.seq].order_indication) , ;001
