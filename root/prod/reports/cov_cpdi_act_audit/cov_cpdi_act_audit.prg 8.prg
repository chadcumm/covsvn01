...
detail
	...
	if ((mod(t_rec->cnt,1000) = 1) or (t_rec->cnt = 1))
 	...
 	;t_rec->qual[t_rec->cnt].patient						= 
 	t_rec->qual[t_rec->cnt].patient_name				= ctl.patient_name
 	;t_rec->qual[t_rec->cnt].performing_prsnl				= 
...
select into $OUTDEV
	...
	PERFORMING_PRSNL	= trim(t_rec->qual[d1.seq].performing_prsnl)
	PATIENT				= trim(t_rec->qual[d1.seq].patient)
	...
	BATCH_NAME			= trim(t_rec->qual[d1.seq].batch_name)
	C_CDI_QUEUE_DISP	= trim(t_rec->qual[d1.seq].c_cdi_queue_disp)
	CREATE_DT_TM		= trim(format(t_rec->qual[d1.seq].create_dt_tm,";;q"))
	DEVICE_NAME			= trim(t_rec->qual[d1.seq].device_name)
	DOCUMENT_TYPE_ALIAS	= trim(t_rec->qual[d1.seq].document_type_alias)
	DOC_TYPE			= trim(t_rec->qual[d1.seq].doc_type)
	FINANCIAL_NBR		= trim(t_rec->qual[d1.seq].financial_nbr)
	MRN					= trim(t_rec->qual[d1.seq].mrn)
	PAGE_CNT			= t_rec->qual[d1.seq].page_cnt
	PARENT_ENTITY_NAME	= trim(t_rec->qual[d1.seq].parent_entity_name)
	PATIENT_NAME		= trim(t_rec->qual[d1.seq].patient_name)
	C_REASON_DISP		= trim(t_rec->qual[d1.seq].c_reason_disp)
