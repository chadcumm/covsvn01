 ...
 ********************************************************************************************
    ...
    Soluation:         	Periopative / SurgiNet
    Source file name:	cov_peri_surg_scorecard_procs.prg
    Object name:       	cov_peri_surg_scorecard_procs
 	CR #:			   	517
 
    Program purpose:   	Creates extract for Surgery Procedures.
 
    				   	File goes to the AStream folder -
    				   	\\chstn_astream_prod.cernerasp.com\middle_fs\to_client_site\
						p0665\ClinicalNursing\Surgery\PAExports
 
    				   	Filename: surgprocdaily.txt
 
    Executing from:    	CCL.
 
    Special Notes:		This extract was formally "cov_ic_NHSHDenomProcedure"
...
*	---------	----------  ----------------    -------------------------------------------
*				07/30/2019	Dan Herren			Extract name & job change.
*												Jobname: "COV Surg Scorecard Procs Extract"
*	001			05/27/2020	Dan Herren			CR 7824
*	002			11/24/2020	Dan Herren			CR 8898
*
*********************************************************************************************/
 
drop program cov_peri_surg_scorecard_procs:dba go
create program cov_peri_surg_scorecard_procs:dba
...
;---CASE
	...
	2 casequal[*]
		...
		3 gen_anesth_type    = c30   ;general anesthesia type ;002
 		3 asablob			 = c2
...
declare MHAS_DOC_TYPE_VAR = f8 with constant(uar_get_code_by("DISPLAY_KEY",14258,"MHASINTRAOPRECORDOR")),protect   ;2562863611
...
;
/**************************************************************
; SET DATA-RANGE VARIABLES FOR THE PREVIOUS 14 DAYS
**************************************************************/
declare START_DATE = f8
declare END_DATE   = f8
 
;; 14 DAYS BACK FROM CURRENT DATE
...
;-----------------------------
;set START_DATE = CNVTDATETIME("24-NOV-2020 00:00:00")
;set END_DATE   = CNVTDATETIME("24-NOV-2020 23:59:59")
 
 
/**************************************************************
; SET DATA-FILE VARIABLES
**************************************************************/
;declare filename_var	= vc with constant(build("djhtestfile", ".txt"))
declare filename_var	= vc with constant(build("surgprocdaily", ".txt"))
...
;---CASE
	...
	,gen_anesth_type	= uar_get_code_display(scp.anesth_type_cd) ;002
...
;		and e.loc_facility_cd = 2553765467
		and	e.loc_facility_cd in (
			2553765291.00,  2552503657.00, 2552503635.00, 21250403.00, 2552503653.00,
			2552503639.00, 2552503613.00, 2552503645.00, 2552503649.00, 2553765467.00,
			2553765571.00, 2553765627.00)
		and e.active_ind = 1
...
;---CASE
	...
	enc->list[cnt].casequal[dcnt].asa_class	         = if(asa_class = "See Comments") "" else asa_class endif
	enc->list[cnt].casequal[dcnt].asa_class_anes     = asa_class_anes
	enc->list[cnt].casequal[dcnt].gen_anesth	     = gen_anesth
	enc->list[cnt].casequal[dcnt].gen_anesth_type    = gen_anesth_type ;002
	enc->list[cnt].casequal[dcnt].surg_case_nbr      = surg_case_nbr
...
join ce
	where ce.person_id = enc->list[dt.seq].person_id
		...
		and ce.performed_dt_tm between CNVTDATETIME(START_DATE)
...
head report
	blobin = fillstring(40000,' ')
	blobsize = 0
	blobout = fillstring(40000,' ')
	bsize = 0
	blobnortf = fillstring(40000,' ')
	rval = 0
	blobres = fillstring(40000,' ')
	asapos = 0
	casenbrpos = 0
	endcasepos = 0
	casenbr = fillstring(25,' ')
...
detail
	...
	if (cb.compression_cd = compcd)
		rval = UAR_OCF_UNCOMPRESS(blobin,40000,blobout,40000, 0)
 	...
 	;FIND CASE NBR VALUE
 	casenbrpos = findstring("Case Number:",blobres)
 
 	if (casenbrpos > 0)
 		endcasepos = findstring("/",blobres,casenbrpos)
 
 		if (endcasepos  > 0)
 			casenbr = substring(casenbrpos + 13,(endcasepos-1) - (casenbrpos + 13),blobres)
 		endif
 	endif
 
	;FIND ASA STRING VALUE
	asapos = findstring("ASA Classification:",blobres)
	...
	if (asapos > 0)
 	...
 	for (cnt = 1 TO enc->list[dt.seq].casecnt)
 		if (enc->list[dt.seq].casequal[cnt].surg_case_nbr = casenbr)
 			enc->list[dt.seq].casequal[cnt].asablob = asablob
 		endif
 	endfor
...
join pd
	where pd.surg_case_id = scp.surg_case_id
		...
		and pd.doc_type_cd in (
			FLMC_DOC_TYPE_VAR, FSR_DOC_TYPE_VAR, LCMC_DOC_TYPE_VAR, MHAS_DOC_TYPE_VAR,
			MHHS_DOC_TYPE_VAR, MMC_DOC_TYPE_VAR, PWMC_DOC_TYPE_VAR, RMC_DOC_TYPE_VAR
			)
...
;	;----PERSON
		...
		,cmrn        	    = trim(substring(1,20,enc->list[dt.seq].cmrn),3)
		,mrn         	    = trim(substring(1,20,enc->list[dt.seq].mrn),3)
	...
	;	,fac_abbr    	    = enc->list[dt.seq].fac_abbr
		,fac_unit    	    = trim(substring(1,50,enc->list[dt.seq].fac_unit),3)
...
;		,asa_class		    = trim(enc->list[dt.seq].asa_class,3)
		,asa_class          = if (enc->list[dt.seq].casequal[d2.seq].asa_class > ' ')
							  trim(enc->list[dt.seq].casequal[d2.seq].asa_class,3)
							  elseif(enc->list[dt.seq].casequal[d2.seq].asa_class_anes > ' ')
							  trim(enc->list[dt.seq].casequal[d2.seq].asa_class_anes,3)
							  elseif(enc->list[dt.seq].casequal[d2.seq].asablob > ' ')
							  trim(enc->list[dt.seq].casequal[d2.seq].asablob,3)
							  endif
		...
		,gen_anesth_type    = trim(enc->list[dt.seq].casequal[d2.seq].gen_anesth_type,3) ;002
