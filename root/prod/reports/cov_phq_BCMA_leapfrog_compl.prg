...
----------	-------------------------------------------------------------------------------------------------------
05/29/19    Geetha       CR#5017  - Include all depts/organizations and sort order to personnel, Medication Name
...
07/09/20    Geetha       CR#8075  - Add override reason (only to Patient level report)
08/09/21    Geetha       CR#10608 - Prsnl prompt missing Rad Net positions
...
drop program cov_phq_bcma_leapfrog_compl:DBA go
create program cov_phq_bcma_leapfrog_compl:DBA
...
join od where od.order_id = bcma->plist[d.seq].orderid
...
Head od.order_id
	idx = 0
	cnt = 0
      idx = locateval(cnt ,1 ,size(bcma->plist,5) ,od.order_id ,bcma->plist[cnt].orderid)
      if(idx > 0)
		bcma->plist[idx].home_med = 'Yes'
	endif	
With nocounter
...
select into $outdev
	...
	, pat_own_home_med = trim(substring(1, 5, bcma->plist[d1.seq].home_med))
