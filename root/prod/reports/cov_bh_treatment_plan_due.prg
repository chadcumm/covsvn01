...
******************************************************************************
	...
	Source file name:	      cov_bh_treatment_plan_due.prg
	Object name:		cov_bh_treatment_plan_due
	Request#:			6723
	Program purpose:	      Track Op Counseler and OP Outreach Speciallist documentation
	Executing from:		DA2
...
drop program cov_bh_treatment_plan_due:dba go
create program cov_bh_treatment_plan_due:dba
...
; DECLARE op_mtp_update_var = f8 WITH constant (uar_get_code_by ("DISPLAY" ,72 ,"BH OP Cons MTP Update Date" ) ) ,protect
 
 ;2861906885.00	BH OS MTP Update Date
 ;2861906897.00	BH OP Cons MTP Update Date
 ...
 SUBROUTINE  main_query (null )
  SELECT INTO "nl:"
   FROM (encounter e ),
    ...
    (encntr_alias ea )
   PLAN (e
    ;WHERE (e.reg_dt_tm BETWEEN cnvtdatetime (begin_dt_tm ) AND cnvtdatetime (end_dt_tm ) )
