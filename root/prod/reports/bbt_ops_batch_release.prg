/***********************************************************
Author 			:	Cerner
Date Written	:	08/13/2018
Program Title	:	BB Ops CrossMatch Release
Source File		:	bbt_ops_batch_release.prg
Object Name		:	bbt_ops_batch_release
Directory		:	cust_script
DVD Version		:	2017.07.1.81
HNA Version		:	2015.01
CCL Version		:	8.8.3
Purpose			: 	This program is the standard version released 
					by Cerner. This should only be needed if the
					custom version has issues and we need to roll
...
----    ----------- ----------------------- ---------------------------
001		08/13/2018	Cerner    				Original Release
...
CREATE PROGRAM bbt_ops_batch_release :dba
 ...
 SELECT INTO cpm_cfn_info->file_name_logical
  ...
  HEAD pe.product_event_id
   ...
   IF ((pos > 0 ) )
    ...
    ELSE status = "Not Released" ,
     ...
     ENDIF
    ENDIF
    ,col 58 ,prod_num ,col 84 ,prod_disp ,col 97 ,status ,col 113 ,ops_request->productlist[d_ar.seq
    ].productevent[pos ].xm_exp_dt_tm "@DATETIMECONDENSED;;d" ,row + 1 ,formatted_acc = cnvtacc (ord
     .accession ) ,col 58 ,formatted_acc ,col 97 ,reason ,row + 1
   ENDIF
  DETAIL
   row + 0
  FOOT PAGE
  ...
  WITH nocounter ,outerjoin (d1 ) ,outerjoin (d2 ) ,outerjoin (d_pra ) ,dontcare (pra ) ,outerjoin (
    d_ord ) ,maxrow = 63 ,compress ,nolandscape ,nullreport ,expand = 1
 ;end select
