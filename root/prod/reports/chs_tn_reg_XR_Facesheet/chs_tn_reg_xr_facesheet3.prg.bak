...
Source File		:	chs_tn_reg_xr_facesheet3.prg
Object Name		:	chs_tn_reg_xr_facesheet3
Directory		:	cust_script
DVD Version		:
HNA Version		:
CCL Version		:
Purpose			: 	This program is a translated copy of custom script
					chs_tn_reg_xr_facesheet2.
 
					CR 4063 requested that SSN be unmasked.
...
Special Notes	:
Usage			:	chs_tn_reg_xr_facesheet3 go
Mod		Date		Engineer				Comment
----    ----------- ----------------------- ---------------------------
001		03/19/2019	Todd A. Blanchard		CR 4063
...
DROP PROGRAM chs_tn_reg_xr_facesheet3:DBA GO
CREATE PROGRAM chs_tn_reg_xr_facesheet3:DBA
 prompt
	"Output to File/Printer/MINE" = "MINE"   ;* Enter or select the printer or file name to send this report to.
	, "Person ID" = "0.0"                    ;* Please enter the person ID for which data should be retrieved
	, "Encounter ID" = "0.0"                 ;* Please enter the encounter ID for which data shall be retrieved
	, "Scheduling Event ID" = "0.0"          ;* Please enter the scheduling event ID for which data shall be retrieved
	, "Scheduling ID" = "0.0"                ;* Please enter the schedule ID for which data shall be retrieved
...
; /**************************************************************/
; ;001
; DECLARE ssn_var = f8 WITH noconstant(uar_get_code_by("DISPLAYKEY", 4, "SSN")) ,protect
; 
; ; overwrite record structure with unmasked ssn data
; select into "NL:"
; from
; 	PERSON_ALIAS pa
; 
; where
;	pa.person_id = post_doc_rec->person_id
;	and pa.person_alias_type_cd = ssn_var
;	and pa.active_ind = 1
; 
; detail
; 	post_doc_rec->patient_ssn = cnvtalias(pa.alias, pa.alias_pool_cd, 4)
; 
; with nocounter
...
;      post_doc_rec->patient_birth_dt_tm ) ) ,char (0 ) ) ) ,protect
 ...
 SUBROUTINE  initializereport (dummy )
  ...
  SET rptreport->m_reportname = "chs_tn_reg_xr_facesheet3"
