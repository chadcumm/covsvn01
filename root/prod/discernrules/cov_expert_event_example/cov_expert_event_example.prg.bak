...
select into "nl:"
from encounter enc,encntr_alias ea
;plan enc where enc.disch_dt_tm between cnvtdatetime(curdate-3, 0) and cnvtdatetime(curdate ,2359)
plan ea where ea.alias = $1 join enc where enc.encntr_id = ea.encntr_id
order by enc.encntr_id

head report
cnt = 0
EKSOPSRequest->expert_trigger = cnvtupper(curprog) 
EKSOPSRequest->expert_trigger = cnvtupper("CCUMMIN4_TEST_PAGE")
head enc.encntr_id
cnt = cnt + 1
