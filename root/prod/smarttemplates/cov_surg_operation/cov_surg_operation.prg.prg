...
;001 ccummin4 - removed time restriction
 ...
 SUBROUTINE  PUBLIC::get_surg_opnote_operation (null )
  ...
  ;002   AND (scp.active_ind = 1 ) )
   ...
   DETAIL
	...
	sn->operation[operation_cnt ].surg_start_dt_tm = trim(format(surg->cases[d1.seq ].surg_start_dt_tm,"mm/dd/yyyy;;q"))
   WITH nocounter
  ;end select
 ...
 ; set surg_proc_cd_desc = concat(surg_proc_cd_desc, " (",trim(surg_start_dt_tm),") ")
  ...
  ELSE
   RETURN (concat(surg_proc_cd_desc, " (",trim(surg_start_dt_tm),") " ))
  ENDIF
 END ;Subroutine
#exit_script
