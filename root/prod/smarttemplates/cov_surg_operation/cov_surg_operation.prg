...
--- 	----------	--------------------	----------------------------------
000 	03/01/2019  Chad Cummings
001		04/17/2019  Chad Cummings			Removed time restriction
002		05/31/2019	Chad Cummings			Allowed multiple procedures per case
003		05/31/2019	Chad Cummings			Added date to end of procedure string
******************************************************************************/
...
;001 ccummin4 - removed time restriction
 ...
 DECLARE PUBLIC::build_procedure_line ((surg_proc_cd_description = vc ) ,(surg_proc_detail = vc ) ,(
  modifier = vc ),(surg_start_dt_tm = vc) ) = vc WITH protect ,copy
 ...
 SUBROUTINE  PUBLIC::get_surg_opnote_operation (null )
  ...
  ;002 SELECT INTO "nl:"
  ;002  FROM (surg_case_procedure scp )
  ;002  PLAN (scp
  ;002   WHERE (scp.surg_case_id = surg->cases[1 ].surg_case_id )
  ;002   AND (scp.active_ind = 1 ) )
    
   ;002 start
   select into "nl:"
   from 
   	 surg_case_procedure scp
   	,(dummyt d1 with seq=size(surg->cases,5))
   plan d1
   join scp
   	where scp.surg_case_id = surg->cases[d1.seq ].surg_case_id
   ...
   DETAIL
    ...
    ;002 sn->operation[operation_cnt ].surg_start_dt_tm = trim(format(surg->cases[1 ].surg_start_dt_tm,"mm/dd/yyyy;;q"))
	sn->operation[operation_cnt ].surg_start_dt_tm = trim(format(surg->cases[d1.seq ].surg_start_dt_tm,"mm/dd/yyyy;;q"));002
   WITH nocounter
  ;end select
 ...
 SUBROUTINE  PUBLIC::format_output (operation_cnt )
  ...
  ENDFOR
 END ;Subroutine
 SUBROUTINE  build_procedure_line (surg_proc_cd_desc ,surg_proc_detail ,modifier, surg_start_dt_tm )
 ...
 ; set surg_proc_cd_desc = concat(surg_proc_cd_desc, " (",trim(surg_start_dt_tm),") ")

  IF ((surg_proc_cd_desc > " " ) AND (surg_proc_detail > " " ) AND (modifier > " " ) )
   RETURN (concat (surg_proc_cd_desc ,", " ,surg_proc_detail ,", " ,modifier, " (",trim(surg_start_dt_tm),") " ) )
  ELSEIF ((surg_proc_cd_desc > " " ) AND (surg_proc_detail > " " ) )
   RETURN (concat (surg_proc_cd_desc ,", " ,surg_proc_detail, " (",trim(surg_start_dt_tm),") " ) )
  ELSEIF ((surg_proc_cd_desc > " " ) AND (modifier > " " ) )
   RETURN (concat (surg_proc_cd_desc ,", " ,modifier, " (",trim(surg_start_dt_tm),") " ) )
  ELSE
  ;003 RETURN (surg_proc_cd_desc)
   RETURN (concat(surg_proc_cd_desc, " (",trim(surg_start_dt_tm),") " ));003
  ENDIF
 END ;Subroutine
#exit_script
