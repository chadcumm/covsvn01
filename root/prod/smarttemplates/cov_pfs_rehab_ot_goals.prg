...
******************************************************************************
	...
	Source file name:		cov_pfs_rehab_ot_goals.prg
	Object name:		cov_pfs_rehab_ot_goals
 
	Request#:			951(part of)
	Program purpose:		Alteration/Customization on Cerner source code as per Covenants need
 
	Executing from:		Smart template - OT Goals/PowerForm
 
 	Special Notes:      	Original Cerner Source : pfs_rehab_OT_goals
...
DROP PROGRAM cov_pfs_rehab_ot_goals :dba GO
CREATE PROGRAM cov_pfs_rehab_ot_goals :dba
 ...
 ;----- CR 951 update
 DECLARE ote_g1_goal   = f8 WITH CONSTANT(UAR_GET_CODE_BY('DISPLAYKEY', 72,'OTTHERAPEUTICEXERCISEGOAL1')), PROTECT
 DECLARE ote_g1_astlvl = f8 WITH CONSTANT(UAR_GET_CODE_BY('DISPLAYKEY', 72,'OTTHERAPEUTICEXERCISEGOALASSIST1')), PROTECT
 DECLARE ote_g1_reps   = f8 WITH CONSTANT(UAR_GET_CODE_BY('DISPLAYKEY', 72,'OTTHERAPEUTICEXERCISEGOALREPS1')), PROTECT
 DECLARE ote_g1_sets   = f8 WITH CONSTANT(UAR_GET_CODE_BY('DISPLAYKEY', 72,'OTTHERAPEUTICEXERCISEGOALSETS1')), PROTECT
 DECLARE ote_g1_tmfrm  = f8 WITH CONSTANT(UAR_GET_CODE_BY('DISPLAYKEY', 72,'OTTHERAPEUTICEXERCISEGOALTIMEFRAME1')), PROTECT
 DECLARE ote_g1_stat   = f8 WITH CONSTANT(UAR_GET_CODE_BY('DISPLAYKEY', 72,'OTTHERAPEUTICEXERCISEGOALSTATUS1')), PROTECT
 DECLARE ote_g1_dtmet  = f8 WITH CONSTANT(UAR_GET_CODE_BY('DISPLAYKEY', 72,'OTTHERAPEUTICEXERCISEGOALDATEMET1')), PROTECT
 DECLARE ote_g1_cmt    = f8 WITH CONSTANT(UAR_GET_CODE_BY('DISPLAYKEY', 72,'OTTHERAPEUTICEXERCISEGOALCOMMENT1')), PROTECT
;------------
  ...
  DETAIL
   IF ((first_dttm = "Y" ) )
    ...
    IF ((skip_flag = "N" ) )
     ...
     ENDIF
     , ;----------
     IF ((ce.event_cd IN (ote_g1_goal, ote_g1_astlvl, ote_g1_reps, ote_g1_sets, ote_g1_tmfrm, ote_g1_stat, ote_g1_dtmet, ote_g1_cmt) ) )
		CASE (ce.event_cd )
			OF ote_g1_goal :
		        IF ((ce.result_status_cd IN (altered, modified ) ) )
        			out->ote[1].goal = concat (trim (ce.result_val ) ," (modified)" )
        		ELSE
        			out->ote[1].goal = trim (ce.result_val )
        		ENDIF
			OF ote_g1_astlvl :
		        IF ((ce.result_status_cd IN (altered, modified ) ) )
        			out->ote[1].astlvl = concat (trim (ce.result_val ) ," (modified)" )
        		ELSE
        			out->ote[1].astlvl = trim (ce.result_val )
        		ENDIF
			OF ote_g1_reps :
		        IF ((ce.result_status_cd IN (altered, modified ) ) )
        			out->ote[1].reps = concat (trim (ce.result_val ) ," (modified)" )
        		ELSE
        			out->ote[1].reps = trim (ce.result_val )
        		ENDIF
			OF ote_g1_sets :
		        IF ((ce.result_status_cd IN (altered, modified ) ) )
        			out->ote[1].sets = concat (trim (ce.result_val ) ," (modified)" )
        		ELSE
        			out->ote[1].sets = trim (ce.result_val )
        		ENDIF
			OF ote_g1_tmfrm :
		        IF ((ce.result_status_cd IN (altered, modified ) ) )
        			out->ote[1].tmfrm = concat (trim (ce.result_val ) ," (modified)" )
        		ELSE
        			out->ote[1].tmfrm = trim (ce.result_val )
        		ENDIF
			OF ote_g1_stat :
		        IF ((ce.result_status_cd IN (altered, modified ) ) )
        			out->ote[1].stat = concat (trim (ce.result_val ) ," (modified)" )
        		ELSE
        			out->ote[1].stat = trim (ce.result_val )
        		ENDIF
			OF ote_g1_dtmet :
		        IF ((ce.result_status_cd IN (altered, modified ) ) )
        			out->ote[1].dtmet = concat (trim (ce.result_val ) ," (modified)" )
        		ELSE
        			out->ote[1].dtmet = trim (ce.result_val )
        		ENDIF
			OF ote_g1_cmt :
		        IF ((ce.result_status_cd IN (altered, modified ) ) )
        			out->ote[1].cmt = concat (trim (ce.result_val ) ," (modified)" )
        		ELSE
        			out->ote[1].cmt = trim (ce.result_val )
        		ENDIF
		ENDCASE
     ENDIF ;-------------
 ...
 ;----------951
  FOR (a = 1 TO 1 )
	  IF ((((out->ote[a].goal > "" ) ) OR ((((out->ote[a].astlvl > "" ) ) OR ((((out->ote[a].reps > "" ))
	  	OR ((((out->ote[a].sets > "" )) OR ((((out->ote[a].tmfrm > "" )) OR ((((out->ote[a].stat > "" ))
	  	OR ((((out->ote[a].dtmet > "" )) OR ((out->ote[a].cmt > "" ) )) )) )) )) )) )) )) )
 
		   SET first = "Y"
		   SET reply->text = concat (reply->text ,wb ,"Therap Ex Goal #" ,cnvtstring (a ,1 ,0 ,l ) ,":" )
		   IF ((out->ote[a].goal > "" ) )
			    SET reply->text = concat (reply->text ," " ,wr ,out->ote[a].goal )
			    SET first = "N"
		   ENDIF
		   IF ((out->ote[a ].astlvl > "" ) )
			    IF ((first = "Y" ) )
		     		SET reply->text = concat (reply->text ," " ,wr ,out->ote[a].astlvl )
		     		SET first = "N"
		    	ELSE
		     		SET reply->text = concat (reply->text ,"; " ,out->ote[a].astlvl )
			    ENDIF
		   ENDIF
		   IF ((out->ote[a].reps > "" ) )
			    IF ((first = "Y" ) )
		     		SET reply->text = concat (reply->text ," " ,wr ,out->ote[a].reps )
		     		SET first = "N"
		    	ELSE
		     		SET reply->text = concat (reply->text ,"; " ,out->ote[a].reps )
			    ENDIF
		    ENDIF
		   IF ((out->ote[a].sets > "" ) )
			    IF ((first = "Y" ) )
		     		SET reply->text = concat (reply->text ," " ,wr ,out->ote[a].sets )
		     		SET first = "N"
		    	ELSE
		     		SET reply->text = concat (reply->text ,"; " ,out->ote[a].sets )
			    ENDIF
		    ENDIF
		   IF ((out->ote[a ].tmfrm > "" ) )
			    IF ((first = "Y" ) )
		     		SET reply->text = concat (reply->text ," " ,wr ,out->ote[a].tmfrm )
		     		SET first = "N"
		    	ELSE
		     		SET reply->text = concat (reply->text ,"; " ,out->ote[a].tmfrm )
			    ENDIF
		    ENDIF
		   IF ((out->ote[a ].stat > "" ) )
			    IF ((first = "Y" ) )
		     		SET reply->text = concat (reply->text ," " ,wr ,out->ote[a].stat )
		     		SET first = "N"
		    	ELSE
		     		SET reply->text = concat (reply->text ,"; " ,out->ote[a].stat )
			    ENDIF
		    ENDIF
		   IF ((out->ote[a ].dtmet > "" ) )
			    IF ((first = "Y" ) )
		     		SET reply->text = concat (reply->text ," " ,wr ,out->ote[a].dtmet )
		     		SET first = "N"
		    	ELSE
		     		SET reply->text = concat (reply->text ,"; " ,out->ote[a].dtmet )
			    ENDIF
		    ENDIF
		   IF ((out->ote[a ].cmt > "" ) )
			    IF ((first = "Y" ) )
		     		SET reply->text = concat (reply->text ," " ,wr ,out->ote[a].cmt )
		     		SET first = "N"
		    	ELSE
		     		SET reply->text = concat (reply->text ,"; " ,out->ote[a].cmt )
			    ENDIF
		    ENDIF
		   SET reply->text = concat (reply->text ,reol )
	  ENDIF
