...
******************************************************************************
	...
	Source file name:	      cov_pfs_pt_plan.prg
	Object name:		cov_pfs_pt_plan
 
	Request#:			951(part of)
	Program purpose:		Alteration/Customization on Cerner source code as per Covenants need
 
	Executing from:		Smart template - Plan Review/PowerForm
 
 	Special Notes:      	Original Cerner Source : pfs_PT_plan
...
DROP PROGRAM cov_pfs_pt_plan :dba GO
CREATE PROGRAM cov_pfs_pt_plan :dba
 ...
 ;---CR 951 update
 DECLARE frequency_acute  = f8 WITH public ,constant (uar_get_code_by("DISPLAY", 72, 'PT Frequency Rehab Acute'))
 DECLARE duration_acute   = f8 WITH public ,constant (uar_get_code_by("DISPLAY", 72, 'PT Duration Acute') )
 DECLARE treatment_acute  = f8 WITH public ,constant (uar_get_code_by("DISPLAY", 72, 'PT Anticipated Treatments, Needs Acute') )
 ;-------------------
 ...
 SELECT INTO "nl:"
  ...
  ce_event_cd =
  IF ((ce.event_cd IN (frequency ,  frequency_new, frequency_acute ) ) ) freq
	  ELSEIF ((ce.event_cd IN (duration ,  duration_new, duration_acute ) ) ) dur
	  ELSEIF ((ce.event_cd IN (treatment ,  treatment_new, treatment_acute ) ) ) treat
  ENDIF
...
#exit_script
 ...
 SET script_version = "010 04/12/2018 LB015443"
END GO
