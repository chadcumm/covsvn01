...
drop program cov1_sn_sched_ops go
create program cov1_sn_sched_ops
...
head  SURG_AREA
	...
	ENDIF
	AREA_CNT=AREA_CNT+1
	stat = alterlist(totals->list,area_cnt)
	totals->list[area_cnt].area_cnt = 0
	totals->list[area_cnt].area_name = surg_area
	SCHED_OP_cnt = 0
head Surg_date
	if (date_cnt>0)
		Y_POS=5
...
head SCHED_OP
	SCHED_OP_cnt = (SCHED_OP_cnt + 1)
	stat = alterlist(totals->list[area_cnt].room,SCHED_OP_cnt)
	totals->list[area_cnt].room[SCHED_OP_cnt].room_cnt = 0
	totals->list[area_cnt].room[SCHED_OP_cnt].room_name = SCHED_OP
HEAD CASE_NUMBER
...
;	CALL print (calcpos (96 ,(y_pos  ) ) ),MODIFIER
 ...
 ;PRINT SCHEDULE COMMENT
	...
	stat=ALTERLIST(CS->PROC     ,0)
	PROC_COUNT=0
	totals->list[area_cnt].area_cnt = (totals->list[area_cnt].area_cnt + 1)
	totals->list[area_cnt].room[SCHED_OP_cnt].room_cnt = (totals->list[area_cnt].room[SCHED_OP_cnt].room_cnt + 1)
foot Surg_Area
	room_cnt=0
	date_cnt=0
	tot = cnvtstring(totals->list[area_cnt].area_cnt)
    CALL print (calcpos (20 ,(y_pos ) ) ) ,SURG_AREA, " TOTAL CASES:",tot
    row+1,
	for (i = 1 to size(totals->list[area_cnt].room,5))
		Y_POS=Y_POS+10,
		row+1,
		room_tot = cnvtstring(totals->list[area_cnt].room[i].room_cnt)
		room_total = concat(trim(totals->list[area_cnt].room[i].room_name)," CASE COUNT: ",trim(room_tot))
    	CALL print (calcpos (30 ,(y_pos ) ) ) ,room_total
	endfor
foot page
	y_pos=730
 	y_val= 792- y_pos
