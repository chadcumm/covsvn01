...
5.0         10/22/2022  Mark Maples         Added patient first name, last name, dob
**************************************************************/
...
drop program 1chs_tn_foreign_system_launch:dba go
create program 1chs_tn_foreign_system_launch:dba
...
prompt
	...
	, "User Id:" = 0.0
	, "System URL:" =
		"http://covhppmodules.covhlth.net/ColdFusionApplications/Cerner_ForeignSystemLaunch_iFrame/Default.cfm"
 
with OUTDEV, personId, encntrId, userId, launchURL
...
set record_data->person_id = CNVTREAL($personId)
set record_data->encntr_id = CNVTREAL($encntrId)
set record_data->user_id = CNVTREAL($userId)
set record_data->launch_url = BUILD($launchURL)
if (record_data->person_id = 0.0 or record_data->encntr_id = 0.0 or record_data->user_id = 0.0 or record_data->launch_url = "")
...
subroutine GetPatientData( dummy )
	...
	join ea
		...
		and ea.alias_pool_cd in (STAR_FIN_CD, CERNER_FIN_CD) ;v2.0
		and ea.active_ind = 1
	join oa ;v3.0 - Include the join to return the facility specific MRN.
		...
		or oa.alias_pool_cd = ea.alias_pool_cd) ;v3.0
	head report
		record_data->organization_id = e.organization_id
		record_data->encntr_type = build2(uar_get_code_display(e.encntr_type_cd)) ;v4.0
	detail
		record_data->patient_first = p.name_first
		record_data->patient_last = p.name_last
		record_data->patient_dob = FORMAT(p.birth_dt_tm, "MM/DD/YYYY")
 
		if (ea.alias_pool_cd = STAR_FIN_CD)
			record_data->fin = ea.alias
		elseif (ea.alias_pool_cd = CERNER_FIN_CD)  ;v2.0
			record_data->fin = ea.alias  ;v2.0
		endif
		if (pa.alias_pool_cd = SSN_CD)
			record_data->ssn = pa.alias
		elseif (pa.alias_pool_cd = CMRN_CD)
			record_data->cmrn = pa.alias
		elseif (pa.alias_pool_cd = NEXT_GEN_CD)
			record_data->next_gen_id = pa.alias
		elseif (findstring(STAR_MRN_STR,UAR_GET_DISPLAYKEY(pa.alias_pool_cd),1,0))
			record_data->mrn = pa.alias
		elseif (pa.alias_pool_cd = 263_MRN_CERN) ;v3.0
			record_data->mrn = pa.alias ;v3.0
		endif
	with nocounter
	if (not CURQUAL)
		set record_data->status_data.subeventstatus[1].operationname = "select"
...
subroutine GetHTML( dummy )
	...
	if (record_data->status_data.status = "F")
		...
		^>^,CHAR(10),CHAR(13),
		  ...
		  ^<INPUT TYPE="hidden" NAME="strPatientNameFirst" VALUE="^,record_data->patient_first,^">^,CHAR(10),CHAR(13),
		  ^<INPUT TYPE="hidden" NAME="strPatientNameLast" VALUE="^,record_data->patient_last,^">^,CHAR(10),CHAR(13),
		  ^<INPUT TYPE="hidden" NAME="strPatientDOB" VALUE="^,record_data->patient_dob,^">^,CHAR(10),CHAR(13),
