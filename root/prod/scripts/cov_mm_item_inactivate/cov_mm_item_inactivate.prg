...
******************************************************************************
   ...
   Program:             cov_mm_item_inactivate
   Folder:              CUST_SCRIPT
   Owner:               Covenant
   ...
   Purpose: Program to inactivate items from a file from Supply Chain
...
******************************************************************************
 ...
 ---- ----------   -------------------	---------------------------------------
 0001 03/01/2021   Dawn Greer, DBA      Pull the file from ASTREAM
******************************************************************************/
...
prompt
	...
	, "Filename" = "cov_mm_items.csv"     ;* Name of file in CCLUSERDIR
	, "Test Mode [ (N)o, (Y)es - Default ]" = "Y"
 
with OUTDEV, FILENAME, TEST_MODE
 
/* Declared Variables */
DECLARE sourcepath = vc    ;0001
DECLARE sourcefile = vc	   ;0001
DECLARE destpath = vc WITH noconstant("/cerner/d_p0665/ccluserdir/")	;0001
DECLARE destfile = vc	;0001
DECLARE cmd	= vc WITH noconstant("")	;0001
DECLARE len	= i4 WITH noconstant(0)		;0001
DECLARE stat = i4 WITH noconstant(0)	;0001
...
/* Move files to CCLUSERDIR */
SET sourcepath = "/cerner/w_custom/p0665_cust/from_client_site/Supply_Chain/item_inactivation/"
SET sourcefile = $FILENAME
SET destpath = "/cerner/d_p0665/ccluserdir/"
SET destfile = sourcefile

SET cmd = build2("mv ", BUILD(sourcepath,sourcefile), " ", BUILD(destpath,destfile))
...
set t_rec->filename 		= BUILD(destpath,destfile)
set t_rec->filename_no_path = destfile
...
for (i = 1 to t_rec->cnt)
	...
	if (900060_reply->status_data.status = "S")
		...
		elseif (size(900060_reply->qual,5) < 1)
			...
			if (900060_reply->qual[1].search_on_disp_key = t_rec->qual[i].item_alias)
				...
				set stat = alterlist(t_rec->qual[i].step_log,t_rec->qual[i].step_cnt)
				set t_rec->qual[i].step_log[t_rec->qual[i].step_cnt].step_title 		= "ITEM_ID"
				set t_rec->qual[i].step_log[t_rec->qual[i].step_cnt].step_message 		= "Item Alias returned but did not match"
				set t_rec->qual[i].step_log[t_rec->qual[i].step_cnt].step_status		= "F"
			endif
		endif
...
if (t_rec->error_ind = 1)
	set reply->status_data.status = "F"
	set reply->status_data.subeventstatus[1].operationname = "Item Inactivation"
	set reply->status_data.subeventstatus[1].operationstatus = "F"
	set reply->status_data.subeventstatus[1].targetobjectvalue = substring(1,100,t_rec->error_msg)
