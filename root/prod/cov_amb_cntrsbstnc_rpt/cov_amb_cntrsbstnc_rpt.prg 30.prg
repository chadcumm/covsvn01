...
004 08/14/2019  Cummings,Chad		added co-sign information                                 
******************************************************************************/
...
Record audit(
	...
	1 plist[*]
		...
		2 orders[*]
			...
			3 duration_unit  = vc
			3 pain_contract  = vc
 			3 order_dt_tm      = dq8	;004
 			3 sign_off_req     = vc	;004
 			3 phys_sign_req	   = vc ;004
 			3 phys_sign_off    = vc	;004
 			3 sign_off_status  = vc ;004
 			3 ordering_prov    = vc ;004
 			3 signoff_req_dt   = vc	;004
			3 supervising_phys = vc	;004
 			3 medicare         = vc	;004
			3 medicare_phys    = vc	;004
			3 action_sequence  = i2 ;004
	)
...
join o
	...
	and o.orig_ord_as_flag not in(1,2,3) ;exclude home, patient own meds
...
join pr2
	where pr2.person_id = oa.supervising_provider_id
 
join pra where pra.person_id = outerjoin(pr.person_id)
	and pra.alias_pool_cd = outerjoin(value(uar_get_code_by("DISPLAY",263,"DEA"))) ;DEA ;        263		DEA
...
Head o.person_id
	cnt = cnt + 1
	stat = alterlist(audit->plist, cnt)
	audit->reccnt = cnt
	audit->plist[cnt].practice = org.org_name ;003			;001 - Changed to practice
	audit->plist[cnt].provider_id = pr.person_id
	audit->plist[cnt].provider_name = replace(pr.name_full_formatted, ",", " ",0)
	audit->plist[cnt].provider_dea = pra.alias
	audit->plist[cnt].patientid = o.person_id
	audit->plist[cnt].encntrid = o.encntr_id
	audit->plist[cnt].pat_name = p.name_full_formatted
	audit->plist[cnt].pat_dob = format(cnvtdatetimeutc(datetimezone(p.birth_dt_tm,p.birth_tz),1),'mm/dd/yyyy;;q')
	audit->plist[cnt].zip = a.zipcode
	ocnt = 0
head o.order_id
	ocnt = ocnt + 1
	stat =  alterlist(audit->plist[cnt].orders, ocnt)
	...
	audit->plist[cnt].orders[ocnt].order_dt_tm = o.orig_order_dt_tm
	audit->plist[cnt].orders[ocnt].action_sequence = oa.action_sequence
	audit->plist[cnt].orders[ocnt].supervising_phys = pr2.name_full_formatted
	audit->plist[cnt].orders[ocnt].ordering_prov = pr.name_full_formatted
With nocounter
...
join ov
	...
	and   ov.review_type_flag =           2.00	;Doctor Cosign
	and   ov.action_sequence =  audit->plist[d1.seq].orders[d2.seq].action_sequence
join p1
	where p1.person_id = ov.provider_id
join p2
	where p2.person_id = ov.review_personnel_id 
order by
	o.order_id
	,ov.review_dt_tm
...
head report
	...
	idx = 0
head o.order_id
 	idx = locateval(cnt,1,size(audit->plist->orders,5), o.order_id, audit->plist[d1.seq].orders[d2.seq].orderid)
detail
 stat = 0
 call echo(build("order_id=",o.order_id,":","provider_id=",ov.provider_id))
 audit->plist[d1.seq].orders[d2.seq].sign_off_req = "Yes"
 audit->plist[d1.seq].orders[d2.seq].phys_sign_off = p2.name_full_formatted
 audit->plist[d1.seq].orders[d2.seq].signoff_req_dt = format(ov.review_dt_tm ,";;q")
 audit->plist[d1.seq].orders[d2.seq].phys_sign_req = p1.name_full_formatted
 if (ov.reviewed_status_flag = 0)
		audit->plist[d1.seq].orders[d2.seq].sign_off_status = 	"Not Reviewed"
		   elseif (ov.reviewed_status_flag = 1)
		audit->plist[d1.seq].orders[d2.seq].sign_off_status =    	"Accepted"
		   elseif (ov.reviewed_status_flag = 2)
		audit->plist[d1.seq].orders[d2.seq].sign_off_status =    	"Rejected"
		   elseif (ov.reviewed_status_flag = 3)
		audit->plist[d1.seq].orders[d2.seq].sign_off_status =    	"No Longer Needing Review"
		   elseif (ov.reviewed_status_flag = 4)
		audit->plist[d1.seq].orders[d2.seq].sign_off_status =    	"Superceded"
		   elseif (ov.reviewed_status_flag = 5)
		audit->plist[d1.seq].orders[d2.seq].sign_off_status =    	"Reviewed"
			endif
...
plan d1
join ea
	where ea.encntr_id = audit->plist[d1.seq].encntrid
...
head ea.encntr_id
	stat = 0	
detail
	audit->plist[d1.seq].fin = cnvtalias(ea.alias,ea.alias_pool_cd)
with nocounter
...
SELECT DISTINCT INTO value($OUTDEV)
	...
	, COSIGN_REQUEST = trim(SUBSTRING(1, 200, AUDIT->plist[D1.SEQ].orders[D2.SEQ].sign_off_status))
	, COSIGN_REQUESTED = trim(SUBSTRING(1, 200, AUDIT->plist[D1.SEQ].orders[D2.SEQ].phys_sign_req))
	, COSIGN_COMPLETED = trim(SUBSTRING(1, 200, AUDIT->plist[D1.SEQ].orders[D2.SEQ].phys_sign_off))
	, COSIGN_REQUEST_DT_TM = trim(SUBSTRING(1, 200, AUDIT->plist[D1.SEQ].orders[D2.SEQ].signoff_req_dt))
