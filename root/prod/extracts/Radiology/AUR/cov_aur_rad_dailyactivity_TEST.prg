...
* -------	---	----------	----------	-------------------------------------------------------------------------------*
			...
			013	05/14/2021	TAB			Added logic for order details with meanings QCDSMUTILIZED and AUCORDERADHERENCE.
			014	04/06/2022	TAB			Added logic to handle empty record structure.
 
************************* END OF ALL MODCONTROL BLOCKS *****************************************************************/
 
 
DROP PROGRAM 	cov_aur_rad_dailyactivity_test:dba GO
CREATE PROGRAM 	cov_aur_rad_dailyactivity_test:dba
...
2	cptcd 			= vc
2   cptcdmod		= vc
2	qcdsmutilized		= vc ;013
2	aucorderadherence	= vc ;013
...
%i cust_script:aur_radiologists.inc ;012
...
detail 
	...
	rData->ENC[d.seq].qcdsmutilized = trim(od.oe_field_display_value) ;013
	rData->ENC[d.seq].aucorderadherence = trim(od2.oe_field_display_value) ;013
