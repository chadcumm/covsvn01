...
* -------	---	----------	----------	-------------------------------------------------------------------------------*
			...
			002	10/29/2018	TAB			Adjusted start date and end date prompts.
			003	10/31/2018	TAB			Changed output format of patient date of birth.
										Fixed issue where extra blank lines appeared after each dictation.
			004	11/01/2018	TAB			Changed output format of procedure ordered section.
			005	01/04/2019	TAB			Updated list of radiologists.
			006	01/16/2019	TAB			Updated list of radiologists.
			007	01/17/2019	TAB			Removed accession exclusion of CA.
										Included accession of CA for derivation of value for RAD_PROC_TYPE.
			008	02/01/2019	TAB			Updated list of radiologists.
			009	06/06/2019	TAB			Updated list of radiologists.
			010	06/14/2019	TAB			Updated list of radiologists.
			011	08/02/2019	TAB			Updated list of radiologists.
			012	08/26/2019	TAB			Updated list of radiologists.
			013	09/23/2019	TAB			Corrected list of radiologists.
			014	09/24/2019	TAB			Moved list of radiologists to include file.
			015	02/05/2021	TAB			Added logic for contract accounts.
			016	05/14/2021	TAB			Added logic for order details with meanings QCDSMUTILIZED and AUCORDERADHERENCE.
			017	11/05/2021	TAB			Adjusted final output to remove extraneous spaces.
			018	04/06/2022	TAB			Added logic to handle empty record structure.
 
************************* END OF ALL MODCONTROL BLOCKS *****************************************************************/
 
 
DROP PROGRAM 	cov_aur_rad_dict_test:dba GO
CREATE PROGRAM 	cov_aur_rad_dict_test:dba
...
set output->filenamed = concat('aur_code',format(curdate, "yyyymmdd;;d"), '_cer', '.txt')
set output->astream = "/cerner/w_custom/p0665_cust/to_client_site/ClinicalAncillary/Radiology/Extracts/AUR/"
set output->temp_dict = concat('cer_temp:' , 'aur_code',  format(curdate, "yyyymmdd;;d"),'_cer', '.txt')
...
2 cptcd				=   vc
2 cptcdmod			=   vc
2 qcdsmutilized		=	vc ;016
2 aucorderadherence	=	vc ;016

2 ce_blob_event_id = f8
1 total = i4
1 subt_demo = i4
1 subt_dict = i4
)
...
%i cust_script:aur_radiologists.inc ;014
...
and 	not substring(6,2,ce.accession_nbr) in ('NP', 'SC') ;007
...
; AUR docs from include file
  and expand(num, 1, radiologists->cnt, orrs.radiologist_id, radiologists->list[num].person_id) ;014
...
rDATA->ENC[cnt].NU_CD 		= E.location_cd
rDATA->ENC[cnt].NU 			= UAR_GET_CODE_DISPLAY(E.location_cd)
...
rDATA->ENC[cnt].ce_blob_event_id = cb.event_id
...
rDATA->ENC[d.seq].order_cat_cd  	= cnvtstring(o.catalog_cd) ;004
...
detail
	...
	rData->ENC[d.seq].qcdsmutilized = trim(od.oe_field_display_value) ;016
	rData->ENC[d.seq].aucorderadherence = trim(od2.oe_field_display_value) ;016
...
;declare print_blob = c6000 with protect ;017
declare print_blob = vc with noconstant(fillstring(6000, " ")), protect ;017
   ...
   ;****** initialize blobout to a size that will be large enough to hold the full uncompressed blob
    ...
    blobout = notrim(concat(notrim(blobout),notrim(substring(1,finlen,fillstring(32768, " ")))))
 	...
 	;LINE 1
		...
		CK_IN_DT_T		=   FORMAT(rDATA->ENC[idx].check_in_dt_tm, "HHMM;;D")
		...
		ofill2			=   replace(trim(cnvtstring(cnvtint(rDATA->ENC[idx].order_id)),3),CHAR(0),"") ;004
 
		if (substring(4,2,cnvtupper(rDATA->ENC[idx].accessiond)) in ("IR", "CA")) ;007
 	...
 	;;LINE 2
		...
		DOB				= FORMAT(rDATA->ENC[idx].BIRTH_DT_TM, "YYYY-MM-DD;;D") ;003
;		dob_year   		= FORMAT(rDATA->ENC[idx].BIRTH_DT_TM, "YYYY;;D") ;003
;		dob_month   	= FORMAT(rDATA->ENC[idx].BIRTH_DT_TM, "MM;;D") ;003
;		dob_day   		= FORMAT(rDATA->ENC[idx].BIRTH_DT_TM, "DD;;D") ;003
	...
	;line 3
;		RAD_ORD_PROC 	=  replace(trim(rDATA->ENC[idx].order_mnem,3),CHAR(0),"") ;016
	;line 4
...
;	dob_day   		,tabVar, ;003
	...
	IPlan_Code   	,crVar
  	row + 1
;LINE 3
;	"Procedure Ordered: ", RAD_ORD_PROC	,crVar, row + 1 ;004
 
	;004
	"Procedure Ordered: ", ck_in_num, tvar,
...
;    call echo(build("size of BlobNoRTF:",size(BlobNoRTF)))
    ...
    while (offset < size(trim(BlobNoRTF,3)))
        ...
        if (size(print_blob) > 0)
			print_blob = trim(replace(print_blob, "  ", " ", 4)) ;017
