...
* -------	---	----------	----------	-------------------------------------------------------------------------------*
			...
			003	10/22/2018	TAB			Added logic for symbol mapping.
			004	10/23/2018	TAB			Adjusted logic for symbol mapping.
										Adjusted final output.
			005	11/01/2018	TAB			Changed record structure data type for cpt elements.
			006 11/20/2018	TAB			Added physicians.
			007	01/09/2019	TAB			Fixed missing outerjoin for insurances.
			008	01/16/2019	TAB			Removed physician.
			009 02/13/2019	TAB			Added physician.
			010 04/29/2019	TAB			Added physician.
			011	12/11/2019	TAB			Moved list of radiologists to include file.
			012	05/11/2020	TAB			Corrected issue with line 1 field 1 of demographics output.
										Set demographics facility to match dictation facility.
			013	02/04/2021	TAB			Added logic for contract accounts.
			014	05/14/2021	TAB			Added logic for order details with meanings QCDSMUTILIZED and AUCORDERADHERENCE.
			015	07/06/2021	TAB			Added logic to allow accession numbers with CA when facility is MHHS.
			016	11/04/2021	TAB			Adjusted final output to remove extraneous spaces.

************************* END OF ALL MODCONTROL BLOCKS ****************************************************************/
 
DROP PROGRAM 	COV_RAD_VISTA_EXT_TEST:dba GO
CREATE PROGRAM 	COV_RAD_VISTA_EXT_TEST:dba
...
set output->astream = build("/cerner/w_custom/", cnvtlower(curdomain),
						"_cust/to_client_site/ClinicalAncillary/Radiology/Extracts/Vista/")
...
RECORD rDATA(
	1 ENC[*]						;one occurrence per exam per encouonter per patient
		...
		2 demog_fac			= c1 ;012
		2 demog_perf_dept	= c40 ;012
		2 demog_pat_type	= c3 ;012
		2 demog_dept		= c3 ;012
		2 demog_fac_disp	= c100 ;012
		
		2 cptcd				= vc ;005
		2 cptcdmod			= vc ;005
		2 qcdsmutilized		= vc ;014
		2 aucorderadherence	= vc ;014
...
declare fac_mhhs_var			= f8 with constant(2552503639.00) ;015
...
%i cust_script:vista_radiologists.inc ;011
...
; Vista docs from include file
  and expand(num, 1, radiologists->cnt, oros.radiologist_id, radiologists->list[num].person_id) ;011
...
;and	not substring(6,2,ce.accession_nbr) in ('CA', 'NP', 'SC') ;015
and 	not substring(6,2,ce.accession_nbr) in ('NP', 'SC') ;015
...
and 	PA.alias_pool_cd in (STARDOC_CD, statedoc_cd) ; check on this type
...
detail
	...
	rData->ENC[d.seq].qcdsmutilized = trim(od.oe_field_display_value) ;014
	rData->ENC[d.seq].aucorderadherence = trim(od2.oe_field_display_value) ;014
...
; The following will need to be outbound aliased for the longer term
  ...
  if (  rDATA->ENC[I].FACILITY_CD in (
	...
	;012
	set rDATA->ENC[I].demog_fac			= cnvtupper(substring(1, 1, rDATA->ENC[I].facility))
	set rDATA->ENC[I].demog_perf_dept	= cnvtupper(rDATA->ENC[I].perf_dept)
	set rDATA->ENC[I].demog_pat_type  	= cnvtupper(substring(1, 3, rDATA->ENC[I].pat_type))
	set rDATA->ENC[I].demog_dept		= fillstring(3, " ")
 
	set rDATA->ENC[I].demog_dept		= evaluate2(
											if (rDATA->ENC[I].demog_fac = "F" and 
												rDATA->ENC[I].demog_perf_dept not like "*FSR*WEST*")
												"XRF"
											elseif (rDATA->ENC[I].demog_perf_dept like "*FSR*WEST*")
												"XRD"
											elseif (rDATA->ENC[I].demog_fac = "S")
												"XRS"
											else
												" "
											endif
	...
	set rDATA->ENC[I].demog_fac_disp	= evaluate2(
											if (rDATA->ENC[I].demog_pat_type = "DCF")
												"W"
											else
												rDATA->ENC[I].demog_fac
											endif
										)
	;
 endfor
...
;select distinct  into "NL:"
	dispfac	= concat(trim(replace(rDATA->ENC[d.seq].demog_fac_disp ,CHAR(0),"|"), 3), char(32), ;012
					 trim(replace(rDATA->ENC[d.seq].pat_type_ioe,CHAR(0),"|"), 3), char(32), ;012
					 trim(replace(rDATA->ENC[d.seq].pat_type,CHAR(0),"|"), 3)) ;012
...
;declare print_blob = c6000 with protect ;016
declare print_blob = vc with noconstant(fillstring(6000, " ")), protect ;016
 ...
 ;	if (idx > 0)
  ...
  ;  col 0 "*****************FIN:"   fin, crVar,    row +1
   ; col 0 "*****************Event ID:"  cb.event_id, crVar, row +1
	...
	;LINE 1
 	fac				=	cnvtupper(substring(1, 1, rDATA->ENC[idx].facility)) ;003
 	perf_dept		=	cnvtupper(rDATA->ENC[idx].perf_dept) ;003
	pat_type  		=	cnvtupper(substring(1, 3, rDATA->ENC[idx].pat_type)) ;003
 	dept			=	fillstring(3, " ") ;004
 
 	;004
	dept			=	evaluate2(
							if (fac = "F" and perf_dept not like "*FSR*WEST*")
								"XRF"
							elseif (perf_dept like "*FSR*WEST*")
								"XRD"
							elseif (fac = "S")
								"XRS"
							else
	...
	FAC_DISP		=	evaluate2(
							if (pat_type = "DCF")
								"W"
							elseif (pat_type = "CNF" and dept = "XRF")
								"@"
							elseif (pat_type = "CNF" and dept = "XRD")
								"#"
;							elseif (pat_type = "AOF" and exam_cd_key = 41016)
;								"%"
							elseif (pat_type = "COS" and dept = "XRS")
								"$"
							else
								fac
							endif
...
; 	 fac			=   build(replace(substring(1,1,  trim(rDATA->ENC[idx].FACILITY,3)), char(0),  "" )) ;003
 	...
 	,CK_IN_DT_Y 	=   FORMAT(rDATA->ENC[idx].check_in_dt_tm, "YYYY;;D") ;004
	,CK_IN_DT_M  	= 	FORMAT(rDATA->ENC[idx].check_in_dt_tm, "MM;;D") ;004
	,CK_IN_DT_D  	= 	FORMAT(rDATA->ENC[idx].check_in_dt_tm, "DD;;D") ;004
	,CK_IN_DT_T  	= 	FORMAT(rDATA->ENC[idx].check_in_dt_tm, "HHMM;;D") ;004
...
;		,pat_type  		= replace(trim(rDATA->ENC[idx].pat_type,3),CHAR(0),"") ;003
	...
	;LINE 3
;		,RAD_ORD_PROC 	= concat("Procedure Ordered: " , rDATA->ENC[idx].order_mnem) ;014
;		,ADMIT_DX  		= concat("Admitting DX: ",  rDATA->ENC[idx].ADMIT_DX ) ;014
;		,WORK_DX  		= concat("Working DX: ",  rDATA->ENC[idx].work_dx ) ;014
;		,rad_dr			= rDATA->ENC[idx].RADIOLOGIST_ID ;014
 		...
 		col 0	"Procedure Ordered: ", ck_in_num, tvar,
		 		...
		 		ofill2, crvar, row + 1, ; rad_ord_proc, crvar,row + 1
 
 	;014
 	;LINE 4
  		col 0	"AUC: ", rDATA->ENC[idx].qcdsmutilized , tvar,
...
;foot 	oros.rad_report_id
    ...
    while (offset < size(trim(BlobNoRTF,3)))
        ...
        if (size(print_blob) > 0)
			print_blob = trim(replace(print_blob, "  ", " ", 4)) ;004 ;016
...
;		transcriptionist = concat ("Transcriptionist- ",  rDATA->ENC[idx].TRANSCRIBED ) ;004
;		readby			 = concat ("Read By- ",   rDATA->ENC[idx].TRANSCRIBED ) ;004
;		revby			 = concat ("Reviewed and E-Signed By- ",  rDATA->ENC[idx].APPROVED ) ;004
