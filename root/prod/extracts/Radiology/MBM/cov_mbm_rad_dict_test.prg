...
* -------	--- ----------	----------	-------------------------------------------------------------------------------*
			...
			008	11/05/2021	TAB			Adjusted final output to remove extraneous spaces.
...
;declare print_blob = c6000 with protect ;008
declare print_blob = vc with noconstant(fillstring(6000, " ")), protect ;008
...
;    call echo(build("size of BlobNoRTF:",size(BlobNoRTF)))
    ...
    while (offset < size(trim(BlobNoRTF,3)))
        ...
        if (size(print_blob) > 0)
			print_blob = trim(replace(print_blob, "  ", " ", 4)) ;008
