...
DROP PROGRAM cov_pha_scorecard_extract :dba go
CREATE PROGRAM cov_pha_scorecard_extract :dba
...
***************************************************************************/
 ...
 ;declare filename_var  = vc with constant('cer_temp:cov_pha_scorecard_medadmin.txt'), protect
...
join pa where pa.person_id = outerjoin(med_admin->mlist[d.seq].personid)
...
Head e.encntr_id
	cnt = 0
	idx = 0
      idx = locateval(cnt,1,size(med_admin->mlist,5),e.encntr_id, med_admin->mlist[cnt].encntrid)
 
      while(idx > 0)
		med_admin->mlist[idx].fin = ea1.alias
		med_admin->mlist[idx].mrn = ea.alias
		med_admin->mlist[idx].cmrn = pa.alias
		med_admin->mlist[idx].admit_dt = format(e.reg_dt_tm, 'mm/dd/yyyy hh:mm;;q')
		med_admin->mlist[idx].disch_dt = format(e.disch_dt_tm, 'mm/dd/yyyy hh:mm;;q')
 		idx = locateval(cnt,(idx+1),size(med_admin->mlist,5),e.encntr_id, med_admin->mlist[cnt].encntrid)
	endwhile
...
Head mi.item_id
	...
	cnt = 0
      ...
      idx = locateval(cnt ,1, med_admin->med_rec_cnt ,mi.item_id ,med_admin->mlist[cnt].item_id)
...
Foot  mi.item_id
    while(idx > 0)
	     med_admin->mlist[idx].drug_brand_name = drug_brand_name
	     med_admin->mlist[idx].item_number = item_number
	     med_admin->mlist[idx].drug_generic_name = drug_generic_name
	     idx = locateval(cnt,(idx+1), med_admin->med_rec_cnt ,mi.item_id ,med_admin->mlist[cnt].item_id)
    endwhile
...
Head oa.order_id
	...
	idx = 0
      ...
      while(idx > 0)
		...
		med_admin->mlist[idx].ordering_physician_name = trim(pr.name_full_formatted)
   ...
   Select into value(filename_var)
	...
	Head report
		file_header_var = build(
			...
			,wrap3("End date")
			,wrap3("event_id")
			,wrap3("parent_event_id")
			,wrap3("ori_order_id")
			,wrap3("Facility Code")
			,wrap3("Hospital Account Number")
			,wrap3("Star Master Patient Account")
			,wrap3("Cerner Master Patient Account")
			,wrap3("encounter class")
			,wrap3("encounter type")
			,wrap3("patient type")
			,wrap3("HMM Item Number")
			,wrap3("Prescription Number")
			,wrap3("Med Admin date")
			,wrap3("Medication Name")
			,wrap3("Order Mnemonic")
			;,wrap3("Detail Display")
  			,wrap3("Drug Generic Name")
  			,wrap3("Brand Name")
  			,wrap3("Strength Dose")
			...
			,wrap3("Attending Physician Number")
			,wrap3("Ordering Physician Number")
			,wrap3("Performed Personnel Username")
			,wrap3("Admit Date Time")
			,wrap3("Discharge Date Time") )
	...
	Head d.seq
		...
		output_orders = build(output_orders
			...
			,wrap3(cnvtstring(med_admin->mlist[d.seq].event_id))
			,wrap3(cnvtstring(med_admin->mlist[d.seq].parent_event_id))
			,wrap3(cnvtstring(med_admin->mlist[d.seq].orig_orderid))
			,wrap3(med_admin->mlist[d.seq].strata_facility_cd)
			,wrap3(med_admin->mlist[d.seq].fin)
			,wrap3(med_admin->mlist[d.seq].mrn)
			,wrap3(med_admin->mlist[d.seq].cmrn)
			,wrap3(med_admin->mlist[d.seq].encntr_class)
			,wrap3(med_admin->mlist[d.seq].encntr_type)
			,wrap3(med_admin->mlist[d.seq].pat_type)
			,wrap3(med_admin->mlist[d.seq].item_number)
			,wrap3(cnvtstring(med_admin->mlist[d.seq].orderid))
			,wrap3(med_admin->mlist[d.seq].med_admin_dt)
			,wrap3(med_admin->mlist[d.seq].medication)
			,wrap3(med_admin->mlist[d.seq].ordered_mnemonic)
			;,wrap3(med_admin->mlist[d.seq].detail_display)
			,wrap3(med_admin->mlist[d.seq].drug_generic_name)
			,wrap3(med_admin->mlist[d.seq].drug_brand_name)
			,wrap3(med_admin->mlist[d.seq].strength_dose)
			,wrap3(med_admin->mlist[d.seq].strength_dose_unit)
			,wrap3(med_admin->mlist[d.seq].route_of_admin)
			,wrap3(med_admin->mlist[d.seq].volume_dose)
			,wrap3(med_admin->mlist[d.seq].volume_dose_unit)
			,wrap3(med_admin->mlist[d.seq].rate)
			,wrap3(med_admin->mlist[d.seq].rate_unit)
			,wrap3(med_admin->mlist[d.seq].drug_form)
			,wrap3(med_admin->mlist[d.seq].frequency)
			,wrap3(cnvtstring(med_admin->mlist[d.seq].quantity))
			,wrap3(med_admin->mlist[d.seq].quantity_unit)
			,wrap3(med_admin->mlist[d.seq].duration)
			,wrap3(med_admin->mlist[d.seq].duration_unit)
			,wrap3(med_admin->mlist[d.seq].drug_class_code1)
			,wrap3(med_admin->mlist[d.seq].drug_class_description1)
			,wrap3(med_admin->mlist[d.seq].drug_class_code2)
			,wrap3(med_admin->mlist[d.seq].drug_class_description2)
			,wrap3(med_admin->mlist[d.seq].drug_class_code3)
			,wrap3(med_admin->mlist[d.seq].drug_class_description3)
			,wrap3(med_admin->mlist[d.seq].atend_prsnl_number)
			,wrap3(med_admin->mlist[d.seq].ordering_phys_number)
			,wrap3(med_admin->mlist[d.seq].perform_username)
			,wrap3(med_admin->mlist[d.seq].admit_dt)
			,wrap3(med_admin->mlist[d.seq].disch_dt) )
