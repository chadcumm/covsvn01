...
002     01/21/2020  Marcus Flowers, DBA Modified from original to run as an Ops Job
*****************************************************************************/
 
drop program cov_ops_jobs_monitor:dba go
create program cov_ops_jobs_monitor:dba
...
;---CASE
	...
	2 surg_case_nbr		 = c16	;surgical case number
...
set output->directory = "/cerner/w_custom/p0665_cust/to_client_site/Extracts/aStreamMonitoring/"
set output->filename   = concat("astreamjobs",format(curdate, "MM-DD-YYYY;;d_"),"_",format(curtime, "hhmm;;m"))
...
set END_DATE   = cnvtlookahead("curdate, 23:23:59")
;set END_DATE   = cnvtdatetime("1,SEC", END_DATE)
 
 
;FOR MANUAL EXTRACT RUN
;set START_DATE = CNVTDATETIME("22-JAN-2019 00:00:00")  ;$STARTDATE_PMPT  30-OCT-2018
;set END_DATE   = CNVTDATETIME("curdate, 23:23:59")  ;$ENDDATE_PMPT
...
;============================
	...
	declare hdr6  = c15 with noconstant(fillstring(15, ' ')), protect  ;surg_case_nbr
...
;	declare sep   = c1  with noconstant(fillstring(1, '|' )), protect  ;pipe
	...
	declare col6  = c15 with noconstant(fillstring(15, ' ')), protect  ;surg_case_nbr
