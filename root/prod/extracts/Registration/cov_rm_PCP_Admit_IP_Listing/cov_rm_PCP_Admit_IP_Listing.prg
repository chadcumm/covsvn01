...
******************************************************************************
	...
	Source file name:	cov_rm_PCP_Admit_IP_Listing.prg
	Object name:		cov_rm_PCP_Admit_IP_Listing
	Request #:			11991, 12488
 
	Program purpose:	Provides data for inpatient admit patients.
 
	Executing from:		CCL
 
 	Special Notes:		Adopted from STAR SQL:
 							- QBCC_ADMIT_IP_ONLY_BY_PCP_WRPT
 							- QBCC_ADMIT_IP_ONLY_BY_PCP_RPT
...
******************************************************************************
 	...
 	----------	--------------------	--------------------------------------
001	03/18/2022	Todd A. Blanchard		Added description for patient type.
002	04/08/2022	Todd A. Blanchard		Added page headers for physicians and groups
										on page breaks.
 
******************************************************************************/

DROP PROGRAM cov_rm_PCP_Admit_IP_Listing:dba GO
CREATE PROGRAM cov_rm_PCP_Admit_IP_Listing:dba
...
if ($weekend_only)
	set file_var = build("admit_ip_only_by_pcp_weekend_ccl", ".asc")
else
	set file_var = build("admit_ip_only_by_pcp_ccl", ".asc")
endif
...
from
	...
	, (inner join CODE_VALUE_OUTBOUND cvo2 on cvo2.code_value = e.encntr_type_cd
		and cvo2.contributor_source_cd = covenant_var)
	
	, (inner join CODE_VALUE_OUTBOUND cvo3 on cvo3.code_value = e.loc_nurse_unit_cd
		and cvo3.contributor_source_cd = covenant_var)
	
	, (inner join CODE_VALUE_OUTBOUND cvo4 on cvo4.code_value = e.loc_room_cd
		and cvo4.contributor_source_cd = covenant_var)
	
	, (inner join CODE_VALUE_OUTBOUND cvo5 on cvo5.code_value = e.loc_bed_cd
		and cvo5.contributor_source_cd = covenant_var)
...
head efh.encntr_flex_hist_id
	idx = 0
 	...
 	if (idx > 0)
		...
		enc_flex->list[idx].pat_type				= cvo2.alias
		enc_flex->list[idx].pat_type_desc			= uar_get_code_display(e.encntr_type_cd) ;001
		enc_flex->list[idx].nurse_unit				= cvo3.alias
		enc_flex->list[idx].room					= cvo4.alias
		enc_flex->list[idx].bed						= cvo5.alias
...
into value(output_var)	
	...
	, pat_type					= build(enc_flex->list[d1.seq].pat_type, enc_flex->list[d1.seq].fac)
	, pat_type_desc				= trim(enc_flex->list[d1.seq].pat_type_desc, 3) ;001
	
	, nurse_unit				= trim(enc_flex->list[d1.seq].nurse_unit, 3)
	, room						= trim(enc_flex->list[d1.seq].room, 3)
	, bed						= trim(enc_flex->list[d1.seq].bed, 3)
...
head page
	...
	row + 2
 	...
 	if ($weekend_only)
 		title = build2("Primary Care Physician Listing of Inpatient Admits for Weekend Starting", " ", start_date)
 	else
 		title = build2("Primary Care Physician Listing of Inpatient Admits for ", " ", start_date)
 	endif
...
detail	
	...
	col 46	dob
	col 59	ptd = substring(1, 20, pat_type_desc), ptd ;001
	col 79	b = build2(trim(nurse_unit), " ", trim(room), " ", trim(bed)), b
	col 105	admit_dt_tm
	col 145	rv = substring(1, 90, reason_for_visit), rv
	row + 1
