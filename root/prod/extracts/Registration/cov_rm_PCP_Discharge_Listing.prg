...
******************************************************************************
	...
	Source file name:	cov_rm_PCP_Discharge_Listing.prg
	Object name:		cov_rm_PCP_Discharge_Listing
	Request #:			11991, 12488
 
	Program purpose:	Provides data for discharged patients.
 
	Executing from:		CCL
 
 	Special Notes:		Adopted from STAR SQL:
 							- QBCC_DISCHARGED_BY_PCP_W_RPT
 							- QBCC_DISCHARGED_BY_PCP_RPT
...
DROP PROGRAM cov_rm_PCP_Discharge_Listing:dba GO
CREATE PROGRAM cov_rm_PCP_Discharge_Listing:dba
...
if ($weekend_only)
	set file_var = build("discharged_by_pcp_weekend_ccl", ".asc")
else
	set file_var = build("discharged_by_pcp_ccl", ".asc")
endif
...
where
	...
	and efh.encntr_type_cd not in (contract_var, prereg_var, scheduled_var)
...
head efh.encntr_flex_hist_id
	idx = 0
 	...
 	if (idx > 0)
		enc_flex->list[idx].disch_dt_tm				= e.disch_dt_tm	
		...
		enc_flex->list[idx].disch_disp				= uar_get_code_display(e.disch_disposition_cd)
...
into value(output_var)	
	...
	, discharge_disp			= trim(enc_flex->list[d1.seq].disch_disp, 3)
	...
	, discharge_dt				= cnvtupper(
									format(enc_flex->list[d1.seq].disch_dt_tm, "mm/dd/yyyy;;d"))
...
head page
	...
	row + 2
 	...
 	if ($weekend_only)
 		title = build2("Primary Care Physician Listing of Discharge Patients for Weekend Starting", " ", start_date)
 	else
 		title = build2("Primary Care Physician Listing of Discharge Patients for ", " ", start_date)
 	endif
...
detail	
	...
	col 46	dob
	col 59	ptd = substring(1, 30, pat_type_desc), ptd ;001
	col 90	d = substring(1, 30, discharge_disp), d ;001
	col 122	admit_dt_tm
	col 145	discharge_dt
	col 160	rv = substring(1, 75, reason_for_visit), rv
	row + 1
