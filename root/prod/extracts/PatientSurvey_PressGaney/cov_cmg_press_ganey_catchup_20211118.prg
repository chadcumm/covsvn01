...
***********************************************************************************************
 ...
 0008  05/07/2021     Dawn Greer, DBA        CR 10316 - Remove quotes from address 2 field.
 0009  07/19/2021     Dawn Greer, DBA        CR 10875 - Found an error in pulling the e-mail
 0010  07/21/2021     Dawn Greer, DBA        CR 10875 - Problem with the process.  Bad table alias. Fixed it.
 0011  07/23/2021     Dawn Greer, DBA        CR 10875 - Added more exclusions to the Patient Name for test patients
 0012  11/18/2021     Dawn Greer, DBA        CR 11633 - Since the upgrade the data volume for CMG has been low
                                             The encntr_prsnl_reltn.priority_seq was set to pull when the value was
                                             1 but after the upgrade the value changed to 0 (ZERO).
                                             I added code to look for both (encntr_prsnl_reltn.priority_seq IN (0,1))
***********************************************************************************************/
...
IF ($RunType = 'D')   ;Sending Daily File ;007
	SET startdate = CNVTDATETIME(CURDATE-4,0)
...
SELECT DISTINCT 
	  ...
	  ,Address_2 = EVALUATE2(IF (SIZE(addr.street_addr2) = 0) " " ELSE REPLACE(TRIM(addr.street_addr2,3),'"','') ENDIF)
	  ...
	  ,Location_Code = EVALUATE2(IF (SIZE(orga.alias) = 0) " "
	  		ELSEIF (res.resource_cd = 3785513577.00 /*FLPC Walk-in*/) "FLPCW"
	  		ELSE orga.alias ENDIF)
	  ,Location_Name = EVALUATE2(IF (res.resource_cd = 3785513577.00 /*FLPC Walk-in*/) "Fort Loudoun Primary Care Walk-In"
	  		ELSE EVALUATE(orga.alias,
	  ...
	  ,Email = EVALUATE2(IF (SIZE(email_addr.street_addr) = 0
	  	...
	  	OR CNVTUPPER(email_addr.street_addr) LIKE '*@.COM*'  ;0009
	  	OR CNVTUPPER(email_addr.street_addr) LIKE '*REFUSED*'
	  	OR CNVTUPPER(email_addr.street_addr) LIKE 'DECLINED*' ;0009
	  	OR CNVTUPPER(email_addr.street_addr) LIKE 'ASK@*' ;0009
	  	OR CNVTUPPER(email_addr.street_addr) LIKE 'ASKFOREMAIL@*') " " ELSE TRIM(email_addr.street_addr,3) ENDIF)  ;0009
	  ,Hospitalist_1 = " "
	  ,Hospitalist_2 = " "
	  ,ER_Admit = " "
...
FROM SCH_APPT sch
	, (INNER JOIN SCH_APPT res ON (sch.sch_event_id = res.sch_event_id
		AND res.role_meaning IN ('RESOURCE','DEFRESROLE')
		))
	...
	, (LEFT JOIN ENCNTR_ALIAS fin_nbr ON (enc.encntr_id = fin_nbr.encntr_id
		...
		AND fin_nbr.end_effective_dt_tm > CNVTDATETIME(CURDATE,CURTIME3)
   		AND fin_nbr.alias_pool_cd = 2554138229.00  /*HOME*/
   		))
	...
	, (INNER JOIN ENCNTR_PRSNL_RELTN epr ON (enc.encntr_id = epr.encntr_id
		AND epr.priority_seq IN (0,1)	;0012
...
DETAIL
	...
	exp_data->list[cnt].prov_person_id = prov.person_id
...
CALL ECHO ("***** GETTING PROVIDER SPECIALTY DATA ******")
CALL ECHO (BUILD("***** Start Date: ",FORMAT(startdate, "MM/DD/YYYY HH:mm:ss;;q"), " *****"))
CALL ECHO (BUILD("***** End Date: ",FORMAT(enddate, "MM/DD/YYYY HH:mm:ss;;q"), " *****"))
/**************************************************************
; Get Provider Specialty Data
**************************************************************/
...
FROM (dummyt d WITH seq = VALUE(SIZE(exp_data->list,5)))
	, PRSNL_GROUP_RELTN prov_grp_rel
	, PRSNL_GROUP prov_grp
PLAN d
JOIN prov_grp_rel WHERE prov_grp_rel.person_id = exp_data->list[d.seq].prov_person_id
...
JOIN prov_grp WHERE prov_grp_rel.prsnl_group_id = prov_grp.prsnl_group_id
	AND prov_grp.active_ind = 1
	AND prov_grp.prsnl_group_class_cd = 678635.00  /* Service */
 
/****************************************************************************
	Populate Record structure with Provider Specialty Data
*****************************************************************************/
 
HEAD prov_grp_rel.person_id
	cnt = 0
	idx = 0
	subcnt = 0
	idx = LOCATEVAL(cnt,1,SIZE(exp_data->list,5),prov_grp_rel.person_id, exp_data->list[cnt].prov_person_id)
 
FOOT prov_grp_rel.person_id
