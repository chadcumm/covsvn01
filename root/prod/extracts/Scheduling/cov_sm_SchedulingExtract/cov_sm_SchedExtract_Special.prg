...
******************************************************************************
	...
	Source file name:	cov_sm_SchedExtract_Special.prg
	Object name:		cov_sm_SchedExtract_Special
	Request #:			3740, 6711, 7783, 8233, 12349
...
009	07/21/2020	Todd A. Blanchard		Adjusted CCL for special request.
010	03/08/2022	Todd A. Blanchard		Changed practice site display to org name.
 
******************************************************************************/
 
drop program cov_sm_SchedExtract_Special:DBA go
create program cov_sm_SchedExtract_Special:DBA
...
declare start_datetime			= dq8 with noconstant(cnvtdatetime("01-JAN-2020 000000")) ;002 ;006 ;009
...
declare file_var				= vc with constant("sched_extract_special.csv")
...
from
	...
	, (inner join SCH_EVENT_ACTION seva on seva.sch_event_id = sa.sch_event_id
		...
		and seva.action_dt_tm = (
			...
			where
				sch_event_id = seva.sch_event_id
				and schedule_id = seva.schedule_id
				and action_meaning not in ("LINK", "SHUFFLE", "UNDO", "VIEW")
				and active_ind = 1
			group by
				sch_event_id
...
detail 
	...
	sched_appt->list[idx].ord_phys_group			= trim(org_ps.org_name, 3) ;010
...
WITH nocounter, expand = 1, time = 3600 ;003 ;005 ;009
	...
	, (left join ORDER_DETAIL od on od.order_id = o.order_id
		...
		and od.action_sequence = (
			...
			where 
				...
				and oe_field_meaning = "SCHEDAUTHNBR"
			group by
				order_id
		))
	...
	, (left join ORDER_DETAIL od3 on od3.order_id = o.order_id
		...
		and od3.action_sequence = (
			...
			where 
				...
				and oe_field_meaning = "REQSTARTDTTM"
			group by
				order_id
		))
...
WITH nocounter, expand = 1, time = 3600 ;009
...
select if (validate(request->batch_selection) = 1 or $output_file = 1)
	with nocounter, pcformat (^"^, ^,^, 1), format = stream, format, time = 3600 ;003 ;005 ;009
else
	with nocounter, separator = " ", format, time = 3600 ;003 ;005 ;009
endif
