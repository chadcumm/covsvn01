...
**************************************************************************************************************
    ...
    004        03/29/2020    Dawn Greer, DBA          Fixed extra line feeds
**************************************************************************************************************/
...
SELECT DISTINCT
    ...
    , UPDATEDBY = TRIM(P1.NAME_FULL_FORMATTED,3) ;001
FROM
	ENCOUNTER   E
...
IF (amb_notes->output_cnt > 0)
	...
	head dt.seq
		...
		output_rec = build(output_rec,
						...
						amb_notes->list[dt.seq].UPDATEDBY) ;001   ;004 removed CR
 
		output_rec = trim(output_rec,3)
 
	FOOT dt.seq
		col 0 output_rec
		IF (dt.seq < amb_notes->output_cnt) row + 1 ELSE row + 0 ENDIF		;004
 
	WITH nocounter, maxcol = 32000, FORMAT=STREAM, formfeed = none, maxrow=1, NOHEADING, NOFORMFEED		;004
ENDIF
