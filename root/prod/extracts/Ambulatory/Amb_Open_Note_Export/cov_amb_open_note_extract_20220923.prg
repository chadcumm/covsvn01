...
**************************************************************************************************************
    ...
    014		   06/25/2021	David Baumgardner		  Take out character return and line feed on Appointment Location.
    015        06/28/2021   Dawn Greer, DBA           CR 10724 - Removed the return and line feed from the
                                                      Code Value description (LCMC PreAdmission Testing) for the
                                                      Appointment Location causing the issue.  Removed the changes from
                                                      014.
    016        06/23/2022   Dawn Greer, DBA           CR 13131 - Added TOG Oncology Notes to the process
    017        08/15/2022   Dawn Greer, DBA           Commented out the echorecord command
...
FROM ENCOUNTER   E
	...
	, (INNER JOIN ENCNTR_PRSNL_RELTN EP ON (E.ENCNTR_ID = EP.ENCNTR_ID AND EP.ACTIVE_IND = 1
		...
		AND EP.TRANSACTION_DT_TM IN (SELECT MAX(EPR.TRANSACTION_DT_TM) FROM ENCNTR_PRSNL_RELTN EPR
			WHERE EPR.ENCNTR_ID = EP.ENCNTR_ID
			AND EPR.ACTIVE_IND = 1
			AND EPR.ENCNTR_PRSNL_R_CD IN (1119.00 /*Attending*/, 681283.00 /*Nurse Practitioner*/, 681284.00 /*Physician Assistant*/)
			AND EPR.END_EFFECTIVE_DT_TM >= CNVTDATETIME(CURDATE,CURTIME3))))  ;008
...
DETAIL
	...
	amb_notes->list[cnt].appt_location = appt_location    ;009 ;014 ;015
...
JOIN CV_EVENT_CODE WHERE C.EVENT_CD = CV_EVENT_CODE.CODE_VALUE
	...
	AND (CV_EVENT_CODE.DISPLAY_KEY LIKE '*OFFICE*NOTE*'   ;005
		...
		OR CV_EVENT_CODE.DISPLAY_KEY LIKE '*ONCOLOGY*NEW*CONSULT*NOTE*' ;016
		OR CV_EVENT_CODE.DISPLAY_KEY LIKE '*ONCOLOGY*OFFICE*VISIT*FOLLOW*')	;016
