...
******************************************************************************
	...
	Source file name:	cov_rm_PCP_Deceased_Listing.prg
	Object name:		cov_rm_PCP_Deceased_Listing
	Request #:			11991
 
	Program purpose:	Provides data for deceased patients.
 
	Executing from:		CCL
 
 	Special Notes:		Adopted from STAR SQL:
 							- QBCC_DECEASED_BY_PCP_W_RPT
 							- QBCC_DECEASED_BY_PCP_RPT
...
******************************************************************************
 	...
 	----------	--------------------	--------------------------------------
001	04/08/2022	Todd A. Blanchard		Added page headers for physicians and groups
										on page breaks.
 
******************************************************************************/

DROP PROGRAM cov_rm_PCP_Deceased_Listing:dba GO
CREATE PROGRAM cov_rm_PCP_Deceased_Listing:dba
...
declare deceased_var		= f8 with constant(uar_get_code_by("DISPLAYKEY", 268, "YES"))
...
if ($weekend_only)
	set file_var = build("deceased_by_pcp_weekend_ccl", ".asc")
else
	set file_var = build("deceased_by_pcp_ccl", ".asc")
endif
...
from
	...
	, (inner join PERSON p on p.person_id = efh.person_id
...
head efh.encntr_flex_hist_id
	idx = 0
 	...
 	if (idx > 0)
		...
		enc_flex->list[idx].dod						= cnvtdatetimeutc(datetimezone(p.deceased_dt_tm, p.deceased_tz), 1)
...
into value(output_var)	
	...
	, dod						= format(enc_flex->list[d1.seq].dod, "mm/dd/yyyy;;d")
...
head page
	...
	row + 2
 	...
 	if ($weekend_only)
 		title = build2("Primary Care Physician Listing of Expired Patients for Weekend Starting", " ", start_date)
 	else
 		title = build2("Primary Care Physician Listing of Expired Patients for ", " ", start_date)
 	endif
