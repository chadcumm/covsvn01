...
004	01/08/2020	Todd A. Blanchard		Adjusted format of CCL.
005	04/22/2020	Todd A. Blanchard		Added columns strength, strength_unit,
...
declare cpt1_var				= f8 with constant(uar_get_code_by("DISPLAYKEY", 14002, "CPT")) ;002
declare hosp1_var				= f8 with constant(uar_get_code_by("DISPLAYKEY", 14002, "HOSPITAL340BTECHJGTB")) ;003
declare hosp2_var				= f8 with constant(uar_get_code_by("DISPLAYKEY", 14002, "HOSPITAL340BTECHMODIFIER")) ;003
declare hosp3_var				= f8 with constant(uar_get_code_by("DISPLAYKEY", 14002, "HOSPITAL340BTECHORPHANED")) ;003
declare hosp4_var				= f8 with constant(uar_get_code_by("DISPLAYKEY", 14002, "HOSPITAL340BTECHSTANDARD")) ;003
...
record pharm_data (
	...
	1	list[*]
		2	cdm				= c200
		2	default_dose	= c60
		2	default_route	= c40 ;005
		2	item_id			= f8
		2	facility		= c40
		2	form			= c40
		2	strength		= f8 ;005
		2	strength_unit	= c40 ;005
		2	volume			= f8 ;005
		2	volume_unit		= c40 ;005
...
from
	...
	, (inner join MED_IDENTIFIER mi2 on mi2.item_id = md.item_id
 	...
 	;004
	, (inner join MED_FLEX_OBJECT_IDX mfoi2 on mfoi2.parent_entity_id = mi2.med_product_id
	...
	, (inner join MED_FLEX_OBJECT_IDX mfoi3 on mfoi3.med_def_flex_id = mdf2.med_def_flex_id)
 	...
 	;004
	, (inner join CODE_VALUE cv1 on cv1.code_value = mfoi3.parent_entity_id
	...
	, (left join BILL_ITEM_MODIFIER bim on bim.bill_item_id = bi.bill_item_id
		and bim.key1_id in (hcpcs1_var, cpt1_var, hosp1_var, hosp2_var, hosp3_var, hosp4_var) ;002 ;003
...
detail
	...
	pharm_data->list[cnt].default_route		= trim(uar_get_code_display(mod.route_cd), 3) ;005
	...
	pharm_data->list[cnt].strength			= mod.strength ;005
	pharm_data->list[cnt].strength_unit		= trim(uar_get_code_display(mod.strength_unit_cd, 3)) ;005
	pharm_data->list[cnt].volume			= mod.volume ;005
	pharm_data->list[cnt].volume_unit		= trim(uar_get_code_display(mod.volume_unit_cd, 3)) ;005
...
into value(output_var)
	...
	, DefaultRoute 				= pharm_data->list[d1.seq].default_route
	, DispItemID 				= pharm_data->list[d1.seq].item_id
;	, ServiceType				= " "
	, Facility					= pharm_data->list[d1.seq].facility
;	, InvItemID					= " "
	, Form						= pharm_data->list[d1.seq].form
	, Strength					= pharm_data->list[d1.seq].strength
	, StrengthUnit				= pharm_data->list[d1.seq].strength_unit
	, Volume					= pharm_data->list[d1.seq].volume
	, VolumeUnit				= pharm_data->list[d1.seq].volume_unit
	, GivenStrength				= pharm_data->list[d1.seq].given_strength
;	, Solution					= " "
;	, NdcProductID				= " "
;	, NDCItemID					= " "
