...
plan oros
	...
	and     oros.updt_dt_tm		>= cnvtdatetime(curdate,0)
join o
...
set t_rec->files.temp_path = build("/cerner/d_",cnvtlower(trim(curdomain)),"/cclscratch/")
...
set t_rec->files.final_filename = build(
...
call writeLog(build2("* START Moving File to AStream  ****************************"))
;t_rec->files.filename to t_rec->files.final_filename
set dclcom = build2(	"cp "
						,t_rec->files.filename
						," "
						,t_rec->files.final_filename
					)
call writeLog(build2(^dclcom="^,^cp ^,t_rec->files.filename,^ ^,t_rec->files.final_filename,^"^))
set dclstat = 0
call writeLog(build2("sending log file dclcom = ",dclcom))
call dcl(dclcom, size(trim(dclcom)), dclstat)
call writeLog(build2("* END   Moving File to AStream  ****************************"))
