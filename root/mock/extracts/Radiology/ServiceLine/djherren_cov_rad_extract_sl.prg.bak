...
drop program djherren_cov_rad_extract_sl:dba go
create program djherren_cov_rad_extract_sl:dba
...
;------------------------------------------------------------------------------
call echo(build("(Q7/10) *** GET RAD REPORT PRELIMINARILY RPT DELIVERY DATE TIME ***"))
select into "nl:"
...
from ORDER_RADIOLOGY ord
	...
	,(inner join ORDER_ACTION oa on oa.order_id = ord.parent_order_id
...
;		with nocounter, separator='|', format, format=stream
		with nocounter, pcformat (^^, ^|^, 1), format, format=stream, formfeed=none
	else
...
into value(OUTPUT_VAR)
	...
	,activate_dt						= format(t_rec->qual[d.seq].activate_dt_tm, "mm/dd/yyyy hh:mm;;d")
