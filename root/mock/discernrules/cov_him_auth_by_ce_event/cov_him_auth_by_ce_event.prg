...
******************************************************************************
	...
	Solution:			   
	Source file name:	   cov_him_auth_by_ce_event.prg
	Object name:		   cov_him_auth_by_ce_event
	Request #:
...
drop program cov_him_auth_by_ce_event:dba go
create program cov_him_auth_by_ce_event:dba
...
call echo(build2("* START Finding Documents **********************************"))
...
head ce.clinical_event_id
	cnt = cnt +1
	if(mod(cnt,100) = 1)
		stat = alterlist(temp_rec->qual, cnt +99)
	endif
;	call writeLog(build2("->Adding clinical_event_id=",trim(cnvtstring(ce.clinical_event_id))))
	call echo(build2("->Adding clinical_event_id=",trim(cnvtstring(ce.clinical_event_id))))
	temp_rec->qual[cnt].clinical_event_id = ce.clinical_event_id
foot report
	 stat = alterlist(temp_rec->qual, cnt)
	 temp_rec->cnt = cnt
;	 call writeLog(build2("-->Total to process:",trim(cnvtstring(temp_rec->cnt))))
	 call echo(build2("-->Total to process:",trim(cnvtstring(temp_rec->cnt))))
with nocounter

call echo(build2("* END   Finding Documents **********************************"))
...
call echo(build2("* START Sending Documents to EKS ***************************"))
...
;call writeLog(build2("Starting EKSOPSRequest calls:",trim(cnvtstring(temp_rec->cnt))))
call echo(build2("Starting EKSOPSRequest calls:",trim(cnvtstring(temp_rec->cnt))))
...
;	call writeLog(build2("-->Setting Expert Trigger to HIM_AUTH_DOCUMENTS"))
	call echo(build2("-->Looking at Item:",trim(cnvtstring(i))))
	call echo(build2("-->Setting Expert Trigger to HIM_AUTH_DOCUMENTS"))
	...
	from
		...
		clinical_event ce
	plan ce
		where ce.clinical_event_id = temp_rec->qual[i].clinical_event_id
...
;			trim(cnvtstring(EKSOPSRequest->qual[cnt].data[1].double_var))))
	...
	if (temp_rec->audit_mode != 1)
	 if (EKSOPSRequest->qual[cnt].person_id > 0.0)
;		call writeLog(build2("------>CALLING srvRequest"))
		call echo(build2("------>CALLING srvRequest"))
		call srvRequest(dparam)
		call pause(1)
	 endif
	else
;		call writeLog(build2("------>AUDIT MODE, Not calling srvRequest"))
		call echo(build2("------>AUDIT MODE, Not calling srvRequest"))
	endif
endfor


call echo(build2("* END   Sending Documents to EKS ***************************"))
...
;	set reply->status_data.subeventstatus[1].targetobjectname	= "CLINICAL_EVENT"
;	set reply->status_data.subeventstatus[1].targetobjectvalue	= "Documents Authenticated"
;endif
