...
;and ce.encntr_id = 125486659.00               
and ce.person_id = 20812082.00
;and ea.alias = '4000000186'
...
,pe.product_id, pe.product_event_id, pe.*, pr.*
...
select * from special_testing_result str
where str.result_id =    846020920.00
in(846018985.00,  846018989.00,   846020934.00)
;where str.product_id in(4212109.00,4212111.00,4212107.00,4212105.00,4212106.00)
 
select * from special_testing str where str.
where str.product_id in(4212109.00,4212111.00,4212107.00,4212105.00,4212106.00)

select * from TRANSFUSION str 
where str.product_event_id in(846019007.00,846019016.00,846018984.00,846018988.00,846018992.00,846018995.00)
;where str.person_id = 20812082.00
;where str.product_id in(4212109.00,4212111.00,4212107.00,4212105.00,4212106.00)


select * from person_trans_req ptr where ptr.encntr_id = 125486659.00
code set = 1611
use : transfusion_requirement table

   20812082.00	  125486659.00

select * from person_antibody pa where pa.encntr_id = 125486659.00
with nocounter, separator=" ", format, format(date,"mm-dd-yyyy hh:mm:ss;;d"), time = 180, uar_code(d,1), maxrow = 10000
ANTIBODY_DISPLAY
Anti-D
...
select * from person_antigen pa where pa.encntr_id = 125486659.00
with nocounter, separator=" ", format, format(date,"mm-dd-yyyy hh:mm:ss;;d"), time = 180, uar_code(d,1), maxrow = 10000
ANTIGEN_DISPLAY
D-
...
125486659.00
clinical_event - result
 21810661.00	Antibody ID	Anti-D
   21810663.00	Antigen Type	D-
   20136583.00	ABORh	O Neg



select * from procedure_specimen_type pst where pst.catalog_cd =    21286621.00
with nocounter, separator=" ", format, format(date,"mm-dd-yyyy hh:mm:ss;;d"), time = 180, uar_code(d,1), maxrow = 10000

select * from COLLECTION_INFO_QUALIFIERS CI 
where ci.specimen_type_cd = 1765.00 ;pst.specimen_type_cd
and ci.catalog_cd = 21286621.00

select * from person_aborh_result par where par.person_id = 20812082.00
SPECIMEN_ID		CONTAINER_ID	
  321807566.00	  321807567.00
  321806807.00	  321806808.00
  321806799.00	  321806801.00
...
select * from container_event ce where ce.container_id in(  321807567.00,  321806808.00,  321806801.00)

select * from specimen_usage su where su.specimen_id in(321807566.00,321806807.00,321806799.00)


select O.* from orders O where O.order_id =  4748415901.00 ;go


select * from order_detail od where od.order_id = 4748415901.00
with nocounter, separator=" ", format;, format(date,"mm-dd-yyyy hh:mm:ss;;d"), time = 180, uar_code(d,1), maxrow = 10000

BB_SPEC_EXPIRE_OVRD

select * from bb_spec_expire_ovrd ov where ov.specimen_id in(321807566.00,321806807.00,321806799.00)
