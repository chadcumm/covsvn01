...
if ($auth_flag = 1)
	...
	for (j = 1 to size(him_temp->qual,5))
		for (k = 1 to size(him_temp->qual[j].doc_qual,5))
			if (him_temp->qual[j].doc_qual[k].scanned_ind = 1)
				set clinical_event_id = him_temp->qual[j].doc_qual[k].clinical_event_id
