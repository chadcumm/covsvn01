...
******************************************************************************
	...
	Source file name:	cov_wh_BirthLogBookExtract_ops.prg
	Object name:		cov_wh_BirthLogBookExtract_ops
 
	Program purpose:
 
	Executing from:		CCL
 
 	Special Notes:		Copied from cov_wh_BirthLogBookExtract and developed
						for a weekly ops-job extract file for Spotfire.
...
DECLARE DELIVERY_ASSISTANCE_VAR		= f8 with constant(uar_get_code_by("DISPLAY",72,"Delivery Assistance:"))
...
;-----------------------------
set START_DATETIME_VAR = CNVTDATETIME("01-SEP-2022 0")
...
;;;		and pnh.name_type_cd = PREV_NAME_TYPE_VAR ;771
	...
	,(inner join CLINICAL_EVENT ce on ce.encntr_id = e.encntr_id and ce.person_id = e.person_id
		...
		and ce.event_cd in (
			...
			HOSPITAL_PEDIATRICIAN_VAR, FOLLOWUP_PEDIATRICIAN_VAR, RESUSCITATION_RN_1_VAR, DELIVERY_ASSISTANCE_VAR )
		and ce.valid_until_dt_tm > cnvtdatetime (curdate ,curtime3))
...
join ce where ce.encntr_id = rec->mother[d1.seq].encntr_id
	...
	and ce.event_cd in (
		DATETIME_BIRTH_VAR, DELIVERYTYPE_VAR, DELIVERY_ASSISTANCE_VAR, ROM_DT_VAR, ROM_TYPE_VAR, AMNIOTIC_FLUID_VAR,
...
head ce.event_id
	case (ce.event_cd)
		...
		of DELIVERY_ASSISTANCE_VAR		: rec->mother[d1.seq].baby[d2.seq].delivery_assistance		= ce.result_val
...
;;		,diagnosis						= substring(1,300,replace(replace(rec->mother[d.seq].diagnosis,
;;											char(13), " ", 0), char(10), " ", 0))
		...
		,delivery_assistance			= substring(1,100,rec->mother[d.seq].baby[d2.seq].delivery_assistance)
