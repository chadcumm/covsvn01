...
******************************************************************************
	...
	Request #:			3613, 4365, 4481, 5092, 5516, 6080, 8487, 8563, 8652, 
						8965, 9016, 9086, 11683, 12349, 13038, 13340
...
022	09/14/2021	Todd A. Blanchard		Adjusted criteria for health plans.
023	12/07/2021	Todd A. Blanchard		Added Covenant Health Diagnostics West to prompt.
024	03/08/2022	Todd A. Blanchard		Changed practice site display to org name.
025	06/08/2022	Todd A. Blanchard		Adjusted criteria for Request List Queue prompt.
026	07/29/2022	Todd A. Blanchard		Added order detail section for scheduling location.
										Revised sort order for results.
******************************************************************************/
 
drop program cov_sm_SchedResource_TEST:DBA go
create program cov_sm_SchedResource_TEST:DBA
...
declare scheduling_location_var			= f8 with constant(uar_get_code_by("DISPLAYKEY", 16449, "SCHEDULINGLOCATION")) ;026
...
from
	...
	, (left join ORGANIZATION org_ps on org_ps.organization_id = ps.organization_id) ;024
...
where
    ...
    and so.mnemonic_key not in ("*IT*USE*ONLY*", "DISPLACE*") ;014 ;025
	and so.active_ind = 1
...
;	sched_obj->list[cnt].earliest_dt_tm				= sen.earliest_dt_tm ;010 
	...
	sched_obj->list[cnt].ord_phys_group				= trim(org_ps.org_name, 3) ;024
...
from	
	...
	, (left join ORDER_DETAIL od on od.order_id = o.order_id
		...
		and od.action_sequence = (
			;019
			select max(m_od.action_sequence)
			from ORDER_DETAIL m_od
			where
				m_od.order_id = od.order_id
			 	and m_od.oe_field_id = od.oe_field_id
		))
	...
	, (left join ORDER_DETAIL od2 on od2.order_id = o.order_id
		...
		and od2.action_sequence = (
			;019
			select max(m_od2.action_sequence)
			from ORDER_DETAIL m_od2
			where
				m_od2.order_id = od2.order_id
				and m_od2.oe_field_id = od2.oe_field_id
		))
	...
	, (left join ORDER_DETAIL od3 on od3.order_id = o.order_id
		...
		and od3.action_sequence = (
			;019
			select max(m_od3.action_sequence)
			from ORDER_DETAIL m_od3
			where 
				m_od3.order_id = od3.order_id
				and m_od3.oe_field_id = od3.oe_field_id
		))
	...
	, (left join ORDER_DETAIL od4 on od4.order_id = o.order_id
		...
		and od4.action_sequence = (
			;019
			select max(m_od4.action_sequence)
			from ORDER_DETAIL m_od4
			where 
				m_od4.order_id = od4.order_id
				and m_od4.oe_field_id = od4.oe_field_id
		)) 
	...
	, (left join ORDER_DETAIL od5 on od5.order_id = o.order_id
		...
		and od5.action_sequence = (
			;019
			select max(m_od5.action_sequence)
			from ORDER_DETAIL m_od5
			where 
				m_od5.order_id = od5.order_id
				and m_od5.oe_field_id = od5.oe_field_id
		)) 
 	
 	;026
	, (left join ORDER_DETAIL od6 on od6.order_id = o.order_id
		and od6.oe_field_id = scheduling_location_var
		and od6.action_sequence = (
			select max(m_od6.action_sequence)
			from ORDER_DETAIL m_od6
			where 
				m_od6.order_id = od6.order_id
				and m_od6.oe_field_id = od6.oe_field_id
		))
...
head o.order_id
	...
	if (sea.order_status_meaning not in ("CANCELED", "COMPLETED", "DISCONTINUED"))
		...
		if (textlen(trim(sched_obj->list[idx].ord_phys_group, 3)) = 0)
			sched_obj->list[idx].ord_phys_group = trim(org_ps.org_name, 3) ;024
		endif
		...
		sched_obj->list[idx].scheduling_loc		= trim(od6.oe_field_display_value, 3) ;026
	...
	, (inner join ENCNTR_PLAN_RELTN epr on epr.encntr_id = sen.encntr_id
		and epr.priority_seq = (
			...
			where
				...
				and eprm.priority_seq > 0 ;022
				and eprm.active_ind = 1
		)
		and epr.beg_effective_dt_tm <= cnvtdatetime (curdate ,curtime3 )
		and epr.end_effective_dt_tm > cnvtdatetime (curdate ,curtime3 )
		and epr.priority_seq > 0 ;022
	...
	, (inner join PERSON_PLAN_RELTN ppr on ppr.person_id = sen.person_id
		and ppr.priority_seq = (
			...
			where
				...
				and pprm.priority_seq > 0 ;022
				and pprm.active_ind = 1
		)
		and ppr.beg_effective_dt_tm <= cnvtdatetime (curdate ,curtime3 )
		and ppr.end_effective_dt_tm > cnvtdatetime (curdate ,curtime3 )
		and ppr.priority_seq > 0 ;022
...
distinct into value(output_var)
	...
	, scheduling_location	= trim(sched_obj->list[d1.seq].scheduling_loc, 3) ;026
...
;	sched_obj->list[d1.seq].sch_obj_desc ;011
    ...
    elseif (cnvtupper(sched_obj->list[d1.seq].sch_obj_desc) in ("*HOLD*")) ;012
        build("2", sched_obj->list[d1.seq].sch_obj_desc) ;012
	else
