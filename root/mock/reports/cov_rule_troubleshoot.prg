...
select * from eks_module_audit_det emad where emad.encntr_id =  125359334
order by emad.updt_dt_tm
...
from eks_module_audit ema, eks_module_audit_det emad
where ema.rec_id = emad.module_audit_id
and cnvtupper(ema.module_name) = 'COV_MISC_COVID_TASK'
...
from eks_module_audit ema, eks_module_audit_det emad
where ema.rec_id = emad.module_audit_id
;and emad.encntr_id = 125348732
and cnvtupper(ema.module_name) = "COV_STK_ANTITHR_DSCHMED"
and emad.updt_dt_tm >= cnvtdatetime('04-MAR-2022 00:00:00')
order by emad.encntr_id, emad.updt_dt_tm 
with nocounter, separator=" ", format, uar_code(d,1), format(date,"mm-dd-yyyy hh:mm:ss;;d"), time = 180



select prsnl = pr.name_full_formatted, patient = p.name_full_formatted
, emad.encntr_id, ema.module_name, emad.logging, emad.updt_dt_tm, emad.updt_id, emad.template_type, emad.template_name
from eks_module_audit ema, eks_module_audit_det emad, prsnl pr, person p
where ema.rec_id = emad.module_audit_id
;and emad.encntr_id = 125348732
and cnvtupper(ema.module_name) = "COV_STK_ANTITHR_DSCHMED"
and emad.updt_dt_tm >= cnvtdatetime('04-MAR-2022 00:00:00')
