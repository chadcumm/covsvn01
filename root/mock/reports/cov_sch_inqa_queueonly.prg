...
CREATE PROGRAM cov_sch_inqa_queueonly :dba
 DECLARE oefieldid_authnumber = f8 WITH protect ,constant (uar_get_code_by ("DISPLAYKEY" ,16449 ,
   "SCHAUTHNUMBER" ) )
 DECLARE oefieldid_ord_physician = f8 WITH protect ,constant (uar_get_code_by ("DISPLAYKEY" ,16449 ,
   "SCHEDULING ORDERING PHYSICIAN" ) ) ;002
 DECLARE oefieldid_sch_instructions = f8 WITH protect ,constant (uar_get_code_by ("DISPLAYKEY" ,16449 ,
   "SCHEDULINGINSTRUCTIO12654" ) ) ;010
 DECLARE oefieldid_special_instructions = f8 WITH protect ,constant (uar_get_code_by ("DISPLAYKEY" ,16449 ,
   "SPECIALINSTRUCTIONS" ) ) ;010
 DECLARE oefieldid_requested_start_datetime = f8 WITH protect ,constant (uar_get_code_by ("DISPLAYKEY" ,16449 ,
   "REQUESTEDSTARTDATETIME" ) ) ;014
 ...
 IF ((cs23010_disppidrl_cd > 0.0 ) )
  ...
  ;end select
 ENDIF
 SET reply->attr_qual_cnt = 34 ; limit columns ;010 ;012
...
ENDFOR
 ...
 SELECT INTO "nl:"
  ...
  PLAN (d )
   JOIN (a
   WHERE (a.sch_event_id = reply->query_qual[d.seq ].hide#scheventid )
   AND (a.oe_field_id = oefieldid_ord_physician ) ;002
 ...
 SELECT INTO "nl:"
  ...
  PLAN (d
   ...
   AND (od.oe_field_id = outerjoin(oefieldid_requested_start_datetime)))
 ...
 SELECT INTO "nl:"
  ...
  PLAN (d )
   ...
   AND (od.oe_field_id = outerjoin(oefieldid_authnumber)))
   
   ;010
   JOIN (od2
   WHERE (od2.order_id = outerjoin(a.order_id))
   AND (od2.oe_field_id = outerjoin(oefieldid_sch_instructions)))
   
   ;010
   JOIN (od3
   WHERE (od3.order_id = outerjoin(a.order_id))
   AND (od3.oe_field_id = outerjoin(oefieldid_special_instructions)))
