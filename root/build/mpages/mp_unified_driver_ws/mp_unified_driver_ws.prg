...
DROP PROGRAM mp_unified_driver_ws :group1 GO
CREATE PROGRAM mp_unified_driver_ws :group1
 PROMPT
 ...
 SUBROUTINE  generatecontentrequirements (null )
  ...
  IF ((textlen (trim (criterion->workflow_base_url ) ) > 0 ) )
   SET javascriptreqs = build2 ('<script type="text/javascript">' ,'window.WORKFLOW_BASE_URL = "' ,
    ...
    criterion->workflow_base_url ,"/assets/workflow/workflow.js'></script>" ,javascriptreqs )
   SET cssreqs = build2 (^<link rel="stylesheet" type="text/css" href='^ ,criterion->
    workflow_base_url ,"/assets/workflow/workflow.css'></script>" ,cssreqs )
  ENDIF
