...
set replyString = build(^import { UIComponent, atomic, composite } from "MPageFusion";^)

set replyString = build(^(() => {^)
set replyString = build(^    const b = new atomic.button.Button({ display: "Hello World " });^)
set replyString = build(^    b.mount("root").update();^)
set replyString = build(^})();^)
