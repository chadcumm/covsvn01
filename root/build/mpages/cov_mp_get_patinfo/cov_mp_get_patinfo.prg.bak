...
(
	...
	1 qual[*]
	 ...
	 2 birth_sex			= vc
	 2 doc_received 		= vc
	 2 doc_received_pos_ind = i2
	 2 primary_poa_first	= vc
	 2 primary_poa_second	= vc
	 2 primary_poa_phone	= vc
	 2 secondary_poa_first	= vc
	 2 secondary_poa_second	= vc
	 2 secondary_poa_phone	= vc
	 2 primary_cons_first	= vc
	 2 primary_cons_second	= vc
	 2 primary_cons_phone	= vc
	 2 secondary_cons_first = vc
	 2 secondary_cons_second= vc
	 2 secondary_cons_phone	= vc
	 2 primary_legal_first	= vc
	 2 primary_legal_second	= vc
	 2 primary_legal_phone	= vc
	 2 secondary_legal_first= vc
	 2 secondary_legal_second= vc
	 2 secondary_legal_phone= vc
	 2 legal_documentation  = vc
	 2 n_selected			= vc
	 2 c_selected			= vc
	 2 g_selected			= vc
	 2 p_selected			= vc
...
if (patinfo->status = "S")
	...
	call GetPersonUserDefinedInfo(null)
...
subroutine GetEncounterInfo(null)
	...
	detail
		...
		patinfo->qual[cnt].disch_dt_tm				= e.disch_dt_tm
		patinfo->qual[cnt].disch_disposition_cd	 	= e.disch_disposition_cd
		if (patinfo->qual[cnt].disch_disposition_cd > 0.0)
			patinfo->qual[cnt].disch_disposition_d = uar_get_code_display(e.disch_disposition_cd)
		else
			patinfo->qual[cnt].disch_disposition_d = "-N/A-"
		endif
	foot report
		patinfo->status = "S"
	with nocounter
...
subroutine GetPersonUserDefinedInfo(null)
select into "nl:"
from 
	person_info pi
	,long_text lt
plan pi
	where pi.person_id = criterion->person_id
join lt
	where lt.long_text_id = pi.long_text_id
head report
	cnt = 0
detail
	patinfo->person_info_cnt = (patinfo->person_info_cnt + 1)
	stat = alterlist(patinfo->person_info,patinfo->person_info_cnt)
	patinfo->person_info[patinfo->person_info_cnt].label	 	= uar_get_code_display(pi.info_sub_type_cd)
	patinfo->person_info[patinfo->person_info_cnt].value		= lt.long_text
	patinfo->person_info[patinfo->person_info_cnt].table_value	= uar_get_code_display(pi.value_cd)
with nocounter

for (i = 1 to patinfo->person_info_cnt)
	case (patinfo->person_info[i].label)
		of "Primary POA First Name": set patinfo->qual[1].primary_poa_first = patinfo->person_info[i].value
		of "Primary POA Last Name": set patinfo->qual[1].primary_poa_second = patinfo->person_info[i].value
		of "Primary POA Phone": set patinfo->qual[1].primary_poa_phone = patinfo->person_info[i].value
		
		of "Primary Legal Guardian First Name": set patinfo->qual[1].primary_legal_first = patinfo->person_info[i].value
		of "Primary Legal Guardian Last Name": set patinfo->qual[1].primary_legal_second = patinfo->person_info[i].value
		of "Primary Legal Guardian Phone": set patinfo->qual[1].primary_legal_phone = patinfo->person_info[i].value
		
		of "Primary Conservator First Name": set patinfo->qual[1].primary_cons_first = patinfo->person_info[i].value
		of "Primary Conservator Last Name": set patinfo->qual[1].primary_cons_second = patinfo->person_info[i].value
		of "Primary Conservator Phone": set patinfo->qual[1].primary_cons_phone = patinfo->person_info[i].value
		
		of "Secondary POA First Name": set patinfo->qual[1].secondary_poa_first = patinfo->person_info[i].value
		of "Secondary POA Last Name": set patinfo->qual[1].secondary_poa_second = patinfo->person_info[i].value
		of "Secondary POA Phone": set patinfo->qual[1].secondary_poa_phone = patinfo->person_info[i].value
		
		of "Secondary Legal Guardian First Name": set patinfo->qual[1].secondary_legal_first = patinfo->person_info[i].value
		of "Secondary Legal Guardian Last Name": set patinfo->qual[1].secondary_legal_second = patinfo->person_info[i].value
		of "Secondary Legal Guardian Phone": set patinfo->qual[1].secondary_legal_phone = patinfo->person_info[i].value
		
		of "Secondary Conservator First Name": set patinfo->qual[1].secondary_cons_first = patinfo->person_info[i].value
		of "Secondary Conservator Last Name": set patinfo->qual[1].secondary_cons_second = patinfo->person_info[i].value
		of "Secondary Conservator Phone": set patinfo->qual[1].secondary_cons_phone = patinfo->person_info[i].value
	
		of "Legal Rel Documentation": set patinfo->qual[1].legal_documentation = patinfo->person_info[i].table_value	
		; 3162627233.00	Legal Relationship Documentation	LGLREL_DOCUM

	endcase
endfor

case (patinfo->qual[1].legal_documentation)
	of "None": set patinfo->qual[1].n_selected = "selected"
	of "Conservator": set patinfo->qual[1].c_selected = "selected"
	of "Legal Guardian": set patinfo->qual[1].g_selected = "selected"
	of "POA": set patinfo->qual[1].p_selected = "selected"
endcase
	...
	if (patinfo->qual[i].encntr_id > 0.0)
		...
		if (patinfo->qual[i].disch_dt_tm > 0.0)
			set replyString = build(replyString,"<tr><td><b>Discharge DT/TM</b></td><td>&nbsp</td><td>",
						...
						"</td></tr>")
			set replyString = build(replyString,"<tr><td><b>Disposition</b></td><td>&nbsp</td><td>",
											trim(patinfo->qual[i].disch_disposition_d),
						"</td></tr>")
		endif
		...
		if (patinfo->qual[i].primary_cons_first > " ")
			set replyString = build2(replyString,"<tr><td><b>Primary Conservator</b></td><td>&nbsp</td><td>",
											trim(patinfo->qual[i].primary_cons_first)," ",
											trim(patinfo->qual[i].primary_cons_second)," (",
											format(trim(patinfo->qual[i].primary_cons_phone),""),")",
						"</td></tr>")
		endif
		
		if (patinfo->qual[i].secondary_cons_first > " ")
			set replyString = build2(replyString,"<tr><td><b>Secondary Conservator</b></td><td>&nbsp</td><td>",
											trim(patinfo->qual[i].secondary_cons_first)," ",
											trim(patinfo->qual[i].secondary_cons_second)," (",
											format(trim(patinfo->qual[i].secondary_cons_phone),""),")",
						"</td></tr>")
		endif

		if (patinfo->qual[i].primary_legal_first > " ")
			set replyString = build2(replyString,"<tr><td><b>Primary Legal Guardian</b></td><td>&nbsp</td><td>",
											trim(patinfo->qual[i].primary_legal_first)," ",
											trim(patinfo->qual[i].primary_legal_second)," (",
											format(trim(patinfo->qual[i].primary_legal_phone),""),")",
						"</td></tr>")
		endif

		if (patinfo->qual[i].secondary_legal_first > " ")
			set replyString = build2(replyString,"<tr><td><b>Secondary Legal Guardian</b></td><td>&nbsp</td><td>",
											trim(patinfo->qual[i].secondary_legal_first)," ",
											trim(patinfo->qual[i].secondary_legal_second)," (",
											format(trim(patinfo->qual[i].secondary_legal_phone),""),")",
						"</td></tr>")
		endif

		if (patinfo->qual[i].primary_poa_first > " ")
			set replyString = build2(replyString,"<tr><td><b>Primary Power of Attorney</b></td><td>&nbsp</td><td>",
											trim(patinfo->qual[i].primary_poa_first)," ",
											trim(patinfo->qual[i].primary_poa_second)," (",
											format(trim(patinfo->qual[i].primary_poa_phone),""),")",
						"</td></tr>")
		endif

		if (patinfo->qual[i].secondary_poa_first > " ")
			set replyString = build2(replyString,"<tr><td><b>Secondary Power of Attorney</b></td><td>&nbsp</td><td>",
											trim(patinfo->qual[i].secondary_poa_first)," ",
											trim(patinfo->qual[i].secondary_poa_second)," (",
											format(trim(patinfo->qual[i].secondary_poa_phone),""),")",
						"</td></tr>")
		endif		
		set replyString = build2(replyString,"<tr><td><b>Documentation Received</b></td><td>&nbsp</td><td>",
											"<select disabled>",
											"<option value=N ",patinfo->qual[1].n_selected,">None",
											"<option value=C ",patinfo->qual[1].c_selected,">Conservator",
											"<option value=G ",patinfo->qual[1].g_selected,">Legal Guardian",
											"<option value=P ",patinfo->qual[1].p_selected,">Power of Attorney",
											"</select>",
						"</td></tr>")
	endif
endfor
