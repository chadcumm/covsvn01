...
join ppr
	where 	ppr.encntr_id = e.person_id
...
head ppr.person_prsnl_r_cd
	case (uar_get_code_meaning(ppr.person_prsnl_r_cd))
		of "ATTENDDOC": patinfo->qual[1].attending_name_first = p1.name_first
				  		patinfo->qual[1].attending_name_last = p1.name_last
		of "REFERDOC": patinfo->qual[1].attending_name_first = p1.name_first
				  		patinfo->qual[1].attending_name_last = p1.name_last
		of "ADMITDOC": patinfo->qual[1].attending_name_first = p1.name_first
				  		patinfo->qual[1].attending_name_last = p1.name_last
	endcase
with nocounter
...
/*		if ((patinfo->qual[i] > " ") or (patinfo->qual[i].attending_name_last > " "))
			set replyString = build(replyString,"<tr><td><b>Attending</b></td><td>&nbsp</td><td>",
							concat(trim(patinfo->qual[i].attending_name_first)," ",trim(patinfo->qual[i].attending_name_last)),
						"</td></tr>")
		endif
