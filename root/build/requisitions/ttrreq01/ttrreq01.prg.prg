...
join oef
	...
	and   oef.description in ("Isolation Reason","Isolation Code")
order by
    cnvtdatetime(o.orig_order_dt_tm) desc,
    od.order_id, od.oe_field_id, od.action_sequence desc
 head report
  	cnt = 0
 head od.order_id
	row + 0
    ...
    orders->isolation[orders->isolation_cnt].order_name = "Isolation" ;trim(ocs.mnemonic)
    orders->isolation[orders->isolation_cnt].order_id = o.order_id
 detail
 case (oef.description)
    	of "Isolation Code": orders->isolation[orders->isolation_cnt].isolation_status = trim(od.oe_field_display_value)
    	of "Isolation Reason": orders->isolation[orders->isolation_cnt].isolation_reason = trim(od.oe_field_display_value)
    endcase
with nocounter
...
order by
 ...
 detail
    ...
    orders->code_status[orders->code_status_cnt].code_status_disp = trim(od.oe_field_display_value)
    orders->code_status[orders->code_status_cnt].order_name = "Code Status"; trim(ocs.mnemonic)
    orders->code_status[orders->code_status_cnt].order_id = o.order_id
with nocounter
...
;002        orders->qual[ord_cnt].mnemonic = concat(trim(orders->qual[ord_cnt].mnemonic),"(",trim(o.order_mnemonic),")")
  ...
  */
  if ((oa.action_type_cd in (order_cd,modify_cd)) AND o.encntr_id>0 AND o.template_order_flag != 7) 
...
head page
    ...
    "{CPI/9}{POS/0/64}{B}" call CENTER(trim(orders->org_name),1,200), row+1 
  	"{CPI/9}{POS/0/80}{B}" call CENTER(trim(orders->org_street_add),1,200), row+1 
  	"{CPI/9}{POS/0/96}{B}" call CENTER(trim(orders->org_city_state_zip),1,200), row+1 
    ...
    "{CPI/15}{POS/85/285}",     call print(concat(orders->code_status[1].order_name,":")), row+1
    "{CPI/15}{POS/145/285}{B}", call print(orders->code_status[1].code_status_disp), "{ENDB}", row+1
    ...
    ;else
    	"{CPI/15}{POS/85/295}",call print(concat(orders->isolation[1].order_name,":"))
    	"{CPI/15}{POS/145/295}{B}", call print(concat(
    				orders->isolation[1].isolation_reason,"; ",
    				orders->isolation[1].isolation_status)), "{ENDB}", row+1
    ;endif
