...
-------	 -------    -----------         ------------------------
21.0	6/09/2018	Susmita Ray			Creation of chstnreq04
22.0	6/20/2018						Modifications for future order location and MA workflow
23.0    7/31/2018   Dawn Greer, DBA     Removing Diagnosis Code which after the Communication type
                                        for the order. We are leaving the Diagnosis codes that are tied
                                        to the order
24.0	9/11/2018	Dawn Greer, DBA		Add AMBPHYSICIANORTHOPAEDIC in the list of
										Physicians Positions
25.0    9/12/2018   Dawn Greer, DBA     Added order action type of Status Change and Complete
26.0    9/25/2018   Dawn Greer, DBA     Core Team changed Position Text from "AMBPHYSICIANORTHOPAEDIC"
                                        to "AMBULATORYPHYSICIANORTHOPAEDIC" and thus the Req had to
                                        be updated
27.0    9/27/2018   Dawn Greer, DBA     Allow the users to print requisition for all statuses except cancel.
**************************************************************/
...
;026 - DG - Core Team changed the Display Key for this position
;declare 88_AMBPHYSICIANORTHOPAEDIC_CD            	 = f8 with protect, constant(uar_get_code_by	;024 - Adding position to list
;                                                       ("DISPLAYKEY", 88, "AMBPHYSICIANORTHOPAEDIC"))	;024
declare 88_AMBPHYSICIANORTHOPAEDIC_CD            	 = f8 with protect, constant(uar_get_code_by	;026 - Changing the text
                                                       ("DISPLAYKEY", 88, "AMBULATORYPHYSICIANORTHOPAEDIC"))	;026
...
join e
  where e.encntr_id = outerjoin(request->order_qual[1].encntr_id)
 ;      856.00    ;021
...
; p12 - signed by
 ...
 if(op.communication_type_cd = 6006_COSIGNED_REQ_CD)
    ...
    if(pl3.position_cd in (88_AMBULATORYCLINICALOFFICEMANAGER_CD
                          ,88_AMBULATORYLPN_CD
   ...
   and pl2.POSITION_CD in (88_AMBPHYSICIANORTHOPAEDIC_CD
   						  ,88_PHYSICIANCARDIOLOGY_CD
  ...
  ;v22.0 - Order sent for cosign, elctronically signed by should be blank
                   (ono.notification_status_flag = 1
                   and pl2.POSITION_CD in (
                   		88_AMBPHYSICIANORTHOPAEDIC_CD,
                        88_PHYSANESTHESIOLOGY_CD,
                        88_PHYSCOLONANDRECTALSURGERY_CD,
                        88_PHYSCARDIOVASCULARSURGERY_CD,
...
;021-new logic for printing
  if(o.CATALOG_TYPE_CD in (6000_AMBULATORYPROCEDURES_CD,6000_Radiology_cd)
  ;025 - DG - Added Status_Change_cd and Complete_cd to the list
  AND (oa.action_type_cd in (order_cd,modify_cd,discont_cd,activate_cd,status_change_cd,complete_cd))
...
#exit_script
set last_mod = "025 09/12/2018 DG   changes mod and declared a variable"
end
go
