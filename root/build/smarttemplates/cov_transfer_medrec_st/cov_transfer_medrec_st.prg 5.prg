...
if ((510001_REPLY->transaction_status = 1) and (size(510001_REPLY->continued_medications,5) > 0))
	set idx = 1
	set stat = alterlist(drec->line_qual, idx)
	drec->line_qual[idx].disp_line = CONCAT(rhead,rh2bu,"NEED TITLE FOR SECTION",wr,reol)
	...
	HEAD REPORT
	 ...
	 drec->line_qual[1].disp_line = CONCAT(rhead,rh2bu,"NEED TITLE FOR EMPTY SECTION",wr,reol)
	 drec->line_qual[2].disp_line = CONCAT(wr,"******* No results Qualified ********",REOL)
	WITH nocounter
endif
