...
if (size(ordersRec->orders_list,5) > 0)
	...
	set drec->line_qual[idx].disp_line = CONCAT(rhead,rh2bu,"TITLE NEEDED Medications",wr,reol)
	for (i=1 to size(ordersRec->orders_list,5))
		set idx = (idx + 1)
		set stat = alterlist(drec->line_qual, idx)
		set drec->line_qual[idx].disp_line = CONCAT(
														wb,
														ordersRec->orders_list[i].mnemonic,
														" ",
														wr,
														ordersRec->orders_list[i].clinical_display_line ,
														REOL)
		set idx = (idx + 1)
		set stat = alterlist(drec->line_qual, idx)
...
set reply->status_data.status = "S"
set reply->status_data.subeventstatus[1].operationname = "TEMPLATE"
set reply->status_data.subeventstatus[1].operationstatus = "S"
set reply->status_data.subeventstatus[1].targetobjectname = "TEMPLATE"
set reply->status_data.subeventstatus[1].targetobjectvalue = "Template created"
