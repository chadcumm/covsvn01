...
if (t_rec->qual.note_type_string > " ")
	...
	detail
		cnt = (cnt + 1)
		stat = alterlist(t_rec->note_type,cnt)
		t_rec->note_type[cnt].note_type_cd = cv.code_value
	foot report
		t_rec->note_type_cnt = cnt
	with nocounter
	if	(t_rec->note_type_cnt = 0)
...
plan ce
	...
	and	  expand(x,1,t_rec->note_type_cnt,ce.event_cd,t_rec->note_type[x].note_type_cd)
order by
