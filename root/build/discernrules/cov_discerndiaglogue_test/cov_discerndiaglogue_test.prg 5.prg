...
if(size(trim(reflect(parameter(2,0))),1) > 0)
  set r_rec->param_encntr_id = cnvtint(value(parameter(2,0)))
endif

if (r_rec->param_encntr_id <= 0.0)
	set r_rec->encntr_parse = build("and e.encntr_id = ",r_rec->param_encntr_id)
else
	set r_rec->encntr_parse = "and e.encntr_id > 0.0"
endif
