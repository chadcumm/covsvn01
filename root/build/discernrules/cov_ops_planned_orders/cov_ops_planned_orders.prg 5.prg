	...
	for (j=1 to t_rec->qual[i].orders_cnt)
		...
		if (t_rec->qual[i].orders[j].level_ind = 1)
			...
			call writeLog(build2("------>age       = ",cnvtstring(t_rec->qual[i].orders[j].age)))
				...
				set EKSOPSRequest->qual[cnt].data[1].vc_var = build2(
																		trim(cnvtstring(t_rec->qual[i].orders[j].pathway_id))	,"|",
																		format(t_rec->qual[i].orders[j].order_dt_tm,";;q")		,"|",
																		trim(cnvtstring(t_rec->qual[i].orders[j].description))
																		)
