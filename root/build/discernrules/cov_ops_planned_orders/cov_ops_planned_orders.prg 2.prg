...
call writeLog(build2("* START Setting Parameters **********************************"))

set t_rec->medical_duration 	= (60 * 4)
set t_rec->critical_duration	= (60 * 1)
set t_rec->compare_dt_tm		= cnvtdatetime(curdate,curtime3)

call writeLog(build2("* END   Setting Parameters **********************************"))
...
if (program_log->run_from_ops = 1)
	...
	call writeLog(build2("->not in ops job, looking for: ",$LOC_DISPLAY))
...
join pc
	...
	and		expand(j,1,t_rec->plan_cnt,pc.pathway_catalog_id,t_rec->plan_qual[j].pathway_catalog_id)
...
head pc.pathway_catalog_id
	...
	t_rec->qual[t_rec->qual_cnt].orders[t_rec->qual[t_rec->qual_cnt].orders_cnt].description	= p.description
	t_rec->qual[t_rec->qual_cnt].orders[t_rec->qual[t_rec->qual_cnt].orders_cnt].level_ind		= t_rec->plan_qual[pos].level_ind
...
for (i=1 to t_rec->qual_cnt)
	...
	for (j=1 to t_rec->qual[i].orders_cnt)
		...
		if (t_rec->qual[i].orders[j].level_ind = 1)
			...
			if (t_rec->qual[i].orders[j].age >= t_rec->critical_duration)
				call writeLog(build2("--------->plan is ",cnvtstring(t_rec->qual[i].orders[j].age)," which is over ",cnvtstring(
																										t_rec->critical_duration)))
				set cnt = (cnt + 1)
				...
				set stat = alterlist(EKSOPSRequest->qual[cnt].data,1)
				set EKSOPSRequest->qual[cnt].data[1].vc_var = format(t_rec->qual[i].orders[j].order_dt_tm,";;q")
				set t_rec->qual[i].send_ind = 2
			endif
		endif
	endfor
endfor
...
set stat = initrec(EKSOPSRequest)
set EKSOPSRequest->expert_trigger = "COV_EE_ED_PLANNED_ORDERS_MEDICAL"
call writeLog(build2("->EKSOPSRequest->expert_trigger =",EKSOPSRequest->expert_trigger)) 
set cnt = 0
for (i=1 to t_rec->qual_cnt)
	for (j=1 to t_rec->qual[i].orders_cnt) 
		call writeLog(build2("---->Reviewing ",cnvtstring(j)," of ",cnvtstring(t_rec->qual[i].orders_cnt)))
		if ((t_rec->qual[i].orders[j].level_ind = 0) and (t_rec->qual[i].send_ind < 2))
			call writeLog(build2("------>level_ind = ",cnvtstring(t_rec->qual[i].orders[j].level_ind)))
			call writeLog(build2("------>age       = ",cnvtstring(t_rec->qual[i].orders[j].age)))
			if (t_rec->qual[i].orders[j].age >= t_rec->medical_duration)
				call writeLog(build2("--------->plan is ",cnvtstring(t_rec->qual[i].orders[j].age)," which is over ",cnvtstring(
																										t_rec->medical_duration)))
				set cnt = (cnt + 1)
				set stat = alterlist(EKSOPSRequest->qual,cnt)
				set EKSOPSRequest->qual[cnt].person_id = t_rec->qual[i].person_id
				set EKSOPSRequest->qual[cnt].encntr_id = t_rec->qual[i].encntr_id
...
call writeLog(build2("* START Planned EKSOPSRequest  *****************************"))
set stat = initrec(EKSOPSRequest)
set EKSOPSRequest->expert_trigger = "COV_EE_ED_PLANNED_ORDERS_PLANNED"
call writeLog(build2("->EKSOPSRequest->expert_trigger =",EKSOPSRequest->expert_trigger)) 
set cnt = 0
for (i=1 to t_rec->qual_cnt)
	for (j=1 to t_rec->qual[i].orders_cnt)
		call writeLog(build2("---->Reviewing ",cnvtstring(j)," of ",cnvtstring(t_rec->qual[i].orders_cnt)))
		if (t_rec->qual[i].orders[j].level_ind in(0,1) and (t_rec->qual[i].send_ind < 1))
			call writeLog(build2("------>level_ind = ",cnvtstring(t_rec->qual[i].orders[j].level_ind)))
			set cnt = (cnt + 1)
			set stat = alterlist(EKSOPSRequest->qual,cnt)
			set EKSOPSRequest->qual[cnt].person_id = t_rec->qual[i].person_id
...
call writeLog(build2("* END   Planned EKSOPSRequest  ******************************"))
