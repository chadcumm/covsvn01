...
******************************************************************************
	...
	Solution:			     
	Source file name:	 cov_eks_add_antic_document.prg
	Object name:		   cov_eks_add_antic_document
	Request #:
...
002     07/02/2019	Chad Cummings			added physician indicator to ED DOC
003	    08/27/2019	Chad Cummings			Corrected periop doc to ce issue
******************************************************************************/
...
if (DEFICIENT_PERSONNEL = "SURGDOC")
	set ClinicalEventID  =cnvtreal(eksdata->tqual[3]->qual[2]->data[2].misc)
	set logmsg = eksdata->tqual[3]->qual[2]->data[2].misc
	...
	join pd
		;003 where pd.periop_doc_id = cnvtreal(substring(1,9,ce.reference_nbr))
		where pd.periop_doc_id = cnvtreal(substring(1,(findstring("SN",ce.reference_nbr)-1),ce.reference_nbr)) ;003
	join scp
	...
	detail
		performed_prsnl_id = scp.primary_surgeon_id
		action_prsnl_id = scp.primary_surgeon_id
		logmsg = build2(p.person_id," ",p.name_full_formatted)
	with nocounter
endif
...
if ((DEFICIENT_PERSONNEL not in("DISCHDOC","EDDOC","SURGDOC")) or (performed_prsnl_id = 0.0))
	select into "nl:"
  ...
  if (performed_prsnl_id = 0.0)
    ...
    elseif (DeficientPrsnl = "SURGDOC")
      call echo(build("Unable to find Primary Surgical Physician for encntr_id = ", EncntrId))
      set logmsg = "Unable to find Primary Surgical Physician"
...
if (DeficientPrsnl = "ADMITDOC")
  ...
  elseif (DEFICIENT_PERSONNEL = "EDDOC")
   ...
   elseif (DEFICIENT_PERSONNEL = "SURGDOC")
    call echo(build("SURGDOC FOUND : PRSNL_ID :", performed_prsnl_id))
  else
  	call echo(build("ATTENDING FOUND : PRSNL_ID :", performed_prsnl_id))
  endif
endif

if (DEFICIENT_PERSONNEL in("SURGDOC"))
	execute cov_eks_add_antic_doc_core
else
