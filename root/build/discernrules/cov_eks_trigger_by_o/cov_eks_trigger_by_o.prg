...
******************************************************************************
	...
	Solution:			   
	Source file name:	   cov_eks_trigger_by_o.prg
	Object name:		   cov_eks_trigger_by_o
	Request #:
	...
	Executing from:		CCL
 	...
 	Special Notes:		WILL NOT WORK WITH FUTURE ORDERS AS IS
...
drop program cov_eks_trigger_by_o:dba go
create program cov_eks_trigger_by_o:dba
...
head o.order_id
	...
	call writeLog(build2("->Adding o.order_id=",trim(cnvtstring(o.order_id))))
...
if (t_rec->cnt = 0)
	if (program_log->run_from_ops = 1)
		...
		set reply->status_data.subeventstatus[1].operationname		= "ORDERS"
		set reply->status_data.subeventstatus[1].operationstatus	= "Z"
		set reply->status_data.subeventstatus[1].targetobjectname	= "ORDERS"
		set reply->status_data.subeventstatus[1].targetobjectvalue	= "No Orders qualified"
	endif
	go to exit_script
endif
call writeLog(build2("************************************************************"))
...
for (i = 1 to t_rec->cnt)
	...
	from
		...
		orders o
	plan o
		where o.order_id = t_rec->qual[i].order_id
	...
	detail
		...
		EKSOPSRequest->qual[cnt].data[1].double_var = o.order_id
...
if (program_log->run_from_ops = 1)
	...
	set reply->status_data.subeventstatus[1].targetobjectname	= "ORDERS"
	set reply->status_data.subeventstatus[1].targetobjectvalue	= "Events Sent"
endif
