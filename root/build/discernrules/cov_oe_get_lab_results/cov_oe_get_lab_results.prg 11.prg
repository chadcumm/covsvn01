...
set 1000001_REQUEST->person_id = t_rec->patient.person_id
set 1000001_REQUEST->order_id = t_rec->patient.order_id
...
plan ce	
	...
	and   ce.result_val        >  " "
detail
	t_rec->result_cnt = (t_rec->result_cnt + 1)
	stat = alterlist(t_rec->result_qual,t_rec->result_cnt)
	t_rec->result_qual[t_rec->result_cnt].clinical_event_id = ce.clinical_event_id
	t_rec->result_qual[t_rec->result_cnt].trigger_name = "COV_POC_UPDT_SRV_R"
with nocounter

if (t_rec->result_cnt = 0)
	set t_rec->return_value = "FALSE"
	go to exit_script
endif
call echo("executing cov_eks_trigger_by_ce")
for (i= 1 to t_rec->result_cnt)
	call echo(build2(^execute cov_eks_trigger_by_ce "MINE",^
		,trim(t_rec->result_qual[i].trigger_name),^,^
		,trim(cnvtstring(t_rec->result_qual[i].clinical_event_id))
		))
	execute cov_eks_trigger_by_ce 	"MINE",
									t_rec->result_qual[i].trigger_name,
									t_rec->result_qual[i].clinical_event_id
endfor
