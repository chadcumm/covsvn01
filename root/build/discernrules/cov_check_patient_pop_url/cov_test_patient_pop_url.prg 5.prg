...
set t_rec->req_type_cd = uar_Get_code_by("MEANING",12600,"CPS_OPN_CHT")
...
head e.encntr_id
	...
	t_rec->qual[t_rec->cnt].birth_Dt_tm = p.birth_dt_tm
	t_rec->qual[t_rec->cnt].Alert_Titlebar = concat("Open Chart - ",trim(p.name_full_formatted))
with nocounter, maxqual(ed,10)
