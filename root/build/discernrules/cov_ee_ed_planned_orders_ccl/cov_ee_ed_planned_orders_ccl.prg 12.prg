...
head ed.encntr_id
	...
	t_rec->qual[cnt].person_id	= ed.person_id
...
head p.pathway_id
	...
	i 		= (i + 1)
	stat = alterlist(t_rec->qual[cnt].orders,i)
	t_rec->qual[cnt].orders[i].pathway_id	= p.pathway_id
	t_rec->qual[cnt].orders[i].description	= p.description
	t_rec->qual[cnt].orders[i].level_ind	= t_rec->plan_qual[pos].level_ind
	t_rec->qual[cnt].orders[i].order_dt_tm	= p.order_dt_tm
foot ed.encntr_id
	i = 0
foot report
	t_rec->qual_cnt = cnt
...
for (i=1 to t_rec->qual_cnt)
	if (t_rec->qual[i].critical_ind = 1)
		...
		set EKSOPSRequest->qual[cnt].data[1].vc_var = format(t_rec->qual[i].crit_order_dt_tm,";;q")
	endif
endfor
...
set cnt = 0
for (i=1 to t_rec->qual_cnt)
	if (t_rec->qual[i].medical_ind = 1)
		set cnt = (cnt + 1)
		set stat = alterlist(EKSOPSRequest->qual,cnt)
		set EKSOPSRequest->qual[cnt].person_id = t_rec->qual[i].person_id
		set EKSOPSRequest->qual[cnt].encntr_id = t_rec->qual[i].encntr_id
		set stat = alterlist(EKSOPSRequest->qual[cnt].data,1)
		set EKSOPSRequest->qual[cnt].data[1].vc_var = format(t_rec->qual[i].med_order_dt_tm,";;q")
	endif
endfor
if (size(EKSOPSRequest->qual,5) > 0)
	set dparam = 0
