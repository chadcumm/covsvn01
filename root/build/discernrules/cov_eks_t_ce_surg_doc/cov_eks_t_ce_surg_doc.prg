...
;    *012 08/27/19 ccummin4				Corrected periop doc to ce issue    *
;DE~*************************************************************************
			...
			if (GetLastResults(gPersonID, gEncounterID) = 0)
				...
				call WriteExitMessage(
					ConCat("No results were found matching EVENT_SET_NAME(s) ",
							...
							" and person=",cnvtstring(gPersonID)," prsnl=",cnvtstring(gPrimarySurgID),
							" on ce_id=",cnvtstring(gClinicalEventID)))
			else
...
subroutine	GetLastResults(cePersonID, ceEncntrID)
	...
	if (bAnyEventSet = False)
		select
			;011 if (linkTo = 0)
			/* 011	from v500_event_set_explode ex, clinical_event ce
		...
		from v500_event_set_explode ex, clinical_event ce
				...
				join ce where ce.person_id = cePersonID
				    ...
				    and ce.publish_flag = 1
					and ce.event_class_cd != ptPlaceHolder
					and ((ce.verified_prsnl_id = gPrimarySurgID) or (ce.performed_prsnl_id = gPrimarySurgID))
		/* 011 */
...
subroutine GetParentLink (linkRequired)
	...
	elseif (CnvtInt(OPT_LINK) > 0)
		...
		call WriteMessage(FAILED, "No link specified.  Failing.")
	endif
...
subroutine	GetPersonID (optLinkTo)
  ...
  select into "nl:"
	...
	join pd
		;012 where pd.periop_doc_id = cnvtreal(substring(1,9,ce.reference_nbr))
		where pd.periop_doc_id = cnvtreal(substring(1,(findstring("SN",ce.reference_nbr)-1),ce.reference_nbr)) ;012
	join scp
