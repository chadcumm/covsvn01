...
drop program cov_him_remove_by_ce:dba go
create program cov_him_remove_by_ce:dba
...
if(size(trim(reflect(parameter(1,0))),1) > 0)
  set t_rec->audit_mode = cnvtint(value(parameter(1,0)))
endif
call writeLog(build2("-->AUDIT MODE:",trim(cnvtstring(t_rec->audit_mode))))
...
plan ce
	...
	and   ce.event_end_dt_tm <= cnvtdatetime(t_rec->start_dt_tm)
...
for (i = 1 to t_rec->cnt)
	...
	call writeLog(build2("-->Setting Expert Trigger to HIM_REMOVE_DOCUMENTS"))
