...
; 		endif
		...
		if (orec.recon_type_flag = 3)
 		...
 		elseif (orec.recon_type_flag in (1,2))
 			if (medrecqual =1 ) ;attending physician
 				if (orec.recon_status_cd IN	(pendcomp, partial, notstarted, pendpart))	
 					retval = 100
 					log_misc1 = cnvtstring(orec.recon_type_flag)
 				elseif (orec.recon_status_cd = complete AND cnt = 1)
 					compsts = 1	
 				endif
 			elseif (medrecqual = 0 AND physind = 0) ;nurse
	 			if (orec.recon_status_cd IN	(pendcomp, partial))
