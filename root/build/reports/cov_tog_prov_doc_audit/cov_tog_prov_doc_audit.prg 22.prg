...
head ce.encntr_id	
	call writeLog(build2("-->Reviewing encounter t_rec->qual[",trim(cnvtstring(d1.seq)),"].encntr_id = ",trim(cnvtstring(e.
		encntr_id))," for documents"))
head ce.event_cd
	call writeLog(build2("---->Document Type ",trim(uar_get_code_display(ce.event_cd)),
		" (",trim(cnvtstring(ce.event_cd)),") found on: ",format(ce.event_end_dt_tm,";;g")," (",trim(cnvtstring(ce.event_id)),")"))
