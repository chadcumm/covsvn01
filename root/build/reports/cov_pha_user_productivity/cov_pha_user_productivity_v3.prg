...
************************************************************/
drop program cov_pha_user_productivity_v3 go
create program cov_pha_user_productivity_v3
 
prompt
	"Output to File/Printer/MINE" = "MINE"   ;* Enter or select the printer or file name to send this report to.
	, "Select Facility" = 0
	, "Select Position" = VALUE(1.0)
	, "Select User" = 0
	, "Select Begin Date/Time" = "SYSDATE"
	, "Select End Date/Time" = "SYSDATE"
 
with OUTDEV, facility, position, Username, begdate, enddate
...
CONSTANT(BUILD('cer_temp:','V3_CovHlth_PR_Rx_Stats_',FORMAT(curdate,'yyyymmdd;;q'),'.txt')), PROTECT
DECLARE posop = vc WITH NOCONSTANT(FILLSTRING(2,' ')), PROTECT
DECLARE userop = vc WITH NOCONSTANT(FILLSTRING(2,' ')), PROTECT
...
;position
IF (SUBSTRING(1,1,reflect(parameter(parameter2($position),0)))="L")
 
	SET posop = "IN"
ELSEIF (parameter(parameter2($position),1) = 1.0)
	SET posop = "!="
ELSE
	SET posop = "="
ENDIF
;user
IF (SUBSTRING(1,1,reflect(parameter(parameter2($username),0)))="L")
 
	SET userop = "IN"
ELSEIF (parameter(parameter2($username),1) = 1.0)
	SET userop = "!="
ELSE
	SET userop = "="
ENDIF
...
AND operator(pr.position_cd,posop,$position)
AND operator(pr.person_id,userop,$username)
...
; 	elseif (pr.person_id IN (1.0, 2.0)) ;cerner system, cerner discern
; 		a->qual[d.seq].ordqual[d2.seq].ordactnqual[cnt].printflg	=	1
;		a->totphaactcnt = a->totphaactcnt + 1
;	endif
	a->qual[d.seq].ordqual[d2.seq].ordactnqual[cnt].printflg	=	1
...
SELECT into 'nl:'
	...
	facility = totals->qual[d.seq].Company
