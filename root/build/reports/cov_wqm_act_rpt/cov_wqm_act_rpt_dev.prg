...
;	t_rec->document_qual[t_rec->document_cnt].page_cnt			= ctl2.page_cnt
	...
	t_rec->document_qual[t_rec->document_cnt].subject			= ctl1.subject
foot report
...
head prsnl	
	...
	t_rec->summary_qual[t_rec->summary_cnt].person_id = t_rec->document_qual[d1.seq].person_id
...
detail
	...
	disp_line = concat(disp_line,',',trim(cnvtstring("0.0")))
...
order by
	...
	,document_id
	,batch_name_key
with uar_code(d),format(date,";;q"),separator = " ", format
	else
