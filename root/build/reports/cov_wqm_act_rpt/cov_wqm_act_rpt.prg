...
drop program cov_wqm_act_rpt:dba go
create program cov_wqm_act_rpt:dba
...
foot batch_key
	...
	batch_min = (batch_min + (datetimediff(t_rec->batch_qual[d1.seq].final_dt_tm,t_rec->batch_qual[d1.seq].create_dt_tm,4)))
foot p.person_id
	call writeLog(build2("----batch count:",trim(cnvtstring(batch_cnt))))
	call writeLog(build2("<---leave:",trim(p.name_full_formatted)," (",trim(cnvtstring(p.person_id)),")"))	
...
if (reply->status_data.status in("F","Z"))
	...
	if (t_rec->report_type = 0)
		select into $OUTDEV
			...
			,batch_min			= datetimediff(t_rec->batch_qual[d1.seq].final_dt_tm,t_rec->batch_qual[d1.seq].create_dt_tm,4)
