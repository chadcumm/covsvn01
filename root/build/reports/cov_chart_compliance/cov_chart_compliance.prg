...
head epr.encntr_id
	 if (idx > 0)
		...
		t_rec->prsnl_qual[idx].encntr_qual[t_rec->prsnl_qual[idx].encntr_cnt].person_id = p.person_id
		t_rec->prsnl_qual[idx].encntr_qual[t_rec->prsnl_qual[idx].encntr_cnt].patient_name = p.name_full_formatted
		t_rec->prsnl_qual[idx].encntr_qual[t_rec->prsnl_qual[idx].encntr_cnt].loc_facility_cd = e.loc_facility_cd
		t_rec->prsnl_qual[idx].encntr_qual[t_rec->prsnl_qual[idx].encntr_cnt].loc_nurse_unit_cd = e.loc_nurse_unit_cd
		t_rec->prsnl_qual[idx].encntr_qual[t_rec->prsnl_qual[idx].encntr_cnt].facilitiy = uar_get_code_display(e.loc_facility_cd)
		t_rec->prsnl_qual[idx].encntr_qual[t_rec->prsnl_qual[idx].encntr_cnt].unit = uar_get_code_display(e.loc_nurse_unit_cd)
		call writeLog(build2("-->Found Encounter:",
							...
							trim(format(epr.beg_effective_dt_tm,";;q")),":",
							trim(format(epr.end_effective_dt_tm,";;q"))))
	 endif
with nocounter
...
call writeLog(build2("************************************************************"))
call writeLog(build2("* START Finding Documentation ******************************"))
...
plan d1
join ta
	where ta.msg_sender_id = t_rec->prsnl_qual[d1.seq].person_id
...
order by
	 ...
	 ,e.encntr_id
	 ,ta.task_dt_tm desc
	 ,taa.beg_eff_dt_tm desc
	 ,ta.task_id
head report
	cnt = 0
head ta.msg_sender_id
	call writeLog(build2("->Found Personnel:",
							trim(cnvtstring(ta.msg_sender_id)),":",trim(t_rec->prsnl_qual[d1.seq].name_full_formatted)))						
head e.encntr_id
	idx = locateval(i,1,t_rec->prsnl_qual[d1.seq].encntr_cnt,e.encntr_id,t_rec->prsnl_qual[d1.seq].encntr_qual[i].encntr_id)
	if (idx > 0)
		t_rec->prsnl_qual[d1.seq].encntr_qual[idx].task_id = ta.task_id
		t_rec->prsnl_qual[d1.seq].encntr_qual[idx].task_assign_id = taa.assign_prsnl_id
		t_rec->prsnl_qual[d1.seq].encntr_qual[idx].task_assign_name = p2.name_full_formatted
		t_rec->prsnl_qual[d1.seq].encntr_qual[idx].task_dt_tm = ta.task_dt_tm	
		call writeLog(build2("-->Found Encounter:",
							trim(cnvtstring(idx)),":",
							trim(cnvtstring(e.encntr_id)),":",
							trim(cnvtstring(ta.task_id)),":",
							trim(format(ta.task_dt_tm,";;q")),":",
							trim(cnvtstring(p2.person_id)),":",
							trim(p2.name_full_formatted)))
	endif
with nocounter

call writeLog(build2("* END   Finding Documentation ******************************"))
call writeLog(build2("************************************************************"))
...
FROM
	encntr_alias   ea
	, code_value   cv
	, (dummyt   d1  with seq=size(t_rec->prsnl_qual,5))
	, (dummyt   d2 with seq = 1)

plan d1
	where maxrec(d2,size(t_rec->prsnl_qual[d1.seq].encntr_qual,5))
join d2
join ea
	where ea.encntr_id	= t_rec->prsnl_qual[d1.seq].encntr_qual[d2.seq].encntr_id
	and	  ea.active_ind	= 1
...
detail
	case (cv.display)
		of "MRN":	t_rec->prsnl_qual[d1.seq].encntr_qual[d2.seq].mrn = cnvtalias(ea.alias,ea.alias_pool_cd)
	endcase
WITH nocounter
...
detail
	case (cv.display)
		of "FIN NBR":	t_rec->prsnl_qual[d1.seq].encntr_qual[d2.seq].fin = cnvtalias(ea.alias,ea.alias_pool_cd)
	endcase
WITH nocounter
...
join ce
	...
	and   ce.event_tag        != "Date\Time Correction"
	;wand   ce.result_val        >  " "
	;and   ce.valid_from_dt_tm >= cnvtdatetime(curdate-7,0)
join e
	where e.encntr_id = ce.encntr_id
	;and   (e.disch_dt_tm = NULL or e.disch_dt_tm >= cnvtdatetime(curdate-7,0))
join p
	where p.person_id = e.person_id
join ea
	where ea.encntr_id = e.encntr_id
