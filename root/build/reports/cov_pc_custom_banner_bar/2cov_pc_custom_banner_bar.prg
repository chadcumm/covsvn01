...
9.0		    5/24/2021   Chad Cummings		CR-10471 - Added Birth and Admin sex to Fall Risk Field

https://wiki.cerner.com/display/public/reference/Understand+Demographics+in+Registration
**************************************************************/
...
drop program cov_pc_custom_banner_bar go
create program cov_pc_custom_banner_bar
...
declare fall_risk_display = vc with noconstant(" "); 9.0
...
head pr.problem_id
 	   fall_risk_display = BUILD2(" ","Yes"," ","(", PR.BEG_EFFECTIVE_DT_TM,")") ;9.0
	   reply->custom_field[iCustomFieldIndex].custom_field_display             =
...
head	d.person_id
 	   fall_risk_display = BUILD2(" ","Yes"," ","(", d.BEG_EFFECTIVE_DT_TM,")") ;9.0
	   reply->custom_field[iCustomFieldIndex].custom_field_display             =
...
with nocounter 
	...
	if(curqual = 0)
	   ...
	   set reply->custom_field[iCustomFieldIndex].custom_field_display        = fall_risk_display ;9.0
	   set reply->custom_field[iCustomFieldIndex].custom_field_index          = 4 ;9.0
	endif
endif
;end 7.0
...
set reply->custom_field[iCustomFieldIndex].custom_field_display = concat(
			"Fall Risk:"
			,reply->custom_field[iCustomFieldIndex].custom_field_display
			)

declare birth_sex_cd = f8 with noconstant(0.0)
declare admin_sex_cd = f8 with noconstant(0.0)
...
plan p
	where p.person_id = request->person_id 
detail
	birth_sex_cd = p.birth_sex_cd
with nocounter

set reply->custom_field[iCustomFieldIndex].custom_field_display = build2(
			reply->custom_field[iCustomFieldIndex].custom_field_display
			," Sex:"
			,trim(uar_get_code_display(birth_sex_cd))
			)
if (birth_sex_cd > 0.0)
	set reply->custom_field[iCustomFieldIndex].custom_field_display = build2(
			reply->custom_field[iCustomFieldIndex].custom_field_display
			," Gender:"
			,trim(uar_get_code_display(admin_sex_cd))
			)
endif
