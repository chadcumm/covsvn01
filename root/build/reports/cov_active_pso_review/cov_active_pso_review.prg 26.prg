...
head e.encntr_id
	...
	t_rec->encntr_qual[t_rec->encntr_cnt].order_age = datetimediff(sysdate,oa.action_dt_tm,1)
with nocounter
...
plan d1
join ce
	 where   ce.encntr_id = t_rec->encntr_qual[d1.seq].encntr_id
	 and     ce.person_id = t_rec->encntr_qual[d1.seq].person_id
	...
	and   ce.result_val        >  " "
	and   ce.event_cd in(
						 	2563737521.00	;Physician Recertification - Psych Form
 							,2563737513.00	;Physician Recertification Form
						)
order by
	ce.encntr_id
	,ce.event_end_dt_tm desc
head ce.encntr_id	
	t_rec->encntr_qual[d1.seq].form_signed = ce.event_title_text
	t_rec->encntr_qual[d1.seq].form_dt_Tm = ce.valid_from_dt_tm
with nocounter
...
if ($SEARCH_TYPE = 1)
	PLAN D1
		...
		and T_REC->encntr_qual[D1.SEQ].facility in("MHHS Behav Hlth","PW Senior Behav","PBH Peninsula")
		and T_REC->encntr_qual[D1.SEQ].order_age > 12.0
	else
		PLAN D1
			where ((T_REC->encntr_qual[D1.SEQ].primary_hp > " ") or (T_REC->encntr_qual[D1.SEQ].secondary_hp > " "))
			;and T_REC->encntr_qual[D1.SEQ].disch_dt_tm = 0.0
			and T_REC->encntr_qual[D1.SEQ].facility not in("MHHS Behav Hlth","PW Senior Behav","PBH Peninsula")
			and T_REC->encntr_qual[D1.SEQ].order_age > 17.0
	endif
 INTO $OUTDEV
	...
	, ENCNTR_QUAL_ORDER_AGE = trim(cnvtstring( T_REC->encntr_qual[D1.SEQ].order_age))
	...
	, ENCNTR_QUAL_RECERT_FORM = SUBSTRING(1, 30, T_REC->encntr_qual[D1.SEQ].form_signed)
	, ENCNTR_QUAL_RECERT_FORM_DT_TM = format(T_REC->encntr_qual[D1.SEQ].form_dt_Tm,";;q")
	, ENCNTR_QUAL_DISCH_DT_TM = T_REC->encntr_qual[D1.SEQ].disch_dt_tm
