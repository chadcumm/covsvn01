...
******************************************************************************
	...
	Author:				Chad M Cummings (Copy of Todd A Blanchard Report)
...
******************************************************************************
     ...
     2/18/2019  Chad M Cummings			Copied Source for TOG specific Report
 	 3/05/2019  Chad M Cummings			Updated output to include the appt_state
   5/16/2019  Chad M Cummings			Updated include all comments
001   01/29/2020  Chad M Cummings			Explicitly added new patient appt types
******************************************************************************/
...
with OUTDEV, facility, start_datetime, end_datetime, RPT_TYPE_IND
...
declare temp_comment						= vc with noconstant(" ")
...
from
	...
	, (left join PERSON_ALIAS eaf2 on eaf2.person_id = e.person_id
		and eaf2.person_alias_type_cd = cmrn_var
    ...
    , (inner join code_value cv on sev.appt_type_cd = cv.code_value
       and cv.display in("ONC Extended Follow Up Visit","ONC Follow Up MD","ONC Follow Up Nurse Practioner",
						;001 "ONC New Patient Appt*","ONC Backload Follow Up MD","ONC Backload Follow Up NP"))
						"ONC New Patient Appt*","ONC Backload Follow Up MD","ONC Backload Follow Up NP", ;001
						"ONC New Patient Appt - GYN Oncology","ONC New Patient Appt - Oncology",		;001
						"ONC New Patient Appt - Hematology",	;001
						"ONC New Patient Appt -  Genetic Counceling","ONC New Patient Appt - Genetic Counceling" ;001		
						)) ;001
...
head sa.sch_appt_id
	...
	sched_appt->list[cnt].mrn				= eaf2.alias
...
head report
	cnt = 0
head sdc.parent_id
	call echo(build("starting new patient: ",sdc.parent_id))
	cnt = 0
	temp_comment = " "
detail
	cnt = (cnt + 1)
	call echo(build("-->new comment:",lt.long_text))
	call echo(build("-->cnt=",cnt))
	if (cnt = 1)
		call echo("--->adding first comment")
		call echo(lt.long_text)
		sched_appt->list[d1.seq].person_comment = trim(lt.long_text)
		temp_comment = trim(lt.long_text)
	else
		call echo("--->adding second comment")
		call echo(lt.long_text)
		sched_appt->list[d1.seq].person_comment = concat(
				trim(sched_appt->list[d1.seq].person_comment)," ,",
				trim(lt.long_text))
	;sched_appt->list[d1.seq].person_comment = temp_comment
	endif
		call echo(build("sched_appt->list[d1.seq].person_comment=",sched_appt->list[d1.seq].person_comment))
foot sdc.parent_id
	cnt = 0
foot report
	cnt = 0
...
select distinct into $OUTDEV
	...
	, appt_dt			= format(sched_appt->list[d1.seq].appt_dt_tm, "mm/dd/yyyy;;d")
	, fin					= sched_appt->list[d1.seq].fin
	, cmrn					= sched_appt->list[d1.seq].mrn
    ...
    , person_comment		= trim(sched_appt->list[d1.seq].person_comment)
...
execute cov_tog_sched_comment_lb $OUTDEV
