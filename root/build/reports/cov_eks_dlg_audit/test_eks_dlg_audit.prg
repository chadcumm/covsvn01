...
******************************************************************************/
drop program test_eks_dlg_audit:dba go
create program test_eks_dlg_audit:dba
prompt
...
plan d1
join p
	where p.person_id = t_rec->qual[d1.seq].prsnl_id
detail
	t_rec->qual[d1.seq].prsnl_name = p.name_full_formatted
	t_rec->qual[d1.seq].prsnl_pos = uar_get_code_display(p.position_cd)
with nocounter
