...
from
    ...
    ,person p
plan ede
where ede.dlg_dt_tm >= cnvtdatetime($BEG_DT_TM) and ede.dlg_name = "PSO_EKM!PSO_INPT_CERTIFICATION_2"
join p
...
elseif ($EVOKE_ACTION = 1)
	...
	from
	    ...
	    ,person p
	plan ede
	where ede.dlg_dt_tm >= cnvtdatetime($BEG_DT_TM) and ede.dlg_name = "PSO_EKM!PSO_INPT_CERT_CHECK_2"
	join p
	    where p.person_id = ede.person_id
	join e
	    where e.encntr_id = ede.encntr_id
