...
CREATE PROGRAM chs_sch_inqa_stjude_queueonly :dba
 DECLARE oefieldid_authnumber = f8 WITH protect ,constant (uar_get_code_by ("DISPLAYKEY" ,16449 ,
   "SCHAUTHNUMBER" ) )
 DECLARE oefieldid_ord_location = f8 WITH protect ,constant (uar_get_code_by ("DISPLAYKEY" ,16449 ,
   "ORDERENTEREDBY" ) )
 ...
 SET reply->attr_qual[t_index ].attr_type = "vc"
 SET t_index = (t_index + 1 )
 SET reply->attr_qual[t_index ].attr_name = "ordering_location"
 ...
 SELECT INTO "nl:"
  ...
  PLAN (d )
   JOIN (a
   WHERE (a.sch_event_id = reply->query_qual[d.seq ].hide#scheventid )
   AND (a.oe_field_id = oefieldid_ord_location )
   AND (a.beg_effective_dt_tm <= cnvtdatetime (curdate ,curtime3 ) )
   AND (a.end_effective_dt_tm > cnvtdatetime (curdate ,curtime3 ) )
   AND (a.active_ind = 1 ) )
  ORDER BY d.seq ,
  ...
  HEAD a.sch_action_id
   null
  DETAIL
   reply->query_qual[d.seq ].ordering_location = trim (a.oe_field_display_value ,3 )
 ...
 IF ((pref_value_disppidrl = 1.0 ) )
  SELECT INTO "nl:"
   ...
   val_ordering_location = substring (1 ,100 ,reply->query_qual[d.seq ].ordering_location ) ,
   ...
   DETAIL
    ...
    reply->query_qual[sch2_idx ].ordering_location = val_ordering_location ,
...
#exit_script
 ...
 FREE SET t_record
 SET lastmod =
 "001 12/13/2017 DS051289 CCPS-12760 Adding Prior Auth and Ordering Physician colomn to Request List."
END GO
