...
drop program cov_rpt_op_readiness:dba go
create program cov_rpt_op_readiness:dba
...
/*
call writeLog(build2("* START Finding Expired Encounters   ************************************"))
...
with nocounter
call writeLog(build2("* END   Finding Expired Encounters   ************************************"))
*/
	...
	if (t_rec->patient_qual[i].lab_results_cnt > 0)
		...
		for (j=1 to t_rec->covid19.positive_cnt)
			if (t_rec->patient_qual[i].lab_results_qual[1].result_val = t_rec->covid19.positive_qual[j].result_val)
				...
				if (t_rec->patient_qual[i].lab_results_qual[1].event_end_dt_tm 
				...
				while (pos <= t_rec->patient_qual[i].lab_results_cnt)
					if (stat = 0)
						if (t_rec->patient_qual[i].lab_results_qual[pos].result_val not in("DUPLICATE","RE-ORDERED","SEE BELOW"))
							...
							set t_output->qual[t_output->cnt].covid19_result_dt_tm = 	
								;changing to clinsig_updt_dt_tm 
								trim(format(t_rec->patient_qual[i].lab_results_qual[pos].event_end_dt_tm,";;q"))					
								;trim(format(t_rec->patient_qual[i].lab_results_qual[1].clinsig_updt_dt_tm,";;q"))
						endif
					endif
					set pos = (pos + 1)
 ...
 elseif (t_output->qual[i].expired_ind = 1)
  set t_output->qual[i].encntr_ignore = 0
	...
	if (t_output->qual[i].encntr_ignore = 0)
		...
		if (t_output->qual[i].previous_onset_conf_ind = 1)
			set nhsn_covid19->patient_qual[nhsn_covid19->patient_cnt].prev_onset_conf = "Y"
		endif
...
call writeLog(build2("************************************************************"))
call writeLog(build2("* START Building Tele-Tracking Data ************************"))
 ...
 elseif (t_output->qual[i].expired_ind = 1)
  set t_output->qual[i].encntr_ignore = 0
	...
	if (t_output->qual[i].encntr_ignore = 0)
		...
		if ( t_output->qual[i].hosp_conf_onset = 1)
			set tt_covid19->patient_qual[tt_covid19->patient_cnt].hosp_conf_onset = "Y"
		endif
		if ((t_output->qual[i].expired_ind = 1) and (t_output->qual[i].positive_ind = 1))
	 ...
	 if (tt_covid19->patient_qual[d1.seq].expired = "Y")
		...
		if (tt_covid19->patient_qual[d1.seq].location_class_1 in("MSU","ICU"))
			...
			if (tt_covid19->patient_qual[d1.seq].confirmed = "Y")
				...
				if ((tt_covid19->patient_qual[d1.seq].los_days >= 14) and (tt_covid19->patient_qual[d1.seq].hosp_conf_onset = "Y"))
...
for (i=1 to tt_covid19->summary_cnt)
	...
	from
	    ...
	    ,code_value_extension cve1
	plan d1
		where tt_covid19->summary_qual[1].facility
	join cv1
	    where cv1.code_set              = t_rec->custom_code_set
	    and   cv1.definition            = trim(cnvtlower(t_rec->curprog))   
	    and   cv1.active_ind            = 1
...
call writeLog(build2("* END   Building Tele-Tracking Data ************************"))
call writeLog(build2("************************************************************"))
...
for (i=1 to t_output->cnt)
	...
	if (t_output->qual[i].hrts_ignore = 0)
		...
		if (t_output->qual[i].hosp_conf_onset = 1)
			set hrts_covid19->patient_qual[hrts_covid19->patient_cnt].hosp_conf_onset = "Y"
		endif
...
select into $OUTDEV
	...
	,hosp_conf_onset		= substring(1,50,hrts_covid19->patient_qual[d1.seq].hosp_conf_onset)
...
elseif (t_rec->prompt_report_type = 6)
...
select into $OUTDEV
	...
	,ICU_Confirmed_Patients						= "tdb"
	,ICU_Patients_Under_Investigation			= "tdb"
...
elseif (t_rec->prompt_report_type = 7)
...
select into $OUTDEV
	...
	,hosp_conf_onset		= substring(1,50,nhsn_covid19->patient_qual[d1.seq].hosp_conf_onset)
