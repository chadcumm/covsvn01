...
******************************************************************************
	...
	Source file name:	cov_rpt_op_readiness.prg
	Object name:		cov_rpt_op_readiness
	Request #:
...
drop program cov_rpt_op_readiness_dev:dba go
create program cov_rpt_op_readiness_dev:dba
...
call writeLog(build2("* START User ******************************"))
select into "nl:" 
from
	prsnl p
plan p
	where p.person_id = reqinfo->updt_id
	and   p.username in("CCUMMIN4","NBROWNLO","LMYERS","HHANKINS","SYSTEM","CERNER","JSANTORO")
with nocounter
...
if (reply->status_data.status = "F") 
	...
	set reply->status_data.status = "F"
	set reply->status_data.subeventstatus.operationname = "USERNAME"
	set reply->status_data.subeventstatus.operationstatus = "F"
	set reply->status_data.subeventstatus.targetobjectname = "USERNAME"
	set reply->status_data.subeventstatus.targetobjectvalue = "Not a Valid User"
	go to exit_script
endif
call writeLog(build2("* END User ******************************"))
					...
					if (t_rec->covid19.covid_oc_qual[k].catalog_cd = t_rec->patient_qual[i].orders_qual[j].catalog_cd)
					 ...
					 if (t_rec->covid19.covid_ignore_cnt > 0)
					 	for (pos=1 to t_rec->covid19.covid_ignore_cnt)
					 	 for (ii=1 to t_rec->patient_qual[i].orders_qual[j].order_detail_cnt)
					 	  if (t_rec->patient_qual[i].orders_qual[j].order_detal_qual[ii].oe_field_id =
					 	  	  ...
					 	  	  set t_rec->patient_qual[i].orders_qual[j].order_detail 
					 	  	  	= t_rec->patient_qual[i].orders_qual[j].order_detal_qual[ii].oe_field_display_value
					 	  	 ...
					 	  	 if (t_rec->patient_qual[i].orders_qual[j].order_detal_qual[ii].oe_field_value = 
					 	  	   ...
					 	  	   set stat = 0 ;REMOVE IGNORE FOR HEATHER TEMP
					 	  	 endif
					 	  endif
					 	 endfor
					 	endfor
						...
						if (ignore = 0)	
							...
							set t_output->qual[t_output->cnt].covid19_order_detail = concat(trim(t_rec->patient_qual[i].orders_qual[j].order_detail))
...
if (t_rec->prompt_report_type = 1) ;NHSN Summary
	select into $OUTDEV
...
elseif (t_rec->prompt_report_type = 3) ;HRTS Summary
	select into $OUTDEV
...
elseif (t_rec->prompt_report_type = 4) ;HRTS Detail
	select into $OUTDEV
...
elseif (t_rec->prompt_report_type = 2) ;NHSN Detail
	select into $OUTDEV
...
elseif (t_rec->prompt_report_type = 5) ;Facility Dashboard
	select into $OUTDEV
...
else	;Full Detail and currently Tele-Tracking
	select into $OUTDEV
		...
		,covid19_order_detail	= substring(1,50,t_output->qual[d1.seq].covid19_order_detail)
...
#exit_script
if (reply->status_data.status = "F")
	if (reply->status_data.subeventstatus.operationname = "USERNAME")
		call writeLog(build2( "This report requires prior authorization.  Contact helpdesk2@covhlth.net for assistance"))
		set program_log->display_on_exit = 0
		select into $OUTDEV
		from (dummyt d1)
		head report
			col 0 "This report requires prior authorization.  Contact helpdesk2@covhlth.net for assistance"
		with nocounter
	else
