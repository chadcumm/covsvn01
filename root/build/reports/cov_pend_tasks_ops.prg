...
FOR (fcnt = 1 to fac->rec_cnt)
 
	FOR (dcnt = 1 to dept->rec_cnt)
 		call echo(build('facility :',fac->qual[fcnt].faccd, ' dept : ', dept->qual[dcnt].deptcd))
 		call echo(build('facility :',fac->qual[fcnt].facdesc, ' dept : ', dept->qual[dcnt].dept))
		EXECUTE cov_pend_tasks_rt_pt_ot_sp "mine", fac->qual[fcnt].faccd, dept->qual[dcnt].deptcd, "BDATE", "EDATE"
