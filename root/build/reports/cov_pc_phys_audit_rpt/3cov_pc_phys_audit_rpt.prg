...
join p
	where p.person_id = c.ord_phys_id
...
head c.charge_item_id
    ...
    phys_data->qual[pos]->charge_qual[i].ord_phys = p.name_full_formatted
    phys_data->qual[pos]->charge_qual[i].ord_phys_id = c.ord_phys_id
    phys_data->qual[pos]->charge_qual[i].prim_cdm = c.prim_cdm
    phys_data->qual[pos]->charge_qual[i].prim_cpt = c.prim_cpt
   	
   	phys_data->qual[pos]->charge_qual[i].charge_title = c.charge_description
   	
   	if (c.prim_cdm > "")
   		phys_data->qual[pos]->charge_qual[i].charge_title = c.prim_cdm
   	endif

   	if (c.prim_cpt > "")
   		phys_data->qual[pos]->charge_qual[i].charge_title = c.prim_cpt
   	endif
   	
    if (format(c.service_dt_tm,"dd-mmm-yyyy;;d") = format(phys_data->qual[pos].note_dt_dtm_tat,"dd-mmm-yyyy;;d"))
    	if (k=0)
    		phys_data->qual[pos].charges = concat(trim(phys_data->qual[pos]->charge_qual[i].charge_title)
    								," (",trim(p.name_full_formatted),")")
    	else
    		phys_data->qual[pos].charges = concat(phys_data->qual[pos].charges,":"
    									,trim(phys_data->qual[pos]->charge_qual[i].charge_title)," (",trim(p.name_full_formatted),")")
    	endif
    	k = 1
    endif
