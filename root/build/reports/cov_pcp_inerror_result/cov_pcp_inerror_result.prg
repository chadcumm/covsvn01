...
******************************************************************************
	...
	Solution:			
	Source file name:	cov_pcp_inerror_result.prg
	Object name:		cov_pcp_inerror_result
	Request #:
...
drop program cov_pcp_inerror_result:dba go
create program cov_pcp_inerror_result:dba
...
join cv1
	...
	and   cv1.code_set = 72
	and   cv1.display = "D-Primary Care Physician"
order by
	ce.event_id
head ce.event_id
...
call writeLog(build2("************************************************************"))
call writeLog(build2("* SART Custom   ********************************************"))
...
set requestin->request.n_person_id = t_rec->qual[1].person_id
set requestin->request.TRANSACTION = "UMPI"
set requestin->request.O_ATTEND_DOC_ID = 1.0
set requestin->request.N_ATTEND_DOC_ID = 2.0
call writeLog(build2("t_rec->qual[",trim(cnvtstring(i)),"].person_id=",t_rec->qual[1].person_id))
call writeLog(build2("t_rec->qual[",trim(cnvtstring(i)),"].encntr_id=",t_rec->qual[1].encntr_id))
call writeLog(build2("t_rec->qual[",trim(cnvtstring(i)),"].event_id=",t_rec->qual[1].event_id))
