...
call echo(build("Starting cleanup"))
...
while (lContinueProcess = 1)    ;loop through 5000 patients at a time
  select into "nl:"
  from tracking_item ti, encounter e
  plan ti
  ...
  if (tiCnt > 0)
    ...
    commit
    call echo(build("updated ->", tiCnt, "<- encounter records." ))
...
endwhile
call echo(build("FNEncounterQuery-> ", build2(cnvtint(curtime3 - FNEncounterQuery)), "0 ms"))
...
while (lContinueProcess = 1)
  select into "nl:"
  from tracking_item ti, person p
  plan ti
  ...
  if (tiCnt > 0)
    ...
    with nocounter, maxcommit = 500
    commit
    call echo(build("updated ->", tiCnt, "<- person records." ))
    set stat = alterlist(tiRecs->ti, 0)
    set tiCnt = 0
  else
    set lContinueProcess = 0      ;no more patients qualified exit while loop
...
endwhile
call echo(build("FNPersonQuery-> ", build2(cnvtint(curtime3 - FNPersonQuery)), "0 ms"))
...
while (lContinueProcess = 1)
  ...
  if (tiCnt > 0)
    ...
    with nocounter, maxcommit = 500
    commit
    call echo(build("updated ->", tiCnt, "<- prsnl records." ))
    set stat = alterlist(tiRecs->ti, 0)
    set tiCnt = 0
  else
    set lContinueProcess = 0      ;no more patients qualified exit while loop
...
endwhile
call echo(build("FNPrsnlQuery-> ", build2(cnvtint(curtime3 - FNPrsnlQuery)), "0 ms"))
...
while (lContinueProcess = 1)
  select into "nl:"
  from tracking_item ti, tracking_locator tl
  plan tl
  ...
  if (tlCnt > 0)
    ...
    with nocounter, maxcommit = 500
    commit
    call echo(build("updated ->", tlCnt, "<- tracking_locator records." ))
...
endwhile
call echo(build("FNTrackingLocatorQuery-> ", build2(cnvtint(curtime3 - FNTrackingLocatorQuery)), "0 ms"))
...
while (lContinueProcess = 1)
  ...
  if (tcCnt > 0)
    ...
    with nocounter, maxcommit = 500
    commit
    call echo(build("updated ->", tcCnt, "<- tracking_checkin records." ))
...
endwhile
call echo(build("FNTrackingCheckinQuery-> ", build2(cnvtint(curtime3 - FNTrackingCheckinQuery)), "0 ms"))

call echo(build("Total Records Updated:", total_cnt))
