DROP PROGRAM cov_ic_au_report :dba GO
CREATE PROGRAM cov_ic_au_report :dba
 PROMPT
  ...
  "File Location:" = "C:\" ,
  "Called From Ind:" = 0
  WITH outdev ,fromdate ,thrudate ,facility ,facwide ,nhsnloc ,unit ,medication ,route ,output ,file
  ,reporttypeind
 ...
 SUBROUTINE  getheaderdata (null )
  ...
  IF (( $REPORTTYPEIND = au_report ) )
   ...
   IF ((errorcode != 0 ) )
    CALL log_message (concat ("Subroutine GetHeaderData failed in fac data: " ,errmsg ) ,
     log_level_debug )
    GO TO exit_script
   ENDIF
 ...
 SUBROUTINE  generatefullxml (null )
  ...
  FOR (month_idx = 1 TO size (reply->months ,5 ) )
   ...
   FOR (loc_idx = 1 TO size (reply->months[month_idx ].locations ,5 ) )
    SET loclist_idx = locateval (idx ,1 ,value (size (loc_list->location ,5 ) ) ,reply->months[
     month_idx ].locations[loc_idx ].location_cd ,loc_list->location[idx ].loc_cd )
    SET cdc_location_cd = loc_list->location[loclist_idx ].cdc_cd
    SET nhsn_your_code = loc_list->location[loclist_idx ].nhsn_your_code
    SET xml_string = createheaderxml (reply->rpt_type_flag ,reply->facility_oid ,month_start ,
     month_end ,cdc_location_cd ,nhsn_your_code ,reply->months[month_idx ].locations[loc_idx ].
     days_present ,reply->months[month_idx ].locations[loc_idx ].admissions )
    FOR (med_idx = 1 TO size (reply->months[month_idx ].locations[loc_idx ].agents ,5 ) )
     IF ((reply->months[month_idx ].locations[loc_idx ].agents[med_idx ].nhsn_med_ind = true ) )
      SET rx_norm = uar_get_code_display (reply->months[month_idx ].locations[loc_idx ].agents[
       med_idx ].agent_cd )
      SET xml_string = build (xml_string ,createsinglemedxml (reply->rpt_type_flag ,reply->months[
        month_idx ].locations[loc_idx ].agents[med_idx ].not_available_flag ,cdc_location_cd ,
        nhsn_your_code ,reply->months[month_idx ].locations[loc_idx ].agents[med_idx ].agent_name ,
        rx_norm ,reply->months[month_idx ].locations[loc_idx ].agents[med_idx ].total ,reply->months[
        month_idx ].locations[loc_idx ].agents[med_idx ].respiratory ,reply->months[month_idx ].
        locations[loc_idx ].agents[med_idx ].digestive ,reply->months[month_idx ].locations[loc_idx ]
        .agents[med_idx ].iv ,reply->months[month_idx ].locations[loc_idx ].agents[med_idx ].im ) )
     ENDIF
    ENDFOR
 ...
 SUBROUTINE  createheaderxml (rpt_type_flag ,fac_oid ,month_start ,month_end ,cdc_loc_cd ,
  ...
  DECLARE sequence_num = i4 WITH noconstant (0 ) ,protect
  SELECT INTO "nl:"
  ...
  ELSE
   ...
   ENDIF
  ENDIF
  SET header_xml_string = build ("<ClinicalDocument xmlns=" ,'"urn:hl7-org:v3"' ," xmlns:xsi=" ,
   '"http://www.w3.org/2001/XMLSchema-instance"' ,">" ,"<realmCode code=" ,'"US"' ,"/>" ,
   "<typeId root=" ,'"2.16.840.1.113883.1.3"' ," extension=" ,'"POCD_HD000040"' ,"/>" ,
   "<templateId root=" ,'"2.16.840.1.113883.10.20.5.4.25"' ,"/>" ,"<templateId root=" ,
   '"2.16.840.1.113883.10.20.5.4.28"' ,"/>" ,"<templateId root=" ,'"2.16.840.1.113883.10.20.5.44"' ,
   "/>" ,"<id root=" ,'"' ,fac_oid ,"." ,sequence_num ,"." ,cnvtstring (cdc_loc_cd ) ,'"' ,
   " extension=" ,'"' ,sequence_num ,'"' ,"/>" ,"<code codeSystem=" ,'"2.16.840.1.113883.6.1"' ,
   " codeSystemName=" ,'"LOINC"' ," code=" ,'"51897-7"' ," displayName=" ,
   '"Healthcare Associated Infection Report"' ,"/>" ,
   "<title>Antimicrobial Use, Pharmacy Option (AUP) Summary Report</title>" ,"<effectiveTime value="
   ,'"' ,cur_date ,'"' ,"/>" ,"<confidentialityCode codeSystem=" ,'"2.16.840.1.113883.5.25"' ,
   " code=" ,'"N"' ,"/>" ,"<languageCode code=" ,'"en-US"' ,"/>" ,"<setId root=" ,'"' ,fac_oid ,"." ,
   sequence_num ,"." ,cnvtstring (cdc_loc_cd ) ,'"' ," extension=" ,'"' ,sequence_num ,'"' ,"/>" ,
   "<versionNumber value=" ,'"' ,sequence_num ,'"' ,"/>" ,"<recordTarget>" ,"<patientRole>" ,
   "<id nullFlavor=" ,'"NA"' ,"/>" ,"</patientRole>" ,"</recordTarget>" ,"<author>" ,"<time value=" ,
   '"' ,cur_date ,'"' ,"/>" ,"<assignedAuthor>" ,"<id root=" ,
   '"2.16.840.1.113883.3.117.1.1.5.2.1.1"' ," extension=" ,'"CERNER"' ,"/>" ,"</assignedAuthor>" ,
   " </author>" ,"<custodian>" ,"<assignedCustodian>" ,"<representedCustodianOrganization>" ,
   "<id root=" ,'"2.16.840.1.114222.4.3.2.11"' ,"/>" ,"</representedCustodianOrganization>" ,
   "</assignedCustodian>" ,"</custodian>" ,"<legalAuthenticator>" ,"<time value=" ,'"' ,cur_date ,
   '"' ,"/>" ,"<signatureCode code=" ,'"S"' ,"/>" ,"<assignedEntity>" ,"<id root=" ,
   '"2.16.840.1.113883.3.117.1.1.5.1.1.2"' ," extension=" ,'"CERNER"' ,"/>" ,"</assignedEntity>" ,
   "</legalAuthenticator>" ,"<participant typeCode=" ,'"SBJ"' ," contextControlCode=" ,'"OP"' ,">" ,
   "<associatedEntity classCode=" ,'"PRS"' ,">" ,"<code codeSystem=" ,'"2.16.840.1.113883.6.96"' ,
   " code=" ,'"389109008"' ," displayName=" ,'"group"' ,"/>" ,"</associatedEntity>" ,
   "</participant>" ,"<participant typeCode=" ,'"LOC"' ," contextControlCode=" ,'"OP"' ,">" ,
   "<associatedEntity classCode=" ,'"SDLOC"' ,">" ,"<id root=" ,'"' ,fac_oid ,'"' ,"/>" ,
   "</associatedEntity>" ,"</participant>" ,"<documentationOf>" ,"<serviceEvent classCode=" ,
   '"CASE"' ,">" ,"<code codeSystem=" ,'"2.16.840.1.113883.6.277"' ," codeSystemName=" ,'"cdcNHSN"' ,
   " code=" ,'"1887-9"' ," displayName=" ,'"Summary data reporting antimicrobial usage"' ,"/>" ,
   "<effectiveTime>" ,"<low value=" ,'"' ,start_date ,'"' ,"/>" ,"<high value=" ,'"' ,end_date ,'"' ,
   "/>" ,"</effectiveTime>" ,"</serviceEvent>" ,"</documentationOf>" ,"<relatedDocument typeCode=" ,
   '"RPLC"' ,">" ,"<parentDocument>" ,"<id root=" ,'"' ,fac_oid ,"." ,sequence_num ,"." ,cnvtstring (
    cdc_loc_cd ) ,'"' ," extension=" ,'"' ,sequence_num ,'"' ,"/>" ,"<setId root=" ,'"' ,fac_oid ,
   "." ,sequence_num ,"." ,cnvtstring (cdc_loc_cd ) ,'"' ," extension=" ,'"' ,sequence_num ,'"' ,
   "/>" ,"<versionNumber value=" ,'"1"' ,"/>" ,"</parentDocument>" ,"</relatedDocument>" ,
   "<component>" ,"<structuredBody>" ,"<component>" ,"<section>" ,"<templateId root=" ,
   '"2.16.840.1.113883.10.20.5.4.26"' ,"/>" ,"<templateId root=" ,'"2.16.840.1.113883.10.20.5.5.51"'
   ,"/>" ,"<code codeSystem=" ,'"2.16.840.1.113883.6.1"' ," codeSystemName=" ,'"LOINC"' ," code=" ,
   '"51900-9"' ," displayName=" ,'"Summary Section"' ,"/>" ,"<title xmlns:cda=" ,'"urn:hl7-org:v3"' ,
   " xmlns:voc=" ,'"http://www.lantanagroup.com/voc"' ,">" ,"Summary Data</title>" ,"<text> </text>"
   ,"<entry typeCode=" ,'"DRIV"' ,">" ,"<encounter classCode=" ,'"ENC"' ," moodCode=" ,'"EVN"' ,">" ,
   "<templateId root=" ,'"2.16.840.1.113883.10.20.5.6.197"' ,"/>" ,"<participant typeCode=" ,'"LOC"'
   ,">" ,"<participantRole classCode=" ,'"SDLOC"' ,">" )
  IF ((rpt_type_flag = facwidein ) )
   SET header_xml_string = build (header_xml_string ,"<id root=" ,'"' ,fac_oid ,'"' ," extension=" ,
    '"FACWIDEIN"' ,"/>" ,"<code codeSystem=" ,'"2.16.840.1.113883.6.259"' ," codeSystemName=" ,
    '"HL7 Healthcare Service Location Code"' ," code=" ,'"1250-0"' ," displayName=" ,'"FACWIDEIN"' ,
    "/>" ,"</participantRole>" ,"</participant>" ,"<entryRelationship typeCode=" ,'"COMP"' ,">" ,
    "<observation classCode=" ,'"OBS"' ," moodCode=" ,'"EVN"' ,">" ,"<templateId root=" ,
    '"2.16.840.1.113883.10.20.5.6.185"' ,"/>" ,"<code codeSystem=" ,'"2.16.840.1.113883.6.277"' ,
    " codeSystemName=" ,'"cdcNHSN"' ," code=" ,'"2525-4"' ," displayName=" ,'"' ,days_present_msg ,
    '"' ,"/>" ,"<statusCode code=" ,'"completed"' ,"/>" ,"<value xsi:type=" ,'"PQ"' ," unit=" ,'"d"'
    ," value=" ,'"' ,days_present ,'"' ,"/>" ,"</observation>" ,"</entryRelationship>" ,
    "<entryRelationship typeCode=" ,'"COMP"' ,">" ,"<observation classCode=" ,'"OBS"' ," moodCode=" ,
    '"EVN"' ,">" ,"<templateId root=" ,'"2.16.840.1.113883.10.20.5.6.185"' ,"/>" ,
    "<code codeSystem=" ,'"2.16.840.1.113883.6.277"' ," codeSystemName=" ,'"cdcNHSN"' ," code=" ,
    '"1862-2"' ," displayName=" ,'"Number of admissions"' ,"/>" ,"<statusCode code=" ,'"completed"' ,
    "/>" ,"<value xsi:type=" ,'"PQ"' ," unit=" ,'"d"' ," value=" ,'"' ,admissions ,'"' ,"/>" ,
    "</observation>" ,"</entryRelationship>" ,"</encounter>" ,"</entry>" )
  ELSE
   SET header_xml_string = build (header_xml_string ,"<id root=" ,'"' ,fac_oid ,'"' ," extension=" ,
    '"' ,nhsn_your_code ,'"' ,"/>" ,"<code codeSystem=" ,'"2.16.840.1.113883.6.259"' ,
    " codeSystemName=" ,'"HL7 Healthcare Service Location Code"' ," code=" ,'"' ,hl7_code ,'"' ,
    " displayName=" ,'"' ,nhsn_loc_name ,'"' ,"/>" ,"</participantRole>" ,"</participant>" ,
    "<entryRelationship typeCode=" ,'"COMP"' ,">" ,"<observation classCode=" ,'"OBS"' ," moodCode=" ,
    '"EVN"' ,">" ,"<templateId root=" ,'"2.16.840.1.113883.10.20.5.6.185"' ,"/>" ,
    "<code codeSystem=" ,'"2.16.840.1.113883.6.277"' ," codeSystemName=" ,'"cdcNHSN"' ," code=" ,
    '"2525-4"' ," displayName=" ,'"' ,days_present_msg ,'"' ,"/>" ,"<statusCode code=" ,
    '"completed"' ,"/>" ,"<value xsi:type=" ,'"PQ"' ," unit=" ,'"d"' ," value=" ,'"' ,days_present ,
    '"' ,"/>" ,"</observation>" ,"</entryRelationship>" ,"</encounter>" ,"</entry>" )
  ENDIF
 ...
 SUBROUTINE  createsinglemedxml (rpt_type_flag ,not_available_flag ,cdc_loc_cd ,nhsn_your_code ,
  ...
  SET antimicrobial_xml_string = build ("<entry typeCode=" ,'"DRIV"' ,">" ,"<encounter classCode=" ,
   '"ENC"' ," moodCode=" ,'"EVN"' ,">" ,"<templateId root=" ,'"2.16.840.1.113883.10.20.5.6.198"' ,
   "/>" ,"<participant typeCode=" ,'"LOC"' ,">" ,"<participantRole classCode=" ,'"SDLOC"' ,">" )
  IF ((rpt_type_flag = facwidein ) )
   SET antimicrobial_xml_string = build (antimicrobial_xml_string ,"<id root=" ,
    '"2.16.840.1.113883.3.117.1.1.5.1.1"' ," extension=" ,'"FACWIDEIN"' ,"/>" ,"<code codeSystem=" ,
    '"2.16.840.1.113883.6.259"' ," codeSystemName=" ,'"' ,hl7_location_msg ,'"' ," code=" ,
    '"1250-0"' ," displayName=" ,'"FACWIDEIN"' ,"/>" )
  ELSE
   SET antimicrobial_xml_string = build (antimicrobial_xml_string ,"<id root=" ,
    '"2.16.840.1.113883.3.117.1.1.5.1.1"' ," extension=" ,'"' ,nhsn_your_code ,'"' ,"/>" ,
    "<code codeSystem=" ,'"2.16.840.1.113883.6.259"' ," codeSystemName=" ,'"' ,hl7_location_msg ,'"'
    ," code=" ,'"' ,hl7_code ,'"' ," displayName=" ,'"' ,nhsn_loc_name ,'"' ,"/>" )
  ENDIF
  SET antimicrobial_xml_string = build (antimicrobial_xml_string ,"</participantRole>" ,
   "</participant>" ,"<participant typeCode=" ,'"CSM"' ,">" ,"<participantRole classCode=" ,'"MANU"'
   ,">" ,"<code codeSystem=" ,'"2.16.840.1.113883.6.88"' ," codeSystemName=" ,'"RxNorm"' ," code=" ,
   '"' ,rx_norm ,'"' ," displayName=" ,'"' ,med_name ,'"' ,"/>" ,"</participantRole>" ,
   "</participant>" ,"<entryRelationship typeCode=" ,'"COMP"' ,">" ,"<observation classCode=" ,
   '"OBS"' ," moodCode=" ,'"EVN"' ,">" ,"<templateId root=" ,'"2.16.840.1.113883.10.20.5.6.185"' ,
   "/>" ,"<code codeSystem=" ,'"2.16.840.1.113883.6.277"' ," codeSystemName=" ,'"cdcNHSN"' ," code="
   ,'"2524-7"' ," displayName=" ,'"' ,therapy_days_msg ,'"' ,"/>" ,"<statusCode code=" ,
   '"completed"' ,"/>" )
  IF ((not_available_flag = available ) )
   SET antimicrobial_xml_string = build (antimicrobial_xml_string ,"<value xsi:type=" ,'"PQ"' ,
    " unit=" ,'"d"' ," value=" ,'"' ,total_val ,'"' ,"/>" )
  ELSE
   SET antimicrobial_xml_string = build (antimicrobial_xml_string ,"<value xsi:type=" ,'"PQ"' ,
    " unit=" ,'"d"' ," nullFlavor=" ,'"NA"' ,"/>" )
  ENDIF
  SET antimicrobial_xml_string = build (antimicrobial_xml_string ,"</observation>" ,
   "</entryRelationship>" ,"<entryRelationship typeCode=" ,'"COMP"' ,">" ,"<observation classCode=" ,
   '"OBS"' ," moodCode=" ,'"EVN"' ,">" ,"<templateId root=" ,'"2.16.840.1.113883.10.20.5.6.194"' ,
   "/>" ,"<code codeSystem=" ,'"2.16.840.1.113883.6.277"' ," codeSystemName=" ,'"cdcNHSN"' ," code="
   ,'"2524-7"' ," displayName=" ,'"' ,therapy_days_msg ,'"' ,"/>" ,"<statusCode code=" ,
   '"completed"' ,"/>" )
  IF ((not_available_flag = available ) )
   SET antimicrobial_xml_string = build (antimicrobial_xml_string ,"<value xsi:type=" ,'"PQ"' ,
    " unit=" ,'"d"' ," value=" ,'"' ,resp_val ,'"' ,"/>" )
  ELSE
   SET antimicrobial_xml_string = build (antimicrobial_xml_string ,"<value xsi:type=" ,'"PQ"' ,
    " unit=" ,'"d"' ," nullFlavor=" ,'"NA"' ,"/>" )
  ENDIF
  SET antimicrobial_xml_string = build (antimicrobial_xml_string ,"<methodCode codeSystem=" ,
   '"2.16.840.1.113883.6.277"' ," codeSystemName=" ,'"cdcNHSN"' ," code=" ,'"' ,nhsn_xml_resp_code ,
   '"' ," displayName=" ,'"Respiratory tract route"' ,"/>" ,"</observation>" ,"</entryRelationship>"
   ,"<entryRelationship typeCode=" ,'"COMP"' ,">" ,"<observation classCode=" ,'"OBS"' ," moodCode=" ,
   '"EVN"' ,">" ,"<templateId root=" ,'"2.16.840.1.113883.10.20.5.6.194"' ,"/>" ,"<code codeSystem="
   ,'"2.16.840.1.113883.6.277"' ," codeSystemName=" ,'"cdcNHSN"' ," code=" ,'"2524-7"' ,
   " displayName=" ,'"' ,therapy_days_msg ,'"' ,"/>" ,"<statusCode code=" ,'"completed"' ,"/>" )
  IF ((not_available_flag = available ) )
   SET antimicrobial_xml_string = build (antimicrobial_xml_string ,"<value xsi:type=" ,'"PQ"' ,
    " unit=" ,'"d"' ," value=" ,'"' ,digestive_val ,'"' ,"/>" )
  ELSE
   SET antimicrobial_xml_string = build (antimicrobial_xml_string ,"<value xsi:type=" ,'"PQ"' ,
    " unit=" ,'"d"' ," nullFlavor=" ,'"NA"' ,"/>" )
  ENDIF
  SET antimicrobial_xml_string = build (antimicrobial_xml_string ,"<methodCode codeSystem=" ,
   '"2.16.840.1.113883.6.277"' ," codeSystemName=" ,'"cdcNHSN"' ," code=" ,'"' ,nhsn_xml_digest_code
   ,'"' ," displayName=" ,'"Digestive tract route"' ,"/>" ,"</observation>" ,"</entryRelationship>" ,
   "<entryRelationship typeCode=" ,'"COMP"' ,">" ,"<observation classCode=" ,'"OBS"' ," moodCode=" ,
   '"EVN"' ,">" ,"<templateId root=" ,'"2.16.840.1.113883.10.20.5.6.194"' ,"/>" ,"<code codeSystem="
   ,'"2.16.840.1.113883.6.277"' ," codeSystemName=" ,'"cdcNHSN"' ," code=" ,'"2524-7"' ,
   " displayName=" ,'"' ,therapy_days_msg ,'"' ,"/>" ,"<statusCode code=" ,'"completed"' ,"/>" )
  ...
  ENDIF
  SET antimicrobial_xml_string = build (antimicrobial_xml_string ,"<methodCode codeSystem=" ,
   '"2.16.840.1.113883.6.96"' ," codeSystemName=" ,'"SNOMED"' ," code=" ,'"' ,nhsn_xml_iv_code ,'"' ,
   " displayName=" ,'"Intravenous route"' ,"/>" ,"</observation>" ,"</entryRelationship>" ,
   "<entryRelationship typeCode=" ,'"COMP"' ,">" ,"<observation classCode=" ,'"OBS"' ," moodCode=" ,
   '"EVN"' ,">" ,"<templateId root=" ,'"2.16.840.1.113883.10.20.5.6.194"' ,"/>" ,"<code codeSystem="
   ,'"2.16.840.1.113883.6.277"' ," codeSystemName=" ,'"cdcNHSN"' ," code=" ,'"2524-7"' ,
   " displayName=" ,'"' ,therapy_days_msg ,'"' ,"/>" ,"<statusCode code=" ,'"completed"' ,"/>" )
  ...
  ENDIF
  SET antimicrobial_xml_string = build (antimicrobial_xml_string ,"<methodCode codeSystem=" ,
   '"2.16.840.1.113883.6.96"' ," codeSystemName=" ,'"SNOMED"' ," code=" ,'"' ,nhsn_xml_im_code ,'"' ,
   " displayName=" ,'"Intramuscular route"' ,"/>" ,"</observation>" ,"</entryRelationship>" ,
   "</encounter>" ,"</entry>" )
