...
/*****************************************************************************
        ...
        Source file name:       chs_wklist_cust_print_temp.prg
        Object name:            chs_wklist_cust_print_temp
        Request #:
...
;    *--- -------- -------------------- ----------------------------------- *
;    *001 mm/dd/yy                                                          *
;    *002 12/04/20 CS075950              CCPS-20131 modified consulting     *
...
CREATE PROGRAM chs_wklist_cust_print_temp:dba
 ...
 SUBROUTINE  getprintdata (null )
  ...
  SELECT INTO "nl:"
   dob = p.birth_dt_tm
   ,unit = uar_get_code_display (e.loc_nurse_unit_cd) ;003
...
DETAIL
 ...
 SUBROUTINE  createprinttemplatelayout (null )
  ...
  SET patientshtml = build2 (patientshtml ,"<thead>" ,"<tr>" ,'<td class="w5">' ,
   '<div class="cell-header">Location</div>' ,"</td>" ,'<td class="w5">' ,
   '<div class="cell-header">Age/Sex</div>' ,"</td>" ,'<td class="w15">' ,
   '<div class="cell-header">Patient</div>' ,"</td>" ,'<td class="w25">' ,
   '<div class="cell-header">Providers</div>' ,"</td>" ,'<td class="w15">' ,
   '<div class="cell-header">LOS</div>' ,"</td>" ,'<td class="w35">' ,
   '<div class="cell-header">Notes</div>' ,"</td>" ,"</tr>" ,"</thead>" )
  FOR (index = 1 TO numberofpatients )
   SET patientshtml = build2 (patientshtml ,"<tbody>" ,"<tr>" ,"<td>" ,'<div class="cell-content">' ,
    ...
    printdata->patients[index ].patient_data.age ," " ,printdata->patients[index ].sex ,"</div>" ,
    "</td>" ,"<td>" ,'<div class="cell-content">' ,printdata->patients[index ].name_full ,"</div>" ,
   ...
   att_phy ) )
    SET patientshtml = build2 (patientshtml ,
     '<div class="cell-content"><b><u>Primary Contact-</u></b>' ,printdata->patients[index ].
     primary_contact ,"</div>" ,'<div class="cell-content"><b><u>Attending-</u></b>' ,printdata->
     patients[index ].consultant.att_phy ,"</div>" ,
     '<div class="cell-content"><b><u>Admitting-</u></b>' ,printdata->patients[index ].consultant.
     adm_phy ,"</div>" )
   ELSE
    SET patientshtml = build2 (patientshtml ,
     '<div class="cell-content"><b><u>Primary Contact-</u></b>' ,printdata->patients[index ].
   ...
   IF ((printdata->patients[index ].length_of_stay = 0 ) )
    SET patientshtml = build2 (patientshtml ,'<div class="cell-content"></div>' )
   ELSE
    SET patientshtml = build2 (patientshtml ,'<div class="cell-content">' ,printdata->patients[index
     ].length_of_stay ," days</div>" ,'<div class="cell-content">Adm Dt:' ,printdata->patients[index
     ].admit_date ,"</div>" )
   ENDIF
   SET patientshtml = build2 (patientshtml ,"</td>" )
   SET patientshtml = build2 (patientshtml ,"<td>" ,'<div class="cell-content">' ,printdata->
    patients[index ].notes ,"</div>" ,"</td>" ,"</tr>" ,"<tr>" ,'<td colspan= "6">' ,
    '<div class="consults-content">consults: ' ,printdata->patients[index ].consultant.consul_phy ,
    "</div>" ,"</td>" ,"</tr>" ,"<tr>" ,'<td colspan="14" class="dash1"/>' )
  ...
  SET sfinalhtml = build2 ("<!doctype html>" ,"<html>" ,"<head>" ,'<meta charset="utf-8">' ,
   ...
   "Provider List" ,"</span>" ,"</div>" ,'<div class="printed-date">' ,"<span>" ,formatteddate ,
