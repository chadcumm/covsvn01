...
005		02/19/2020  Chad Cummings			added prod_desc and order_id to output
******************************************************************************/
			...
			for (j=1 to size(390051reply->wastelist,5))
				if (size(390051reply->wastelist[j].prodlist,5) > 0)
					for (k=1 to size(390051reply->wastelist[j].prodlist,5))					
						...
						set t_rec->qual[i].prod_desc = 390051reply->wastelist[j].prodlist[k].prod_desc	;005
						set t_rec->qual[i].status = 2
					endfor
				endif
			endfor
...
for (i = 1 to t_rec->cnt)
	...
	;005 set document_request->notetext = nullterm(build2("Waste Quantity: ",trim(cnvtstring(t_rec->qual[i].waste_qty,10,4))," EA")) ;004
	set document_request->notetext 				= build2("Waste Quantity: ",trim(cnvtstring(t_rec->qual[i].waste_qty,10,4))," EA");004;005
	set document_request->notetext 				= build2(document_request->notetext,char(10),t_rec->qual[i].prod_desc) ;005
	set document_request->notetext 	= build2(document_request->notetext,char(10),"ORDER_ID:",cnvtstring(t_rec->qual[i].order_id)) ;005
	set document_request->notetext	=	nullterm(document_request->notetext) ;005
