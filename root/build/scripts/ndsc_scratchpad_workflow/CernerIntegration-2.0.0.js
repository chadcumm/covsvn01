            ...
            GetConfigs(sessionInfo, data, function (configData, data) {
                ...
                GetResults(configurations, function (results, configs) {
                    if (results.Status === 0) {
                        ...
                        GetExamInfo(configs, results, function (examInfo, results) {
...
function GetResults(configs, callback) {
    ...
    $.ajax({
        ...
        success: function (data, status, req) {
            ...
            results.Claims = data.Responses[0].Claims;
...
function GetExamInfo(configs, results, sucessCallback, failCallback) {
    ...
    examRequest.open('GET', "PAMA_GET_ORDER_INFO");
...
function FormatOrder(examInfo, results, configs) {
    ...
    if (configs.Configuration.CernerCdsmHcpcsCodeOEFID && results.Claims && results.Claims.CdsmHcpcsCode) {
        if (detailCounter > 0) {
            newOrderData += '@@';
        }
        detailCounter++;
        newOrderData += configs.Configuration.CernerCdsmHcpcsCodeOEFID;
        newOrderData += '~~';
        newOrderData += 'OTHER';
        newOrderData += '~~';
        newOrderData += '0.00';
        newOrderData += '~~';
        newOrderData += results.Claims.CdsmHcpcsCode;
        newOrderData += '~~';
        var hcpcs = $.grep(examInfo.hcpcs, function (element, index) {
            return (element.oe_field_display_value === results.Claims.CdsmHcpcsCode);
        });
        if (hcpcs.length > 0) {
            newOrderData += hcpcs[0].oe_field_value;
            newOrderData += '.00';
        }
        else {
            newOrderData += '0.00';
    ...
    if (configs.Configuration.CernerCdsmHcpcsModifierOEFID && results.Claims && results.Claims.CdsmHcpcsModifier) {
        if (detailCounter > 0) {
            newOrderData += '@@';
        }
        detailCounter++;
        newOrderData += configs.Configuration.CernerCdsmHcpcsModifierOEFID;
        newOrderData += '~~';
        newOrderData += 'OTHER';
        newOrderData += '~~';
        newOrderData += '0.00';
        newOrderData += '~~';
        newOrderData += results.Claims.CdsmHcpcsModifier;
        newOrderData += '~~';
        var modifier = $.grep(examInfo.modifier, function (element, index) {
            return (element.oe_field_display_value === results.Claims.CdsmHcpcsModifier);
        });
        if (modifier.length > 0) {
            newOrderData += modifier[0].oe_field_value;
            newOrderData += '.00';
        }
        else {
            newOrderData += '0.00';
