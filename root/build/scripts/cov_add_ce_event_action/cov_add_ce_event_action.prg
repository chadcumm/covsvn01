...
set a_rec->request_prsnl_id		= $PRSNL_ID
set a_rec->event_id 			= $EVENT_ID
set a_rec->action_status 		= $ACTION_STATUS
set a_rec->action_type 			= $ACTION_TYPE

set a_rec->action_status_cd = uar_get_code_by("MEANING",103,nullterm(a_rec->action_status))
set a_rec->action_type_cd 	= uar_get_code_by("MEANING",21,nullterm(a_rec->action_type))


free record ensure_request2 
free record ensure_reply2 
 
record ensure_request2 (
   1 req                   [*]
      2 ensure_type           = i2
      2 version_dt_tm         = dq8
...
set stat = alterlist(ensure_request2->req, 1) 
set ensure_request2->req[1].ensure_type 			 		= 2 
set ensure_request2->req[1].version_dt_tm_ind 			= 1 
set ensure_request2->req[1].event_prsnl.event_id 		= a_rec->event_id
set ensure_request2->req[1].event_prsnl.action_type_cd 	= a_rec->action_type_cd  
set ensure_request2->req[1].event_prsnl.action_dt_tm 	= cnvtdatetime(curdate,curtime3) 
set ensure_request2->req[1].event_prsnl.updt_dt_tm 		= cnvtdatetime(curdate,curtime3)
set ensure_request2->req[1].event_prsnl.action_prsnl_id 	= a_rec->request_prsnl_id
set ensure_request2->req[1].event_prsnl.proxy_prsnl_id 	= 0.00 
set ensure_request2->req[1].event_prsnl.action_status_cd = a_rec->action_status_cd
set ensure_request2->req[1].event_prsnl.defeat_succn_ind = 1 
set ensure_request2->req[1].event_prsnl.action_comment 	= "" 

if (a_rec->cancel_ind = 0)
 if (a_rec->event_id > 0.0)
	...
	update into clinical_event 
			...
			where event_id = a_rec->event_id

			and valid_until_dt_tm >= cnvtdatetime(curdate, curtime3)
			and valid_from_dt_tm <= cnvtdatetime(curdate, curtime3)
			commit
	
	execute inn_event_prsnl_batch_ensure "MINE" with replace("ensure_request",ensure_request2),replace("ensure_reply",ensure_reply2)
 endif
