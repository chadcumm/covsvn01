DROP PROGRAM cov_update_ce :dba GO
CREATE PROGRAM cov_update_ce :dba
 ...
 SET reply->status_data.subeventstatus[1 ].targetobjectname = "UPDATE_CD.PRG"
 SET reply->status_data.subeventstatus[1 ].targetobjectvalue = "Error publishing the clinical event."

 CALL logmsg (debugflag ,concat ("start:" ,format (curdate ,"MM/DD/YYYY;;D" ) ," " ,format (curtime3,"@TIMEWITHSECONDS" ) ) )
 ...
 CALL logmsg (debugflag ,build ("request->service_dt_tm:" ,format (request->service_dt_tm , "dd-mmm-yyyy hh:mm;;D" ) ) )
 ...
 CALL logmsg (debugflag ,build ("request->mediaobjectGroups size:" ,size (request->mediaobjectgroups,5 ) ) )
 ...
 CALL logmsg (debugflag ,concat ("end:" ,format (curdate ,"MM/DD/YYYY;;D" ) ," " ,format (curtime3 , "@TIMEWITHSECONDS" ) ) )
...
SUBROUTINE  postclinicalevent (personid ,encntrid )
  ...
  DECLARE doc = f8 WITH protect ,constant (uar_get_code_by ("MEANING" ,53 ,"DOC" ) )
  ...
  CALL logmsg (debugflag ,concat ("begin:" ,format (curdate ,"MM/DD/YYYY;;D" ) ," " ,format (curtime3 ,"@TIMEWITHSECONDS" ) ) )
  DECLARE i18nhandle = i4 WITH noconstant (0 )
  SET srvstat = uar_i18nlocalizationinit (i18nhandle ,curprog ,"" ,curcclrev )
  DECLARE snote = vc WITH noconstant (uar_i18ngetmessage (i18nhandle ,"documentimage" ,"Document includes an Image." ) )
  ...
  IF ((event_cd = 0.0 ) )
   SET reply->status_data.subeventstatus[1].targetobjectvalue = build("Invalid request->documentType_key:"
   	,request->documenttype_key)
   CALL logmsg (debugflag ,build ("Invalid request->documentType_key:" ,request->documenttype_key ,":" ,event_cd ) )
   RETURN
  ENDIF
  ...
  IF ((iret != 0 ) )
   SET reply->status_data.subeventstatus[1 ].targetobjectvalue ="uar_CrmBeginApp failed in mmf_publish_ce::PostClinicalEvent"
  ...
  IF ((iret != 0 ) )
   SET reply->status_data.subeventstatus[1 ].targetobjectvalue = "uar_CrmBeginTask failed in mmf_publish_ce::PostClinicalEvent"
  ...
  IF ((iret != 0 ) )
    SET reply->status_data.subeventstatus[1 ].targetobjectvalue ="uar_CrmBeginReq failed in mmf_publish_ce::PostClinicalEvent"
   CALL logmsg (debugflag ,"uar_CrmBeginReq failed in mmf_publish_ce::PostClinicalEvent" )
   RETURN
  ENDIF
  
  CALL logmsg (debugflag ,build ("CURRENT_DATE_TIME:" ,format (current_date_time ,"dd-mmm-yyyy hh:mm;;D" ) ) )
  SET passed_date_time = request->service_dt_tm
  CALL logmsg (debugflag ,build ("request->dateofservice_dt_tm:" ,format (request->service_dt_tm ,"dd-mmm-yyyy hh:mm;;D" ) ) )
  CALL logmsg (debugflag ,build ("PASSED_DATE_TIME:" ,format (passed_date_time ,"dd-mmm-yyyy hh:mm;;D" ) ) )
  CALL logmsg (debugflag ,build ("person_id:" ,personid ) )
  
  ;Get the CE struct and set required items
  ;***** MDOC *****
  SET hreq = uar_crmgetrequest (hstep )
  SET srvstat = uar_srvsetshort (hreq ,"ensure_type" ,1 )
  SET hmdoc = uar_srvgetstruct (hreq ,"clin_event" )
  SET srvstat = uar_srvsetshort (hmdoc ,"ensure_type" ,258 )
  ...
  SET srvstat = uar_srvsetstring (hmdoc ,"series_ref_nbr" ,"CCC1134175747" ) 
  ...
  FOR (iindex = 1 TO size (request->personnel ,5 ) )
   ...
   IF (hprsnl )
    ...
    SET stat = uar_srvsetdouble (hprsnl ,"action_type_cd" ,uar_get_code_by ("MEANING" ,21 ,request->personnel[iindex ].action ) )
    ...
    SET stat = uar_srvsetdouble (hprsnl ,"action_status_cd" ,uar_get_code_by ("MEANING" ,103 ,request->personnel[iindex ].status ) )
   ENDIF
  ENDFOR
  
  
  ;;;;;; DOC ROW ;;;;;;
  ;Bind child event list
  ...
  SET srvstat = uar_srvsetdouble (hdoc ,"event_class_cd" ,doc )
  ...
  IF ((request->publishasnote = 0 ) )
   ...
   IF (hblob )
    ...
    SET srvstat = uar_srvsetasis (hblob ,"blob_contents" ,nullterm (trim (snote ) ) ,size (trim (snote ) ) )
  ...
  ;***** build the list of object to publish
  SET istatus = alterlist (publishlist->objectlist ,size (request->mediaobjects ,5 ) )
  FOR (iindex = 1 TO size (request->mediaobjects ,5 ) )
   SET publishlist->objectlist[iindex ].displayname = nullterm (request->mediaobjects[iindex ].display )
   IF ((publishlist->objectlist[iindex ].displayname = "" ) )
    SET publishlist->objectlist[iindex ].displayname = getobjectname (request->mediaobjects[iindex ].identifier )
   ENDIF
   IF ((validateid (request->mediaobjects[iindex ].identifier ) = 1 ) )
    SET publishlist->objectlist[iindex ].mediaobjectid = nullterm (request->mediaobjects[iindex ].identifier )
   ELSE
  ...
  FOR (iindex = 1 TO size (request->mediaobjectgroups ,5 ) )
   CALL logmsg (debugflag ,build (iindex ,": group identifier : " ,request->mediaobjectgroups[iindex].identifier ) )
   IF ((validateid (request->mediaobjectgroups[iindex ].identifier ) = 0 ) )
    SET reply->status_data.subeventstatus[1 ].targetobjectvalue = "invalid identifier"
    CALL logmsg (debugflag ,build ("invalid identifier:" ,request->mediaobjectgroups[iindex ].identifier ) )
    RETURN
   ENDIF
  ...
  FOR (iindex = 1 TO size (publishlist->objectlist ,5 ) )
   CALL logmsg (debugflag ,build (publishlist->objectlist[iindex].mediaobjectid ,":" ,publishlist->objectlist[iindex ].displayname))
   IF ((request->publishasnote = 0 ) )
    ; publish as attachment (default)
    ...
    SET srvstat = uar_srvsetdouble (hattachment ,"event_id" ,request->event_id )
    ...
    SET srvstat = uar_srvsetstring (hattachment ,"event_title_text" ,nullterm (publishlist->objectlist[iindex ].displayname ) )
   ...
   ;;;;; BLOB RESULT
   ;Add the Result blob to child event
   SET stat = uar_srvsetdouble (hbr ,"person_id" ,personid )
   ...
   ELSE
    ...
    IF ((publishas_cd = 0.0 ) )
     SET reply->status_data.status = "F"
     SET reply->status_data.subeventstatus[1 ].targetobjectvalue = build ("Invalid media type for publishasNote" )
   ...
   SET srvstat = uar_srvsetstring (hbr ,"blob_handle" ,nullterm (trim (publishlist->objectlist[iindex ].mediaobjectid ) ) )
  ...
  IF ((iret = 0 ) )
   ...
   IF ((hrep > 0 ) )
    ...
    ; build key in the format urn:cerner:mid:mmf.mom.clinical_event:provide:3045846252
    SET requestowner->ownership_uid=build("urn:cerner:mid:mmf.mom.clinical_event",":",cnvtupper(curdomain),":",reply->parenteventid)
    ...
    IF ((reply->status_data.status = "S" ) )
     FOR (iindex = 1 TO size (publishlist->objectlist ,5 ) )
      ...
      CALL logmsg (debugflag ,build ("   requestOwner->media_object_identifier:" ,requestowner->media_object_identifier ) )
      
      EXECUTE mmf_add_ownership WITH replace ("REQUEST" ,"REQUESTOWNER" ) ,replace ("REPLY" ,"REPLYOWNER" )
      ...
      IF ((replyowner->status_data.status = "F" ) )
       ...
       SET reply->status_data.subeventstatus[1 ].targetobjectvalue 
       								= build ("no ownership for " ,requestowner->media_object_identifier )
      ENDIF
     ENDFOR
    ENDIF
   ENDIF ;hRep
  ENDIF; iRep - uar_CrmPerform
  ...
  ENDIF
  RETURN (0 ) 
 END ;Subroutine postclinicalevent (personid ,encntrid )
 
 
 
 ;***** cer_log:u-notify.MMDD.log
 SUBROUTINE  logmsg (flag ,message )
  ...
  ENDIF
  CALL echo (slogmsg )
 END ;Subroutine logmsg (flag ,message )
 ...
 SUBROUTINE  geteventcode (cvkey )
  ...
  DECLARE contributorsourcecd = f8 WITH protect ,constant (uar_get_code_by ("MEANING" ,73 ,"POWERCHART" ) )
  ...
  RETURN (eventcd )
 END ;Subroutine geteventcode (cvkey )
 ...
 END ;Subroutine getformat (identifier )
 ...
 SUBROUTINE  getobjectname (identifier )
  ...
  RETURN (medianame )
 END ;Subroutine getobjectname (identifier )
 ...
 SUBROUTINE  validateid (identifier )
  ...
  ;end select
  RETURN (validated )
 END ;Subroutine  validateid (identifier )
