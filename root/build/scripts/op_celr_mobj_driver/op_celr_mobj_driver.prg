 ...
 *  ---------------------------------------------------------------------------------------------
 */
execute oencpm_msglog(build("start script:",trim("op_celr_mobj_driver"),char(0) ))
execute oencpm_msglog(build(">declare",char(0) ))
declare encntr = i4
declare clia_nbr = vc
declare org_name = vc
...
execute oencpm_msglog(build(">encntr=",trim(cnvtstring(encntr)),char(0) ))
...
***************************************/
/*
execute oencpm_msglog(build(">oen_reply->control_group[1 ].msh[1 ].version.version_id=",
	oen_reply->control_group[1 ].msh[1 ].version.version_id,char(0) ))
	
set oen_reply->control_group[1 ].msh[1 ].version.version_id = "2.5.1"

execute oencpm_msglog(build("<oen_reply->control_group[1 ].msh[1 ].version.version_id=",
	oen_reply->control_group[1 ].msh[1 ].version.version_id,char(0) ))

execute oencpm_msglog(build(">celr_mobj_oru_out start",char(0) ))
;execute celr_mobj_oru_out
...
/* Get CLIA Number */
/*
execute oencpm_msglog(build(">Get CLIA Number start",char(0) ))
select into "nl:"
from
...
/*
execute oencpm_msglog(build(">Get Org_name via encounter_id",char(0) )) 
...
execute oencpm_msglog(build(">org_name=",trim(org_name),char(0) ))  
execute oencpm_msglog(build(">clia_nbr=",trim(clia_nbr),char(0) ))  

/*
execute oencpm_msglog(build(">Setting OEN_REPLY",char(0) )) 
...
execute oencpm_msglog(build(">oen_reply->CONTROL_GROUP [1]->MSH [1]->sending_facility->name_id=",
	trim(oen_reply->CONTROL_GROUP [1]->MSH [1]->sending_facility->name_id),char(0) ))  
execute oencpm_msglog(build(">oen_reply->CONTROL_GROUP [1]->MSH [1]->sending_facility->univ_id=",
	trim(oen_reply->CONTROL_GROUP [1]->MSH [1]->sending_facility->univ_id),char(0) ))  
execute oencpm_msglog(build(">oen_reply->CONTROL_GROUP [1]->MSH [1]->sending_facility->univ_id_type=",
...
subroutine get_double_value (string_meaning)
  execute oencpm_msglog(build("->get_double_value:",trim(string_meaning),char(0) ))
  declare eso_idx = i4
...
execute oencpm_msglog(build("end script:",trim(curprog),char(0) ))
end go
