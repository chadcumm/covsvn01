...
for (i = 1 to t_rec->cnt)
	...
	set stat = alterlist(t_rec->qual[i].step_log,t_rec->qual[i].step_id)
	set t_rec->qual[i].step_log[t_rec->qual[i].step_id].step_title 		= "ROW PARSE"
	set t_rec->qual[i].step_log[t_rec->qual[i].step_id].step_message 	= "File row was split properly"
	set t_rec->qual[i].step_log[t_rec->qual[i].step_id].step_status		= "S"
endfor
