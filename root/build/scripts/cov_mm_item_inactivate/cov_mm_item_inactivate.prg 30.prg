...
for (i = 1 to t_rec->cnt)
	...
	if ((t_rec->qual[i].item_id > 0.0) and  (t_rec->qual[i].step_id	in(3,4)))
		call writeLog(build2("***Item passed step 3 or 4 ",trim(cnvtstring(t_rec->qual[i].item_alias)),":",
			trim(cnvtstring(t_rec->qual[i].item_id))))
		if ((t_rec->qual[i].surgery_loc_flag = 2) and (size(t_rec->qual[i].surgery_loc,5) > 0))
			...
			call writeLog(build2("---->",900067_reply->status_data.status))
			
			if (900067_reply->status_data.status = "S")
				set t_rec->qual[i].surgery_loc_flag = 5
				for (k = 1 to size(900067_reply->qual,5))
					call writeLog(build2("-------->Removed: ",900067_reply->qual[k].location_disp,"(",
						trim(cnvtstring(900067_reply->qual[k].location_cd)),")"))
				endfor 
			endif
		endif
	endif
