...
for (i = 1 to t_rec->cnt)
	if (t_rec->qual[i].item_id > 0.0)
		...
		detail
			cnt = (cnt + 1)
			disp_line = concat(disp_line,uar_get_code_display(sc.catalog_cd),";")
		foot report
			if (cnt = 0)
				...
				stat = alterlist(t_rec->qual[i].step_log,t_rec->qual[i].step_cnt)
				t_rec->qual[i].step_log[t_rec->qual[i].step_cnt].step_title 		= "PREF_CARD"
				t_rec->qual[i].step_log[t_rec->qual[i].step_cnt].step_message 		= concat("Item on Preference cards for:",disp_line)
