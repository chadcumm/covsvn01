...
for (i = 1 to t_rec->cnt)
	call writeLog(build2("****Checking Surger and POU Status for ",trim(cnvtstring(t_rec->qual[i].item_alias)),":",
			trim(cnvtstring(t_rec->qual[i].item_id))))
	if ((t_rec->qual[i].pou_loc_flag = 1) and (t_rec->qual[i].surgery_loc_flag = 1) and (t_rec->qual[d1.seq].step_id = 6))
