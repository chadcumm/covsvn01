...
for (i = 1 to t_rec->cnt)
	...
	set 900060_request->active_status_qual[1].active_status_cd  = t_rec->active_ind
	
	set stat = tdbexecute(900055, 900060, 900060, "REC", 900060_request, "REC", 900060_reply) 
	call writeLog(build2("--->900060_request->status_data.status=",900060_request->status_data.status))
	if (900060_request->status_data.status = "S")
		...
		elseif (size(900060_reply->qual,5) < 1)
			...
			set t_rec->qual[i].item_id			= 900060_reply->qual[i].item_id 
			set t_rec->qual[i].step_id			= 2
		endif
	
	elseif (900060_request->status_data.status = "Z")
		set t_rec->qual[i].error_status = concat("Item Could not be found based on identifier:",t_rec->qual[i].item_alias)
	else
...
for (i = 1 to t_rec->cnt)
	...
	if ((t_rec->qual[i].item_id > 0.0) and  (t_rec->qual[i].step_id	in(3)))
		call writeLog(build2("----->Item passed step 3 ",trim(t_rec->qual[i].item_alias),":",
			trim(cnvtstring(t_rec->qual[i].item_id))))
