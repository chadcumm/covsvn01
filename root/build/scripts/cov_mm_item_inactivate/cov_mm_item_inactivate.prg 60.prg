...
for (i = 1 to t_rec->cnt)
	if (t_rec->qual[i].item_id > 0.0)
		...
		if (900128_reply->status_data.status = "S")
			for (ii = 1 to size(900128_reply->qual,5))
				for (j = 1 to size(900128_reply->qual[ii].id_qual,5))
					if (900128_reply->qual[ii].id_qual[j].id_type_mean in("DESC_SHORT","DESC","DESC_CLINIC"))
						set stat = 0
					endif
				endfor
			endfor
