...
for (i = 1 to t_rec->cnt)
	if (t_rec->qual[i].item_id > 0.0)
		...
		call writeLog(build2("**Calling 900005_request for ",trim(cnvtstring(t_rec->qual[i].item_id))))
	...
	if (t_rec->qual[i].item_id > 0.0)
		...
		if (900005_reply->status_data.status = "S")
			...
			for (j = 1 to size(900005_reply->qual,5))
				for (k = 1 to size(900005_reply->qual[j].loc_qual,5))
					set cnt = (cnt + 1)
					set stat = alterlist(t_rec->qual[i].pou_loc_flag,cnt)
					set t_rec->qual[i].pou_loc_flag[cnt].location_cd = 900005_reply->qual[j].loc_qual[k].location_cd
				endfor
			endfor
		elseif (900005_reply->status_data.status = "Z")
