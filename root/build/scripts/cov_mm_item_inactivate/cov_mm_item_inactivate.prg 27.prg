	...
	if ((t_rec->qual[i].item_id > 0.0) and  (t_rec->qual[i].step_id	in(3,4)))
		if ((t_rec->qual[i].surgery_loc_flag = 2) and (size(t_rec->qual[i].surgery_loc,5) > 0))
			...
			for (j = 1 to size(t_rec->qual[i].surgery_loc,5))
				set cnt = (cnt + 1)
				set stat = alterlist(request->qual,cnt)
				set request->qual[cnt].item_id 			= t_rec->qual[i].item_id
				set request->qual[cnt].ic_location_cd 	= t_rec->qual[i].surgery_loc[j].location_cd
				set request->qual[cnt].ic_dirty 		= 3 
			endfor
