...
for (i=1 to size(request->encounter_list,5))
	for (k=1 to size(request->encounter_list[i].order_list,5))
		...
		for (j=1 to size(request->encounter_list[i].order_list[k].detail_list,5))
			if (request->encounter_list[i].order_list[k].detail_list[j].oe_field_meaning  ="REQSTARTDTTM")
			set request->encounter_list[i].order_list[k].detail_list[j].oe_field_dt_tm_value = cnvtdatetime(Curdate,curtime3)
			endif
		endfor
	endfor
endfor
