drop program cov_send_a31_by_encntr_id:dba go
create program cov_send_a31_by_encntr_id:dba
...
set a31_rec->encntr_id = $ENCNTR_ID
...
from
	...
	,person p
plan ea
	where ea.encntr_id = a31_rec->encntr_id
	and   ea.active_ind = 1
...
head e.encntr_id
	cnt = cnt +1
	if(mod(cnt,100) = 1)
		stat = alterlist(a31_rec->qual, cnt +99)
	endif
	call writeLog(build2("->Adding encounter=",trim(cnvtstring(e.encntr_id))))
	a31_rec->qual[cnt].encntr_id = e.encntr_id
	a31_rec->qual[cnt].person_id = e.person_id
	a31_rec->fin = ea.alias
foot report
	 stat = alterlist(a31_rec->qual, cnt)
	 a31_rec->cnt = cnt
	 call writeLog(build2("-->Total to process:",trim(cnvtstring(a31_rec->cnt))))
with nocounter
...
for (i = 1 to size(a31_rec->qual,5))
	;call send_outbound(a31_rec->qual[i].person_id,a31_rec->qual[i].encntr_id,a31_rec->qual[i].person_id,"A08")
	call send_outbound(a31_rec->qual[i].person_id,a31_rec->qual[i].encntr_id,a31_rec->qual[i].person_id,"A31")
endfor
