...
******************************************************************************
  ...
  Solution:           
  Source file name:   cov_send_a02_by_encntr_id.prg
  Object name:        cov_send_a02_by_encntr_id
  Request #:
...
---   ----------  --------------------  --------------------------------------
001   03/01/2020  Chad Cummings			initial build
******************************************************************************/
drop program cov_send_a02_by_encntr_id:dba go
create program cov_send_a02_by_encntr_id:dba
...
set a02_rec->encntr_id = $ENCNTR_ID
...
from
	...
	,person p
plan ea
	where ea.encntr_id = a02_rec->encntr_id
	and   ea.active_ind = 1
...
head e.encntr_id
	cnt = cnt +1
	if(mod(cnt,100) = 1)
		stat = alterlist(a02_rec->qual, cnt +99)
	endif
	call writeLog(build2("->Adding encounter=",trim(cnvtstring(e.encntr_id))))
	a02_rec->qual[cnt].encntr_id = e.encntr_id
	a02_rec->qual[cnt].person_id = e.person_id
	a02_rec->fin = ea.alias
foot report
	 stat = alterlist(a02_rec->qual, cnt)
	 a02_rec->cnt = cnt
	 call writeLog(build2("-->Total to process:",trim(cnvtstring(a02_rec->cnt))))
with nocounter
...
for (i = 1 to size(a02_rec->qual,5))
	;call send_outbound(a02_rec->qual[i].person_id,a02_rec->qual[i].encntr_id,a02_rec->qual[i].person_id,"A08")
	call send_outbound(a02_rec->qual[i].person_id,a02_rec->qual[i].encntr_id,a02_rec->qual[i].person_id,"A02")
endfor
