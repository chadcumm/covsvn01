					...
					if (t_rec->temp_orderlist[t_rec->temp_orderlist_cnt].event_id > 0.0)
						...
						while (order_string != notfnd)
							...
							if ((pos = 0) and (cnvtreal(order_string) > 0))
								...
								set t_rec->temp_list[t_rec->temp_cnt].action_type_cd = requestin->request->orderlist[i].ACTIONTYPECD
...
;head o.order_id
detail
	call writeLog(build2("---->inside the d.seq=",trim(cnvtstring(d.seq))))
