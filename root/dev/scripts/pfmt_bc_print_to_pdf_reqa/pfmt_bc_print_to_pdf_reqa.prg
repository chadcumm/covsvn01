...
******************************************************************************
  ...
  Solution:           
  Source file name:   pfmt_bc_print_to_pdf_reqa.prg
  Object name:        pfmt_bc_print_to_pdf_reqa
  Request #:
...
if (not validate(requestin->personid,0))
...
set t_rec->log_filename_a 			= concat ("cclscratch:play_560201_records_" ,t_rec->sysdate_string ,".dat" )
...
detail
	...
	if ((p.name_first_key = "PLAYFIVE") and (p.name_last_key = "CSTPDF*"))
		bc_common->valid_ind = 1
	endif
	if ((p.name_first_key = "PLAYAA") and (p.name_last_key = "CSTPDF*"))
		bc_common->valid_ind = 1
	endif
	if ((p.name_first_key = "PLAYBB") and (p.name_last_key = "CSTPDF*"))
		bc_common->valid_ind = 1
	endif
with nocounter

call writeLog(build2("* Validate Location=",cnvtstring(bc_common->location_cnt)))
call writeLog(build2("* orderlist=",cnvtstring(size(requestin->orderlist,5))))
call writeLog(build2("* 1st encounter id=",cnvtstring(requestin->orderlist[1].originatingencounterid)))
call writeLog(build2("* encounter id=",cnvtstring(requestin->ENCNTRID)))
*/
...
/*	select into "nl:"
	from (dummyt d1 with seq=size(requestin->orderlist,5))
		,encounter e
	plan d1
		where requestin->orderlist[d1.seq].originatingencounterid > 0.0
	join e where e.encntr_id = requestin->orderlist[d1.seq].originatingencounterid
	head report
		call writeLog(build2("->Inside Order Location Query"))
		cnt = 0
	detail
		call writeLog(build2("requestin->orderlist[d1.seq].originatingencounterid=",
			cnvtstring(requestin->orderlist[d1.seq].originatingencounterid)))
...
*/
	...
	from
		...
		,orders o
	plan d1
		where requestin->orderlist[d1.seq].orderid > 0.0
	join o
...
for (i=1 to size(requestin->orderlist,5))
	call writeLog(build2("--->order_id=",trim(cnvtstring(requestin->orderlist[i].orderid))))
	call writeLog(build2("--->action_type_cd=",trim(cnvtstring(requestin->orderlist[i].ACTIONTYPECD))))
	call writeLog(build2("--->action_type=",trim(uar_get_code_display(requestin->orderlist[i].ACTIONTYPECD))))
	call writeLog(build2("--->requisition_format_cd=",trim(cnvtstring(requestin->orderlist[i].REQUISITIONFORMATCD))))
	
	if (requestin->orderlist[i].ACTIONTYPECD in(
		...
		for (j=1 to bc_common->requisition_cnt)
			if (requestin->orderlist[i].REQUISITIONFORMATCD = bc_common->requisition_qual[j].requisition_format_cd)
				set pass_ind = 1
			endif
		endfor
		...
		if (uar_get_code_meaning(requestin->orderlist[i].REQUISITIONFORMATCD) in(
																							"AMBREFERREQ"
																							,"PROVECHOREQ"
																							,"CRDASTATREQ"
																							,"MIREQUISITN"))
			for (j=1 to size(requestin->orderlist[i]->detaillist,5))
				if (requestin->orderlist[i]->detaillist[j].oefieldid in(bc_common->scheduling_location_field_id,
					...
					call writeLog(build2("----->found oef=",requestin->orderlist[i]->detaillist[j].oefieldid))
					call writeLog(build2("----->found oef value=",requestin->orderlist[i]->detaillist[j].oefieldvalue))
					if (requestin->orderlist[i]->detaillist[j].oefieldvalue 
						not in(bc_common->print_to_paper_cd,bc_common->paper_referral_cd))
						if (requestin->orderlist[i]->detaillist[j].oefieldvalue > 0.0)
							set pass_ind = 0
						endif
					endif
				endif
		...
		if (pass_ind = 1)
			call writeLog(build2("----->inside pass_ind = 1"))
				...
				set t_rec->temp_orderlist[t_rec->temp_orderlist_cnt].order_id = requestin->orderlist[i].orderid
				set t_rec->temp_orderlist[t_rec->temp_orderlist_cnt].pathway_catalog_id	= requestin->orderlist[i].PATHWAYCATALOGID
				set t_rec->temp_orderlist[t_rec->temp_orderlist_cnt].encntr_id = requestin->orderlist[i].ORIGINATINGENCOUNTERID
				set t_rec->temp_orderlist[t_rec->temp_orderlist_cnt].action_type_cd = requestin->orderlist[i].ACTIONTYPECD
				/* start 015 */
				...
				set t_rec->temp_orderlist[t_rec->temp_orderlist_cnt].requisition_format_cd 
						= requestin->orderlist[i].REQUISITIONFORMATCD
				set t_rec->temp_orderlist[t_rec->temp_orderlist_cnt].requisition_format 
						= uar_get_code_display(requestin->orderlist[i].REQUISITIONFORMATCD)
				set t_rec->temp_orderlist[t_rec->temp_orderlist_cnt].action_type_mean 
				;015	= uar_get_code_meaning(requestin->orderlist[i].ACTIONTYPECD)
				...
				set t_rec->temp_orderlist[t_rec->temp_orderlist_cnt].order_mnemonic = 
					requestin->orderlist[i].PRIMARYMNEMONIC
				; double check PATHWAYCATALOGID, some requestin doesn't appear to send it
				if (requestin->orderlist[i].PATHWAYCATALOGID = 0.0)
					select into "nl:"
					from orders o where o.order_id = requestin->orderlist[i].orderid
					detail
				...
				if (	(requestin->orderlist[i].PATHWAYCATALOGID > 0.0) and 
						(size(requestin->orderlist[i].PROTOCOLINFO,5) > 0) and ;set to 0 if not DOT
						(uar_get_code_meaning(requestin->orderlist[i].ACTIONTYPECD) in("ORDER")))
						
						select into "nl:"
						from 
							 act_pw_comp apc
							,pathway p 
						where 	apc.parent_entity_id = requestin->orderlist[i].orderid
				...
				if (
					...
					;start 024
					and ( (parser(concat(^ce.reference_nbr="*^,trim(cnvtstring(requestin->orderlist[i].orderid)),^*"^))) 
					    or (parser(concat(^ce.normal_ref_range_txt="*^,trim(cnvtstring(requestin->orderlist[i].orderid)),^*"^)))
					    )
					 ;end 024
					order by
						 ce.reference_nbr
					...
					if ((t_rec->temp_orderlist[t_rec->temp_orderlist_cnt].event_id > 0.0) and
						...
						while (order_string != notfnd)
							...
							if ((pos = 0) and (cnvtreal(order_string) > 0))
								...
								set t_rec->temp_list[t_rec->temp_cnt].encntr_id = requestin->orderlist[i].ORIGINATINGENCOUNTERID
								set t_rec->temp_list[t_rec->temp_cnt].action_type_cd = value(uar_get_code_by("MEANING",6003,"MODIFY"))
												;requestin->orderlist[i].ACTIONTYPECD
								...
								set t_rec->temp_list[t_rec->temp_cnt].action_type_mean 
										= "MODIFY" ;uar_get_code_meaning(requestin->orderlist[i].ACTIONTYPECD)
...
;005 end */
	...
	/*end 023*/
		...
		;;t_rec->grouplist[gcnt].orderlist[cnt].action_cd 			= requestin->orderlist[d.seq].actiontypecd ;013
