		...
		if (t_rec->grouplist[i].order_cnt > 0)
			...
			if (t_rec->grouplist[i].action_type in("MODIFY","ORDER"))
				...
				for (j=1 to t_rec->grouplist[i].order_cnt)
						...
						call writeLog(build2("-->sONCOLOGY_POWERPLAN_ORDER(req_request->order_qual[req_request->cnt ].order_id )="
							,trim(cnvtstring(sONCOLOGY_POWERPLAN_ORDER(req_request->order_qual[req_request->cnt ].order_id)))))
								
						call writeLog(build2("-->sLAB_DOT_ORDER(request->order_qual[req_request->cnt ].order_id )="
								,trim(cnvtstring(sLAB_DOT_ORDER(request->order_qual[req_request->cnt ].order_id )))))
						  ...
						  if (sONCOLOGY_POWERPLAN_ORDER(req_request->order_qual[req_request->cnt ].order_id ) = 1 )
         					 ...
         					 elseif (sLAB_DOT_ORDER(request->order_qual[req_request->cnt ].order_id ) = 1 ) 
            					;Day of Treatment
            					set req_request->order_qual[req_request->cnt].conversation_id = 2.00     
          					elseif (sLAB_DOT_ORDER(request->order_qual[req_request->cnt ].order_id ) = 2 ) 
            					;Single & Multi-phase
