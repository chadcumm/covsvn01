...
set debug_ind = 0	;0 = no debug, 1=basic debug with echo, 2=msgview debug ;000
...
detail
	...
	;THIS WILL PROCESS PLAY PATIENTS
...
if ((bc_common->location_cnt > 0) and (bc_common->valid_ind = 1))
	...
	join e
		where e.encntr_id = requestin->request->orderlist[d1.seq].originatingencounterid
	...
	if (requestin->request->orderlist[i].ACTIONTYPECD in(
		...
		if (pass_ind = 1)
			for (j=1 to size(requestin->request->orderlist[i].detaillist,5))
				if (requestin->request->orderlist[i].detaillist[j].oefieldid 
						in(bc_common->scheduling_location_field_id,bc_common->scheduling_location_field_non_radiology_id))
					if (requestin->request->orderlist[i].detaillist[j].oefieldvalue not 
						in(bc_common->paper_referral_cd,bc_common->print_to_paper_cd))
						set pass_ind = 0
					endif
				endif
			endfor
		...
		if (pass_ind = 1)
			call writeLog(build2("----->inside pass_ind = 1"))
				...
				if (
						...
						(t_rec->temp_orderlist[t_rec->temp_orderlist_cnt].action_type_mean in("MODIFY","CANCEL","RESCHEDULE")) ;003
						)
				...
				if (
					...
					if ((t_rec->temp_orderlist[t_rec->temp_orderlist_cnt].event_id > 0.0) and
						...
						set k = 1
						set order_string = piece(t_rec->temp_orderlist[t_rec->temp_orderlist_cnt].reference_nbr,":",k,notfnd)
...
for (i=1 to t_rec->temp_orderlist_cnt)
	...
	if (t_rec->temp_orderlist[i].requisition_format = "AMB_REFERRAL_REQ")
		if (t_rec->temp_orderlist[i].sCHILD_ORDER = 1)
	...
	if (t_rec->temp_orderlist[i].action_type_mean = "RESCHEDULE")
		...
		if (substring(1,9,t_rec->temp_orderlist[i].event_title_text) in("ACTIONED:","MODIFIED:"))  ;003
			set t_rec->temp_orderlist[i].action_type_mean = ""		  ;003
			set t_rec->temp_orderlist[i].action_type_cd = 0.0 ;003
			set  t_rec->temp_orderlist[i].order_id = 0.0 ;003
		else ;003
...
if (t_rec->temp_powerplan_flag = 0) ;MIGHT NEED TO RESET TO = 0
	...
	join e
		where e.encntr_id	= o.originating_encntr_id
	join apc
		where apc.parent_entity_id = o.protocol_order_id
	join pc1
		where pc1.pathway_comp_id =  apc.pathway_comp_id
	join pc2
		where pc2.pathway_catalog_id = pc1.pathway_catalog_id
