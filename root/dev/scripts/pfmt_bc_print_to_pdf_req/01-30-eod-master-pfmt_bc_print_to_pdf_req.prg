		...
		for (j=1 to bc_common->requisition_cnt)
			if (requestin->request->orderlist[i].REQUISITIONFORMATCD = bc_common->requisition_qual[j].requisition_format_cd)
				...
				if (	(requestin->request->orderlist[i].PATHWAYCATALOGID > 0.0) and 
						(size(requestin->request->orderlist[i].PROTOCOLINFO,5) = 0) and
						(uar_get_code_meaning(requestin->request->orderlist[i].ACTIONTYPECD) in("ORDER")))
						select into "nl:"
						from act_pw_comp apc,pathway p where apc.parent_entity_id = requestin->request->orderlist[i].orderid
						and p.pathway_id = apc.pathway_id and p.order_dt_tm < apc.created_dt_tm
						with nocounter
...
if (t_rec->temp_powerplan_flag = 0)
	...
	head apc2.pathway_id
		...
		call writeLog(build2("looking at t_rec->grouplist[gcnt].orderlist[cnt].pathway_id="
			,trim(cnvtstring(t_rec->grouplist[gcnt].orderlist[cnt].pathway_id))))
		cnt = 0
