...
******************************************************************************
	...
	Source file name:	cov_rm_LabOrders_Audit.prg
	Object name:		cov_rm_LabOrders_Audit
	Request #:			4631
	...
	Executing from:		CCL
 	...
 	Special Notes:		Output files:
 							lab_orders_scheduled.txt
 							lab_orders_clinic.txt
...
drop program cov_rm_LabOrders_Audit:DBA go
create program cov_rm_LabOrders_Audit:DBA
...
declare file_clin_var			= vc with constant("lab_orders_clinic.txt")
...
declare filepath_sch_var		= vc with constant(build("/cerner/w_custom/", cnvtlower(curdomain),
														 "_cust/to_client_site/RevenueCycle/Registration/", file_sch_var))
 
declare filepath_clin_var		= vc with constant(build("/cerner/w_custom/", cnvtlower(curdomain),
														 "_cust/to_client_site/RevenueCycle/Registration/", file_clin_var))
...
;		with nocounter, nullreport, pcformat (^"^, ^,^, 1), format = stream, format, landscape, compress, maxcol = 192
		with nocounter, nullreport, formfeed = none, pcformat (^"^, ^,^, 1), format = stream, format, landscape, compress, maxcol = 192
	else
	...
	head page
		...
		col 0	"Report: cov_rm_LabOrders_Audit"
		row + 2
