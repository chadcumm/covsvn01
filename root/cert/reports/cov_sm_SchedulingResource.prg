...
******************************************************************************
	...
	Request #:			3613, 4365, 4481, 5092, 5516
...
******************************************************************************
 	...
 	02/07/2019	Todd A. Blanchard		Added prompt for physician group.
 										Adjusted prompt for request list queue.
 	02/20/2019	Todd A. Blanchard		Added DOB with time zone adjustment.
 	03/21/2019	Todd A. Blanchard		Adjusted query for prior auth data.
 	08/13/2019	Todd A. Blanchard		Adjusted criteria to include all resources.
 	09/05/2019	Todd A. Blanchard		Added comments.
 	10/01/2019	Daniel Claus			Added Physician Group filter to prompts and query
...
with OUTDEV, request_queue, patient, physician_group
...
WITH nocounter, expand = 1, maxrec = 4000, time = 60
...
head o.order_id
	...
	if (sea.order_status_meaning not in ("CANCELED", "COMPLETED", "DISCONTINUED"))
		...
		sched_obj->list[idx].sch_inst = trim(od2.oe_field_display_value, 3)
		sched_obj->list[idx].special_inst = trim(od3.oe_field_display_value, 3)
	endif
