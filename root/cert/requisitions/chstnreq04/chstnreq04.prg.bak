...
;    *028 12/10/18 ccummin4             for rad, using order_action prsnl   *
;										if last modified by discern expert  *
;~DE~************************************************************************
...
*****************************************************************************/
declare modnum				= c4 with protect,constant("v028") ;026;027;028
...
;021-new logic for printing
  ...
  AND (oa.action_type_cd in (order_cd,modify_cd,discont_cd,activate_cd))
   AND o.template_order_flag != 7)
	...
	orders->qual[ord_cnt]->display_ind = 1
    if ((oa.action_type_cd = modify_cd) and (oa.action_personnel_id = 1.0) and (oa.action_sequence = 2));028
    	orders->rad_cpt_upd_ind = 1																	;028
    endif																								;028
...
;028 start
/******************************************************************************
*     ORDER ACTION INFORMATION FOR RADIOLOGY CATALOG WITH CPT MODIFICATION    *
******************************************************************************/
select into "nl:"
from  orders o,
      order_action oa,
...
join pl
  where pl.person_id = outerjoin(oa.action_personnel_id)
join pa
    where outerjoin(pl.PERSON_ID)=(pa.PERSON_ID)
    and pa.ACTIVE_IND = outerjoin(1)
    and pa.PRSNL_ALIAS_TYPE_CD=outerjoin(NPI_CD)    ;021
order by 
	oa.order_id
head oa.order_id
    orders->qual[d1.seq].enter_by                  = pl.name_full_formatted
    orders->qual[d1.seq].enter_npi                 = pa.alias  
    orders->qual[d1.seq].communication_type        = uar_get_code_display(oa.communication_type_cd)  
with nocounter
;028 end
